...
addon_icon, ls, get_icon, backup_cast_thumbnail = kodi_utils.addon_icon, kodi_utils.local_string, kodi_utils.get_icon, get_icon(
    'genre_family')
...
setting_base, label_base, ratings_icon_base = 'fenda.extras.%s.button', 'button%s.label', 'fenda_flags/ratings/%s'
...
items_list_ids = (recommended_id, year_id, genres_id,
                  networks_id, collection_id)
text_list_ids = (reviews_id, trivia_id, blunders_id,
                 parentsguide_id, comments_id)
art_ids = (posters_id, fanarts_id)
finished_tvshow = ('', 'Ended', 'Canceled')
parentsguide_levels = {'mild': ls(32996), 'moderate': ls(
    32997), 'severe': ls(32998), 'none': ls(33070)}
...
meta_ratings_values = (('Meta', 'metascore', 1), ('Tom/Critic', 'tomatometer', 2),
                       ('Tom/User', 'tomatousermeter', 3), ('IMDb', 'imdb', 4), ('TMDb', 'tmdb', 5))
ratings_null = ('', '%')
_images = Images().run
...
class Extras(BaseDialog):
    ...
    def onAction(self, action):
        ...
        if action == self.info_action:
            ...
            if not focus_id in items_list_ids:
                return
            show_busy_dialog()
            ...
            meta = function('tmdb_id', chosen_listitem.getProperty(
                'tmdb_id'), metadata_user_info(), get_datetime())
            hide_busy_dialog()
            self.show_extrainfo(self.media_type, meta,
                                meta.get('poster', empty_poster))
        ...
        if action in self.selection_actions:
            ...
            if self.control_id in items_list_ids:
                ...
                self.new_params = {'mode': 'extras_menu_choice', 'tmdb_id': chosen_var,
                                   'media_type': self.media_type, 'is_external': self.is_external, 'stacked': 'true'}
                return window_manager(self)
            elif self.control_id == cast_id:
                self.set_current_params()
                self.new_params = {'mode': 'person_data_dialog', 'query': chosen_var,
                                   'reference_tmdb_id': self.tmdb_id, 'is_external': self.is_external, 'stacked': 'true'}
                return window_manager(self)
            elif self.control_id == videos_id:
                chosen = imdb_videos_choice(self.get_attribute(self, chosen_var)[
                                            self.get_position(self.control_id)]['videos'], self.poster)
                if not chosen:
                    return
                if windowed_playback():
                    return self.open_window(('windows.videoplayer', 'VideoPlayer'), 'videoplayer.xml', meta=self.meta, video=chosen)
                self.set_current_params()
                ...
                if self.control_id == parentsguide_id:
                    ...
                    end_index = self.show_text_media(text=self.get_attribute(
                        self, chosen_var), current_index=self.get_position(self.control_id))
                    self.select_item(self.control_id, end_index)
            elif self.control_id in art_ids:
                end_index = _images({'mode': 'imageviewer', 'all_images': self.get_attribute(
                    self, chosen_var), 'current_index': self.get_position(self.control_id)})
                self.select_item(self.control_id, end_index)
            else:
                return
    ...
    def make_plot_and_tagline(self):
        self.plot = self.meta_get(
            'tvshow_plot', '') or self.meta_get('plot', '') or ''
        if not self.plot:
            return
    ...
    def make_cast(self):
        if not cast_id in self.enabled_lists:
            return
    ...
    def make_recommended(self):
        if not recommended_id in self.enabled_lists:
            return
        try:
            ...
            if len(data) in (20, 21):
                try:
                    data += function(self.tmdb_id, 2)['results'][1:]
                except:
                    pass
            item_list = list(self.make_tmdb_listitems(data))
            self.setProperty('recommended.number',
    ...
    def make_reviews(self):
        if not reviews_id in self.enabled_lists:
            return
        ...
        try:
            ...
            item_list = list(builder())
            self.setProperty('imdb_reviews.number',
                             count_insert % len(item_list))
    ...
    def make_comments(self):
        if not comments_id in self.enabled_lists:
            return
        ...
        try:
            ...
            item_list = list(builder())
            self.setProperty('trakt_comments.number',
                             count_insert % len(item_list))
    ...
    def make_trivia(self):
        if not trivia_id in self.enabled_lists:
            return
        ...
        try:
            ...
            item_list = list(builder())
            self.setProperty('imdb_trivia.number',
                             count_insert % len(item_list))
    ...
    def make_blunders(self):
        if not blunders_id in self.enabled_lists:
            return
        ...
        try:
            ...
            item_list = list(builder())
            self.setProperty('imdb_blunders.number',
                             count_insert % len(item_list))
    ...
    def make_parentsguide(self):
        if not parentsguide_id in self.enabled_lists:
            return
        ...
        try:
            ...
            item_list = list(builder())
            self.setProperty('imdb_parentsguide.number',
                             count_insert % len(item_list))
    ...
    def make_videos(self):
        if not videos_id in self.enabled_lists:
            return
        ...
        try:
            ...
            item_list = list(builder())
            self.setProperty('imdb_videos.number',
                             count_insert % len(item_list))
    ...
    def make_artwork(self, image_type):
        if image_type == 'poster' and not posters_id in self.enabled_lists:
            return
        elif not fanarts_id in self.enabled_lists:
            return
        ...
        def builder():
            for count, item in enumerate(data, 1):
                try:
                    listitem = self.make_listitem()
                    thumbnail = change_image_resolution(
                        item[0], 'w300' if 'image.tmdb' in item[0] else '/preview/')
        ...
        try:
            ...
            if not self.meta_user_info['extra_fanart_enabled']:
                all_images = [i for i in all_images if 'image.tmdb' in i]
            data = [(change_image_resolution(
                i, 'original' if 'image.tmdb' in i else '/fanart/'), self.title) for i in all_images]
            if image_type == 'poster':
                _id, art_list_id, self.tmdb_posters, used_image, default_image = posters_id, 'tmdb_posters', data, self.poster, empty_poster
            else:
    ...
    def make_genres(self):
        if not genres_id in self.enabled_lists:
            return
        try:
            function = tmdb_movies_genres if self.media_type == 'movie' else tmdb_tv_genres
            genre_dict = genres_choice(
                self.media_type, self.genre, '', return_genres=True)
            genre_list = ','.join([i['value'][0] for i in genre_dict])
            data = self.remove_current_tmdb_mediaitem(
                function(genre_list, 1)['results'])
            item_list = list(self.make_tmdb_listitems(data))
            self.setProperty('more_from_genres.number',
                             count_insert % len(item_list))
    ...
    def make_network(self):
        if not networks_id in self.enabled_lists:
            return
        try:
            ...
            item_list = list(self.make_tmdb_listitems(data))
            self.setProperty('more_from_networks.number',
                             count_insert % len(item_list))
    ...
    def make_collection(self):
        if self.media_type != 'movie':
            return
        if not collection_id in self.enabled_lists:
            return
        try:
        ...
        try:
            ...
            item_list = list(self.make_tmdb_listitems(
                sorted(data['parts'], key=lambda k: k['release_date'] or '2050')))
            self.setProperty('more_from_collection.name', data['title'])
            self.setProperty('more_from_collection.overview',
                             data['plot'] or data['title'])
            self.setProperty('more_from_collection.poster',
                             data['poster'] or empty_poster)
            self.setProperty('more_from_collection.number',
                             count_insert % len(item_list))
    ...
    def get_extra_ratings(self):
        ...
        if not current_settings:
            return None, None
        data = self.meta_get('extra_ratings', None) or fetch_ratings_info(
    ...
    def get_finish(self):
        ...
        if self.duration_data:
            ...
            if any(i in kodi_clock for i in ('AM', 'PM')):
                ...
                _format = '%H:%M'
            if self.percent_watched in ('0', '100'):
                remaining_time = self.duration_data
            else:
                remaining_time = (
                    (100 - int(self.percent_watched))/100) * self.duration_data
    ...
    def get_progress(self):
        self.percent_watched = get_progress_percent(
            get_bookmarks(self.watched_indicators, 'movie'), self.tmdb_id)
        if not self.percent_watched:
            try:
                watched_info = get_watched_info_movie(self.watched_indicators)
                self.percent_watched = '100' if get_watched_status_movie(
    ...
    def get_next_episode(self):
        ...
        try:
            ...
            info = [i for i in ep_list if i['media_ids']
                    ['tmdb'] == self.tmdb_id][0]
            current_season = info['season']
            current_episode = info['episode']
            season_data = self.meta_get('season_data')
            curr_season_data = [
                i for i in season_data if i['season_number'] == current_season][0]
        except:
            self.nextep_season, self.nextep_episode = 1, 1
        if curr_season_data:
            try:
                adjust_hours = date_offset()
                if current_episode >= curr_season_data['episode_count']:
                    current_season, current_episode, new_season = current_season + 1, 1, True
                else:
                    current_episode, new_season = current_episode + 1, False
                episodes_data = episodes_meta(
                    current_season, self.meta, self.meta_user_info)
                item = [i for i in episodes_data if i['episode']
                        == current_episode][0]
                item_get = item.get
                nextep_season, nextep_episode = item_get(
                    'season'), item_get('episode')
                episode_date, premiered = adjust_premiered_date(
                    item_get('premiered'), adjust_hours)
            except:
                pass
    ...
    def make_tvshow_browse_params(self):
        ...
        if show_all_episodes:
            if all_episodes == 1 and total_seasons > 1:
                url_params = {'mode': 'build_season_list',
                              'tmdb_id': self.tmdb_id}
            else:
                url_params = {'mode': 'build_episode_list',
                              'tmdb_id': self.tmdb_id, 'season': 'all'}
        else:
            url_params = {'mode': 'build_season_list', 'tmdb_id': self.tmdb_id}
        return url_params

    def original_poster(self):
        poster = self.meta_get('custom_poster') or self.meta_get(
            self.poster_main) or self.meta_get(self.poster_backup) or empty_poster
        self.current_poster = poster
        if 'image.tmdb' in self.current_poster:
            try:
                poster = change_image_resolution(
                    self.current_poster, 'original')
            except:
                pass
        elif not self.check_image_cached(self.current_poster):
            self.current_poster = self.meta_get(self.poster_backup) or ''
        return poster

    def original_fanart(self):
        fanart = self.meta_get('custom_fanart') or self.meta_get(
            self.fanart_main) or self.meta_get(self.fanart_backup) or addon_fanart
        return fanart
    ...
    def make_tmdb_listitems(self, data):
        ...
        for item in data:
            try:
                ...
                if poster_path:
                    thumbnail = tmdb_image_base % ('w300', poster_path)
                else:
                    thumbnail = empty_poster
    ...
    def check_image_cached(self, image):
        try:
            if image == '':
                return True
            if fetch_kodi_imagecache(image):
                return True
            return False
        except:
            return True
    ...
    def show_trailers(self):
        ...
        chosen = trailer_choice(self.media_type, self.poster, self.tmdb_id, self.meta_get(
            'trailer'), self.meta_get('all_trailers'))
        if not chosen:
            return ok_dialog()
        elif chosen == 'canceled':
            return
        if windowed_playback():
    ...
    def show_extrainfo(self, media_type=None, meta=None, poster=None):
        text = media_extra_info(
            {'media_type': media_type or self.media_type, 'meta': meta or self.meta})
    ...
    def play_nextep(self):
        if self.nextep_season == None:
            return ok_dialog(text=33116)
        url_params = {'mode': 'playback.media', 'media_type': 'episode',
                      'tmdb_id': self.tmdb_id, 'season': self.nextep_season, 'episode': self.nextep_episode}
        Sources().playback_prep(url_params)
    ...
    def show_director(self):
        ...
        if not director:
            return
        self.current_params = {'mode': 'extras_menu_choice', 'tmdb_id': self.tmdb_id,
                               'media_type': self.media_type, 'is_external': self.is_external}
        self.new_params = {'mode': 'person_data_dialog',
                           'query': director, 'is_external': self.is_external}
        window_manager(self)

    def show_options(self):
        params = {'content': self.options_media_type, 'tmdb_id': str(
            self.tmdb_id), 'poster': self.poster, 'is_external': self.is_external, 'from_extras': 'true'}
        return options_menu_choice(params, self.meta)

    def show_recommended(self):
        mode, action = ('build_movie_list', 'tmdb_movies_recommendations') if self.media_type == 'movie' else (
            'build_tvshow_list', 'tmdb_tv_recommendations')
        self.selected = self.folder_runner(
            {'mode': mode, 'action': action, 'tmdb_id': self.tmdb_id})
        self.close()
    ...
    def playback_choice(self):
        params = {'media_type': self.media_type, 'poster': self.poster,
                  'meta': self.meta, 'season': None, 'episode': None}
    ...
    def assign_buttons(self):
        ...
        for item in button_ids[:-1]:
            ...
            self.setProperty(label_base % item, ls(
                extras_button_label_values[self.media_type][button_action]))
    ...
    def set_current_params(self, set_starting_position=True):
        ...
        if set_starting_position:
            self.current_params['starting_position'] = [
                self.control_id, self.get_position(self.control_id)]

    def set_starting_constants(self, kwargs):
        self.separator = separator % self.highlight_var()
        self.meta = kwargs['meta']
        self.meta_get = self.meta.get
        self.media_type, self.options_media_type = self.meta_get(
            'mediatype'), kwargs['options_media_type']
        ...
        self.tmdb_id, self.imdb_id = self.meta_get(
            'tmdb_id'), self.meta_get('imdb_id')
        self.folder_runner = activate_window if self.is_external == 'true' else container_update
        self.meta_user_info, self.enabled_lists, self.enable_scrollbars = metadata_user_info(
        ), extras_enabled_menus(), extras_enable_scrollbars()
        self.poster_resolution, self.watched_indicators, self.omdb_api = get_resolution(
        )['poster'], watched_indicators(), omdb_api_key()
        ...
        self.title, self.year, self.rootname = self.meta_get(
            'title'), str(self.meta_get('year')), self.meta_get('rootname')
        self.poster, self.fanart = self.original_poster(), self.original_fanart()
        self.clearlogo = self.meta_get('custom_clearlogo') or self.meta_get(
            self.clearlogo_main) or self.meta_get(self.clearlogo_backup) or ''
        self.rating = str(round(self.meta_get('rating'), 1)) if self.meta_get(
            'rating') not in (0, 0.0, None) else None
        self.mpaa, self.genre, self.network = self.meta_get(
            'mpaa'), self.meta_get('genre'), self.meta_get('studio') or ''
        self.status, self.duration_data = self.extra_info_get('status', '').replace(
            ' Series', ''), int(float(self.meta_get('duration'))/60)
        self.status_infoline_value = self.make_status_infoline()
        self.make_plot_and_tagline()

    def set_properties(self):
        self.assign_buttons()
        self.setProperty('media_type', self.media_type), self.setProperty(
            'title', self.title), self.setProperty('year', self.year), self.setProperty('plot', self.plot)
        self.setProperty('genre', self.genre), self.setProperty(
            'network', self.network), self.setProperty('enable_scrollbars', self.enable_scrollbars)

    def make_status_infoline(self):
        status_str = self.status
        if self.media_type == 'tvshow' and self.status == 'Returning':
            try:
                next_aired_date = self.extra_info_get(
                    'next_episode_to_air')['air_date']
            except:
                next_aired_date = None
            if next_aired_date:
                status_str = '%s %s' % (self.status, adjust_premiered_date(
                    next_aired_date, date_offset())[0].strftime('%d %B %Y'))
        return status_str

    def set_infoline1(self, remove_rating=False):
        self.set_label(2001, self.separator.join([i for i in (
            self.year, None if remove_rating else self.rating, self.mpaa, self.get_duration(), self.status_infoline_value) if i]))

    def set_infoline2(self):
        if self.media_type == 'movie':
            line2 = self.separator.join(
                [self.get_progress(), self.get_finish()])
        else:
            line2 = self.separator.join([i for i in (
                self.get_next_episode(), self.get_last_aired(), self.get_next_aired()) if i])
        self.set_label(3001, line2)

    def youtube_installed_check(self):
        if not self.addon_installed(youtube_check):
            return False
        if not self.addon_enabled(youtube_check):
            return False
        return True
...
class ShowTextMedia(BaseDialog):
    ...
    def update_text(self, direction=None):
        ...
        self.set_text(2001, self.text[self.position]
                      if self.text_is_list else self.text)
        if self.text_is_list:
            if self.position == 0:
            ...
            self.setProperty('previous_display', 'false'), self.setProperty(
                'next_display', 'false')
