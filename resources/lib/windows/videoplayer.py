...
class VideoPlayer(BaseDialog):
    ...
    def monitor(self):
        ...
        while self.player.isPlayingVideo():
            ...
            progress += 1
            if progress == 3:
                self.progress_display_toggle('false', incl_info=True)
        self.exit()
    ...
    def seek_forward(self):
        ...
        try:
            self.player.seekTime(
                min(self.player.getTime() + 10.0, self.player.getTotalTime() - 1))
            self.sleep(1000)
        except:
            pass
    ...
    def seek_forward_small(self):
        ...
        try:
            self.player.seekTime(
                min(self.player.getTime() + 2.0, self.player.getTotalTime() - 1))
            self.sleep(1000)
        except:
            pass
        self.progress_display_toggle('false')
    ...
    def progress_display_toggle(self, toggle, incl_info=False):
        if incl_info:
            self.setProperty('display_fenda_hint', toggle)
    ...
    def closing_action(self):
        if self.progress_display_status() == 'true':
            self.progress_display_toggle('false')
        else:
            self.stop()
