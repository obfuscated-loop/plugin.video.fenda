...
class Select(BaseDialog):
    def __init__(self, *args, **kwargs):
        ...
        self.enable_context_menu = self.kwargs.get(
            'enable_context_menu', 'false') == 'true'
        self.item_list = []
        self.chosen_indexes = []
        self.selected = None
        self.set_properties()
    ...
    def onInit(self):
        ...
        if self.preselect:
            if len(self.preselect) == len(self.item_list):
                self.setProperty('select_button', 'deselect_all')
    ...
    def onClick(self, controlID):
        ...
        if controlID in button_ids:
            if controlID == ok_id:
                ...
                if controlID == selectall_id:
                    status, select_property, self.chosen_indexes = 'checked', 'deselect_all', item_list_indexes
                else:
                    status, select_property, self.chosen_indexes = '', 'select_all', []
                for index in item_list_indexes:
                    self.item_list[index].setProperty('check_status', status)
                self.setProperty('select_button', select_property)
                try:
                    self.setFocusId(ok_id)
                except:
                    pass
        else:
        ...
        def builder():
            for count, item in enumerate(self.items, 1):
                ...
                if 'line2' in item:
                    ...
                    line2 = ''
                if 'icon' in item:
                    listitem.setProperty('icon', item['icon'])
                else:
    ...
    def make_context_menu(self, chosen_listitem, context_menu_type='imdb_keywords'):
        ...
        if context_menu_type == 'imdb_keywords':
            ...
            menu_item = json.dumps({'mode': mode, 'action': 'imdb_keywords_list_contents',
                                   'iconImage': 'imdb', 'list_id': keyword.lower()})
