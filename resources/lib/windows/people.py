...
roles_exclude = ('himself', 'herself', 'self',
                 'narrator', 'voice', 'voice (voice)')
button_ids = [10, 11, 50]
...
tmdb_list_ids = (more_from_movies_id, more_from_tvshows_id,
                 more_from_director_id)
...
class People(BaseDialog):
    def __init__(self, *args, **kwargs):
        ...
        self.set_properties()
        self.tasks = (self.set_infoline1, self.make_trivia, self.make_videos,
                      self.make_movies, self.make_tvshows, self.make_director)
    ...
    def onClick(self, controlID):
        ...
        if controlID in button_ids:
            if controlID == 10:
                ...
                _images({'mode': 'people_tagged_image_results',
                        'actor_name': self.person_name, 'actor_id': self.person_id})
            elif controlID == 50:
    ...
    def onAction(self, action):
        ...
        if action == self.info_action:
            ...
            tmdb_list_ids = (more_from_movies_id,
                             more_from_tvshows_id, more_from_director_id)
            if not focus_id in tmdb_list_ids:
                return
            show_busy_dialog()
            from modules.metadata import movie_meta, tvshow_meta
            chosen_listitem = self.get_listitem(focus_id)
            media_type = 'movie' if focus_id in (
                more_from_movies_id, more_from_director_id) else 'tvshow'
            ...
            hide_busy_dialog()
            self.show_extrainfo(
                media_type, meta, meta.get('poster', empty_poster))
        if not self.control_id:
            return
        if action in self.selection_actions:
            chosen_listitem = self.get_listitem(self.control_id)
            chosen_var = chosen_listitem.getProperty(
            ...
            if self.control_id in (more_from_movies_id, more_from_tvshows_id, more_from_director_id):
                if self.control_id in (more_from_movies_id, more_from_director_id):
                    media_type = 'movie'
                else:
                    media_type = 'tvshow'
                self.set_current_params()
                self.new_params = {'mode': 'extras_menu_choice', 'tmdb_id': chosen_var,
                                   'media_type': media_type, 'is_external': self.is_external, 'stacked': 'true'}
                return window_manager(self)
                ...
                chosen = dialogs.imdb_videos_choice(self.get_attribute(self, chosen_var)[
                                                    self.get_position(self.control_id)]['videos'], thumb)
                if not chosen:
                    return
                if extras_windowed_playback():
    ...
    def show_extrainfo(self, media_type, meta, poster):
        text = dialogs.media_extra_info_choice(
    ...
    def make_person_data(self):
        if self.query not in empty_check:
            try:
                ...
                if not self.person_id:
                    try:
                        self.person_id = data[0]['id']
                    except:
                        pass
            except:
                self.person_id = self.actor_id
        else:
            self.person_id = self.actor_id
        if not self.person_id:
            notification(32760)
        ...
        try:
            self.person_gender = gender_dict[person_info.get('gender')]
        except:
            self.person_gender = ''
        place_of_birth = person_info.get('place_of_birth')
        if place_of_birth:
            self.person_place_of_birth = place_of_birth
        else:
            self.person_place_of_birth = ''
        biography = person_info.get('biography', None)
        if biography:
            self.person_biography = biography
        else:
            self.person_biography = ''
        birthday = person_info.get('birthday')
        if birthday:
            self.person_birthday = birthday
        else:
            self.person_birthday = ''
        deathday = person_info.get('deathday')
        if deathday:
            self.person_deathday = deathday
        else:
            self.person_deathday = ''
        if self.person_deathday:
            self.person_age = calculate_age(
                self.person_birthday, '%Y-%m-%d', self.person_deathday)
        ...
        try:
            ...
            self.tvshow_data = [
                i for i in acting_data if i['media_type'] == 'tv']
            self.director_data = [
                i for i in directing_data if i['job'].lower() == 'director']
        except:
            self.movie_data, self.tvshow_data, self.director_data = [], [], []
        ...
        try:
            if media_type == 'movie':
                ...
                if self.exclude_non_acting:
                    try:
                        data = [i for i in data if not 99 in i['genre_ids']
                                and not i['character'].lower() in roles_exclude]
                    except:
                        pass
                ...
                if self.exclude_non_acting:
                    try:
                        data = [i for i in data if not any(
                            x in genres_exclude for x in i['genre_ids']) and not i['character'].lower() in roles_exclude]
                    except:
                        pass
            else:
        ...
        for item in data:
            try:
                ...
                if not poster_path:
                    ...
                    thumbnail = tmdb_image_base % (
                        self.poster_resolution, poster_path)
    ...
    def sort_items_by_release(self, data, key):
        ...
        future = sorted([i for i in data if i[key].split(
            '-')[0] > self.current_year], key=lambda x: x[key], reverse=True)
        present = sorted([i for i in data if i[key] and i not in future],
