...
def window_manager(obj):
	...
	def get_stack():
		try: window_stack = json.loads(get_property('fenda.window_stack'))
		except: window_stack = []
		return window_stack
...
class BaseDialog(window_xml_dialog):
	...
	def highlight_var(self, force=False):
		...
		if not custom_var: return get_infolabel('Window(10000).Property(fenda.main_highlight)')
		if 'Custom' in self.args[1]: var = custom_var
		elif force:
			if use_custom_skins(): var = custom_var
			else: var = 'Window(10000).Property(fenda.main_highlight)'
		else: var = 'Window(10000).Property(fenda.main_highlight)'
