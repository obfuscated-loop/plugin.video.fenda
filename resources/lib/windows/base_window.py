...
extras_keys, folder_options = ('upper', 'uppercase', 'italic', 'capitalize', 'black',
                               'mono', 'symbol'), ('xml', '1080', '720', '1080p', '720p', '1080i', '720i', '16x9')
needed_font_values = ((21, False, 'font10'), (26, False, 'font12'), (30, False,
                      'font13'), (33, False, 'font14'), (38, False, 'font16'), (60, True, 'font60'))
addon_skins_folder = 'special://home/addons/plugin.video.fenda/resources/skins/Default/1080i/'
...
def window_manager(obj):
    def close():
        obj.close()
        clear_property('fenda.window_loaded')
        clear_property('fenda.window_stack')

    def monitor():
        timer = 0
        while not get_property('fenda.window_loaded') == 'true' and timer <= 5:
            sleep(50)
            timer += 0.05
        hide_busy_dialog()
        obj.close()
    ...
    def get_stack():
        try:
            window_stack = json.loads(get_property('fenda.window_stack'))
        except:
            window_stack = []
        return window_stack
    ...
    def add_to_stack(params):
        ...
        set_property('fenda.window_stack', json.dumps(window_stack))
...
def window_player(obj):
    ...
    try:
        ...
        if 'plugin.video.youtube' in window_player_url:
            if not addon_installed('plugin.video.youtube') or not addon_enabled('plugin.video.youtube'):
                return
        clear_property('fenda.window_loaded')
        current_params = obj.current_params
...
class BaseDialog(window_xml_dialog):
    ...
    def highlight_var(self, force=False):
        ...
        if not custom_var:
            return get_infolabel('Window(10000).Property(fenda.main_highlight)')
        if 'Custom' in self.args[1]:
            var = custom_var
        elif force:
            if use_custom_skins():
                var = custom_var
            else:
                var = 'Window(10000).Property(fenda.main_highlight)'
        else:
            var = 'Window(10000).Property(fenda.main_highlight)'
...
class FontUtils:
    def execute_custom_fonts(self):
        ...
        for item in needed_font_values:
            replacement_values_append(self.match_font(*item))
        for item in list_dirs(translate_path(addon_skins_folder))[1]:
            self.replace_font(item, replacement_values)
        for item in ((current_skin_prop, self.current_skin), (use_skin_fonts_prop, self.use_skin_fonts)):
            set_property(*item)

    def get_skin_folder(self):
        skin_folder = None
        try:
            skin_folder = mdParse(translate_path('special://skin/addon.xml')).getElementsByTagName(
                'extension')[0].getElementsByTagName('res')[0].getAttribute('folder')
            if not skin_folder:
                skin_folder = [i for i in list_dirs(translate_path(
                    'special://skin'))[0] if i in folder_options][0]
        except:
            pass
        return skin_folder
    ...
    def skin_change_check(self):
        ...
        if self.current_skin != get_property(current_skin_prop):
            return True
        if self.use_skin_fonts != get_property(use_skin_fonts_prop):
            return True
        return False
    ...
    def match_font(self, size, bold, fallback):
        ...
        compatibility_fonts = [i['name'] for i in self.skin_font_info if i['name']
                               == fallback and i['size'] in compatibility_range]
        if compatibility_fonts:
            return (font_tag, compatibility_fonts[0])
        sized_fonts = [
            i for i in self.skin_font_info if i['size'] in size_range]
        if not sized_fonts:
            return fallback
    ...
    def get_font_info(self, skin_font_xml):
        ...
        try:
            ...
            try:
                fontset = [i for i in all_fonts if i.getAttribute(
                    'id').lower() == 'default'][0]
            except:
                fontset = all_fonts[0]
            for item in fontset.getElementsByTagName('font'):
                try:
                    ...
                    continue
                try:
                    size = int(item.getElementsByTagName(
                        'size')[0].firstChild.data)
                except:
                    continue
                try:
                    style = item.getElementsByTagName(
                        'style')[0].firstChild.data.lower()
                except:
                    style = ''
                ...
                if not extra_styles:
                    extra_styles = any(
                        item in name_compare for item in extras_keys)
                results_append({'name': name, 'size': size,
                               'bold': bold, 'extra_styles': extra_styles})
        except:
            pass
        return results
