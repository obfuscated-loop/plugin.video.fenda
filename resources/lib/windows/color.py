...
button_ids = (10, 11)
palettes = {'basic': palette_basic, 'material_design': palette_material_design,
            'webcolors': palette_web, 'rainbow': palette_rainbow}
...
class SelectColor(BaseDialog):
    ...
    def set_properties(self):
        ...
        self.setProperty('current_palette_name',
                         self.current_palette.capitalize())

    def palette_switcher(self):
        try:
            self.current_palette = palette_list[palette_list.index(
    ...
    def palette_check(self):
        if self.path_exists(colorpalette_path):
            status = True
        else:
            self.busy_dialog()
            try:
