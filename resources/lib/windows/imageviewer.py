...
class ThumbImageViewer(BaseDialog):
    ...
    def onAction(self, action):
        ...
        if action in self.selection_actions:
            if chosen_listitem.getProperty('next_page_item') == 'true':
                self.new_page()
            else:
            ...
            choice = self.make_context_menu(
                enable_delete=chosen_listitem.getProperty('delete') == 'true')
            if choice:
                if choice == 'delete_image':
                    return self.reset_after_delete(chosen_listitem, position)
                # download
                name, thumb, path = chosen_listitem.getProperty(
    ...
    def make_page(self):
        try:
            self.set_properties()
            if self.next_page_params.get('page_no', 'final_page') != 'final_page':
                self.make_next_page()
    ...
    def make_context_menu(self, enable_delete):
        ...
        if enable_delete:
            choices_append((ls(32785), 'delete_image'))
        else:
            choices_append((ls(32747), 'download_image'))
    ...
    def new_page(self):
        try:
            ...
            self.list_items, self.next_page_params = self.ImagesInstance.run(
    ...
    def make_next_page(self):
        try:
            ...
            listitem.setProperty('name', nextpage_str %
    ...
    def reset_after_delete(self, choice, position):
        ...
        self.ImagesInstance.run({'mode': 'delete_image', 'image_url': choice.getProperty(
            'path'), 'thumb_url': choice.getProperty('thumb')})
        while not self.get_home_property('delete_image_finished') == 'true':
            self.sleep(10)
        self.reset_window(self.window_id)
        self.list_items = self.ImagesInstance.browser_image(
            download_directory('image'), return_items=True)
        self.make_page()
