...
list_ids = {'poster': 2020, 'fanart': 2021, 'clearlogo': 2022, 'banner': 2023,
            'clearart': 2024, 'landscape': 2025, 'discart': 2026, 'keyart': 2027}
...
class SelectArtwork(BaseDialog):
    ...
    def onInit(self):
        for image_type, image_list in self.images.items():
            self.make_menu(image_type, image_list)
            ...
            for count, item in enumerate(image_list):
                try:
                    ...
                    listitem.setProperty(
                        'thumbnail', change_image_resolution(item, res_prop))
                    ...
                    if item == self.get_attribute(self, image_type):
                        ...
                        listitem.setProperty('check_status', '')
                    yield listitem
                except:
                    pass
        try:
    ...
    def get_starting_artwork(self):
        ...
        self.poster = self.meta_get('custom_poster') or self.meta_get(
            poster_main) or self.meta_get(poster_backup) or empty_poster
        self.fanart = self.meta_get('custom_fanart') or self.meta_get(
    ...
    def image_check(self, image):
        if 'image.tmdb' in image:
            return 'w300', 'TMDb'
        else:
            return '/preview/', 'Fanart.tv'
