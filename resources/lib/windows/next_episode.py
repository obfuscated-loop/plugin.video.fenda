...
button_actions = {'autoplay_nextep': {10: 'close', 11: 'play', 12: 'cancel'},
...
class NextEpisode(BaseDialog):
    ...
    def get_thumb(self):
        if avoid_episode_spoilers():
            thumb = self.original_fanart()
        else:
            thumb = self.meta.get('ep_thumb', None) or self.original_fanart()
        return thumb
        ...
        if self.selected == 'pause':
            ...
            while current_time <= end_time and self.selected == 'pause':
                try:
                    ...
                    pause_timer = time.strftime(
                        '%M:%S', time.gmtime(max(end_time - current_time, 0)))
