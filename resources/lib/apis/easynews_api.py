...
class EasyNewsAPI:
	...
	def get_farm_and_port(self, files):
		...
		if get_setting('fenda.easynews.use_custom_farm', 'False') == 'True': dl_farm, dl_port = get_setting('fenda.easynews.farm', dl_farm), get_setting('fenda.easynews.port', dl_port)
...
def clear_media_results_database():
	...
	dbcur.execute("SELECT id FROM maincache WHERE id LIKE 'fenda_EASYNEWS_SEARCH_%'")
	easynews_results = [str(i[0]) for i in dbcur.fetchall()]
	if not easynews_results: return 'success'
	try:
		dbcur.execute("DELETE FROM maincache WHERE id LIKE 'fenda_EASYNEWS_SEARCH_%'")
