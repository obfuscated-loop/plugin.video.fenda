...
def import_easynews():
    ...
    # if get_setting('fenda.easynews.api_version') == '0': return EasyNewsAPI()
...
class EasyNewsAPI:
    def __init__(self):
        ...
        self.username = get_setting('fenda.easynews_user')
        self.password = get_setting('fenda.easynews_password')
    ...
    def search(self, query, expiration=48):
        ...
        string = 'fenda_EASYNEWS_SEARCH_' + urlencode(self.params)
    ...
    def account_info(self):
        ...
        try:
            ...
            account_info = parseDOM(account_html, 'form', attrs={
                                    'id': 'accountForm'})
    ...
    def usage_info(self):
        ...
        try:
            ...
            usage_info = parseDOM(usage_html, 'div', attrs={
    ...
    def get_farm_and_port(self, files):
        ...
        if get_setting('fenda.easynews.use_custom_farm', 'False') == 'True':
            dl_farm, dl_port = get_setting('fenda.easynews.farm', dl_farm), get_setting(
                'fenda.easynews.port', dl_port)
    ...
    def _process_files_v3(self, results):
        def _process():
            for item in files:
                try:
                    post_hash, size, post_title, ext, duration, sig = item['hash'], item[
                        'bytes'], item['filename'], item['extension'], item['runtime'], item['sig']
                    if 'alangs' in item and item['alangs']:
                        language = item['alangs']
                    else:
                        language = ''
