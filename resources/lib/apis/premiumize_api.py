...
class PremiumizeAPI:
    ...
    def auth(self):
        ...
        data = {'grant_type': 'device_code',
                'client_id': self.client_id, 'code': device_code}
    ...
    def resolve_magnet(self, magnet_url, info_hash, store_to_cloud, title, season, episode):
        ...
        try:
            ...
            if not 'status' in result or result['status'] != 'success':
                return None
            valid_results = [i for i in result.get('content') if any(i.get(
                'path').lower().endswith(x) for x in extensions) and not i.get('link', '') == '']
            ...
            if season:
                ...
                file_url = max(valid_results, key=lambda x: int(
                    x.get('size'))).get('link', None)
                if not any(file_url.lower().endswith(x) for x in extensions):
                    file_url = None
            if file_url:
                if store_to_cloud:
                    Thread(target=self.create_transfer,
                           args=(magnet_url,)).start()
    ...
    def add_uncached_torrent(self, magnet_url, pack=False):
        ...
        def _return_failed(message=32574, cancelled=False):
            ...
            sleep(500)
            if cancelled:
                if confirm_dialog(heading=32733, text=32044):
    ...
    def user_cloud_all(self):
        string = 'fenda_pm_user_cloud_all_files'
        url = 'item/listall'
        return cache_object(self._get, string, url, False, 0.5)
