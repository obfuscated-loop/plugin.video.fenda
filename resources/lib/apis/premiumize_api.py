...
class PremiumizeAPI:
	...
	def clear_cache(self, clear_hashes=True):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""SELECT id FROM maincache WHERE id LIKE ?""", ('fenda_pm_user_cloud%',))
				try:
			...
			# DOWNLOAD LINKS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('fenda_pm_transfers_list',))
			...
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('fenda_pm_valid_hosts',))
