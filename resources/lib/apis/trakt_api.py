...
def call_trakt(path, params={}, data=None, is_delete=False, with_auth=True, method=None, pagination=False, page_no=1):
    def send_query():
        ...
        if with_auth:
            try:
                try:
                    expires_at = float(get_setting('fenda.trakt.expires'))
                except:
                    expires_at = 0.0
                ...
                pass
            token = get_setting('fenda.trakt.token')
            if token:
        ...
        try:
            if method:
                if method == 'post':
                    resp = requests.post(API_ENDPOINT %
                                         path, headers=headers, timeout=timeout)
                elif method == 'delete':
                ...
                resp = requests.delete(API_ENDPOINT %
                                       path, headers=headers, timeout=timeout)
            else:
                resp = requests.get(
                    API_ENDPOINT % path, params=params, headers=headers, timeout=timeout)
            ...
            return logger('Trakt Error', str(e))
        return resp
    ...
    if method == 'sort_by_headers' and 'X-Sort-By' in headers and 'X-Sort-How' in headers:
        try:
            result = sort_list(
                headers['X-Sort-By'], headers['X-Sort-How'], result, ignore_articles())
        except:
            pass
    if pagination:
        ...
        try:
            ...
            while not progressDialog.iscanceled() and time_passed < expires_in:
                ...
                response = requests.post(
                    API_ENDPOINT % 'oauth/device/token', data=json.dumps(data), headers=headers, timeout=timeout)
                status_code = response.status_code
                if status_code == 200:
                    result = response.json()
                    break
...
def trakt_refresh_token():
    data = {
        ...
        'grant_type': 'refresh_token', 'refresh_token': get_setting('fenda.trakt.refresh')}
...
def trakt_revoke_authentication(dummy=''):
    data = {'token': get_setting(
        'fenda.trakt.token'), 'client_id': CLIENT_ID, 'client_secret': CLIENT_SECRET}
...
def trakt_watched_status_mark(action, media, media_id, tvdb_id=0, season=None, episode=None, key='tmdb'):
    ...
    if media == 'movies':
        ...
        if media == 'episode':
            data = {'shows': [{'seasons': [{'episodes': [
                {'number': int(episode)}], 'number': int(season)}], 'ids': {key: media_id}}]}
        elif media == 'shows':
            data = {'shows': [{'ids': {key: media_id}}]}
        else:
            data = {'shows': [{'ids': {key: media_id},
                               'seasons': [{'number': int(season)}]}]}  # season
...
def trakt_progress(action, media, media_id, percent, season=None, episode=None, resume_id=None, refresh_trakt=False):
    if action == 'clear_progress':
        ...
        if media in ('movie', 'movies'):
            data = {'movie': {'ids': {'tmdb': media_id}},
                    'progress': float(percent)}
        else:
            data = {'show': {'ids': {'tmdb': media_id}}, 'episode': {
                'season': int(season), 'number': int(episode)}, 'progress': float(percent)}
...
def trakt_watchlist(media_type, dummy_arg):
    ...
    if not show_unaired_watchlist():
        ...
        str_format = '%Y-%m-%d' if media_type in (
            'movie', 'movies') else res_format
        data = [i for i in data if i.get('released', None) and js2date(
            i.get('released'), str_format, remove_time=True) <= current_date]
...
def trakt_fetch_collection_watchlist(list_type, media_type):
    def _process(params):
        ...
        return [{'media_ids': {'tmdb': i[key]['ids'].get('tmdb', ''), 'imdb': i[key]['ids'].get('imdb', ''), 'tvdb': i[key]['ids'].get('tvdb', '')}, 'title': i[key]['title'],
                ...
                for i in data]
    key, r_key, string_insert = ('movie', 'released', 'movie') if media_type in (
...
def trakt_fetch_movie_sets():
    ...
    def _process_metadata(media_id):
        try:
            meta = movie_meta('trakt_dict', media_id,
                              meta_user_info, current_date)
            extra_info = meta.get('extra_info', None)
            collection_id = extra_info.get('collection_id', None)
            if collection_id:
                collection_info_append({'title': extra_info.get(
                    'collection_name', None), 'id': collection_id})
        except:
            pass

    def _process(collection_info):
        media_ids = [i['media_ids']
                     for i in trakt_fetch_collection_watchlist('collection', 'movies')]
        threads = list(make_thread_list(_process_metadata, media_ids))
        [i.join() for i in threads]
        collection_info = [i for n, i in enumerate(
            collection_info) if not i in collection_info[n + 1:]]  # remove duplicates
...
def remove_from_list(user, slug, data):
    ...
    notification(32576, 3000)
    trakt_sync_activities()
    if path_check('my_lists') or external():
        kodi_refresh()
    return result
...
def remove_from_watchlist(data):
    ...
    notification(32576, 3000)
    trakt_sync_activities()
    if path_check('trakt_watchlist') or external():
        kodi_refresh()
    return result
...
def remove_from_collection(data):
    ...
    notification(32576, 3000)
    trakt_sync_activities()
    if path_check('trakt_collection') or external():
        kodi_refresh()
    return result
...
def get_trakt_list_contents(list_type, user, slug, with_auth):
    ...
    if user == 'Trakt Official':
        ...
        params = {'path': 'users/%s/lists/%s/items', 'path_insert': (user, slug), 'params': {
            'extended': 'full'}, 'with_auth': with_auth, 'method': 'sort_by_headers'}
    return cache_trakt_object(_process, string, params)
...
def trakt_get_lists(list_type):
    ...
    params = {'path': path, 'params': {'limit': 1000},
              'pagination': False, 'with_auth': True}
    return cache_trakt_object(get_trakt, string, params)
...
def get_trakt_list_selection(list_choice=None):
    ...
    if list_choice == 'nav_edit':
        ...
        my_lists.insert(0, {'name': 'Collection', 'display': '[B][I]%s [/I][/B]' % ls(
            32499).upper(), 'user': 'Collection', 'slug': 'Collection'})
        my_lists.insert(0, {'name': 'Watchlist', 'display': '[B][I]%s [/I][/B]' % ls(
            32500).upper(),  'user': 'Watchlist', 'slug': 'Watchlist'})
    list_items = [{'line1': item['display']} for item in my_lists]
    kwargs = {'items': json.dumps(list_items), 'heading': ls(
        32193), 'narrow_window': 'true'}
...
def trakt_add_to_list(params):
    tmdb_id, tvdb_id, imdb_id, media_type = params['tmdb_id'], params[
        'tvdb_id'], params['imdb_id'], params['media_type']
    if media_type == 'movie':
...
def trakt_indicators_movies():
    def _process(item):
        ...
        if not tmdb_id:
            return
        obj = ('movie', tmdb_id, '', '',
               item['last_watched_at'], movie['title'])
        insert_append(obj)
    insert_list = []
    insert_append = insert_list.append
    params = {'path': 'sync/watched/movies%s',
              'with_auth': True, 'pagination': False}
...
def trakt_indicators_tv():
    def _process(item):
        ...
        if reset_at:
            reset_at = js2date(reset_at, res_format)
        show = item['show']
        ...
        for s in seasons:
            ...
            for e in episodes:
                ...
                if reset_at and reset_at > js2date(last_watched_at, res_format):
                    continue
                insert_append(('episode', tmdb_id, season_no,
                              e['number'], last_watched_at, title))
    insert_list = []
    insert_append = insert_list.append
    params = {'path': 'users/me/watched/shows?extended=full%s',
              'with_auth': True, 'pagination': False}
    result = get_trakt(params)
    threads = list(make_thread_list(_process, result))
    [i.join() for i in threads]
...
def trakt_progress_movies(progress_info):
    def _process(item):
        ...
        if not tmdb_id:
            return
        obj = ('movie', str(tmdb_id), '', '', str(round(
            item['progress'], 1)), 0, item['paused_at'], item['id'], item['movie']['title'])
        insert_append(obj)
    insert_list = []
    insert_append = insert_list.append
    progress_items = [i for i in progress_info if i['type']
                      == 'movie' and i['progress'] > 1]
...
def trakt_progress_tv(progress_info):
    ...
    progress_items = [i for i in progress_info if i['type']
                      == 'episode' and i['progress'] > 1]
    if not progress_items:
        return
    all_shows = [i['show'] for i in progress_items]
    all_shows = [i for n, i in enumerate(
        all_shows) if not i in all_shows[n + 1:]]  # remove duplicates
...
def trakt_official_status(media_type):
    if not addon_installed('script.trakt'):
        return True
    if not addon_enabled('script.trakt'):
        return True
    ...
    if exclude_http in ('true', ''):
        return True
    media_setting = 'scrobble_movie' if media_type in (
        'movie', 'movies') else 'scrobble_episode'
    try:
...
def trakt_calendar_days(recently_aired, current_date):
    if recently_aired:
        start, finish = (current_date - timedelta(days=14)
                         ).strftime('%Y-%m-%d'), '14'
    else:
        previous_days = int(get_setting(
            'fenda.trakt.calendar_previous_days', '3'))
        future_days = int(get_setting('fenda.trakt.calendar_future_days', '7'))
        start = (current_date - timedelta(days=previous_days)
                 ).strftime('%Y-%m-%d')
...
def trakt_get_activity():
    params = {'path': 'sync/last_activities%s',
              'with_auth': True, 'pagination': False}
    return get_trakt(params)
...
def trakt_sync_activities(force_update=False):
    ...
    if not get_setting('fenda.trakt.user', '') and not force_update:
        return 'no account'
    try:
    ...
    if _compare(latest_episodes['watched_at'], cached_episodes['watched_at']):
        trakt_indicators_tv()
    if _compare(latest_movies['paused_at'], cached_movies['paused_at']):
        refresh_movies_progress = True
    if _compare(latest_episodes['paused_at'], cached_episodes['paused_at']):
