...
class RealDebridAPI:
	...
	def clear_cache(self, clear_hashes=False):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""SELECT data FROM maincache WHERE id=?""", ('fenda_rd_user_cloud',))
				try: 
				...
				if not user_cloud_success:
					dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('fenda_rd_user_cloud',))
					clear_property("fenda_rd_user_cloud")
					for i in user_cloud_info_caches:
						dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('fenda_rd_user_cloud_info_%s' % i,))
			...
			# DOWNLOAD LINKS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('fenda_rd_downloads',))
			...
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('fenda_rd_valid_hosts',))
