...
class FurkAPI:
	...
	def get_api(self):
		api_key = get_setting('fenda.furk_api_key', '')
		if not api_key:
			try:
				user_name, user_pass = get_setting('fenda.furk_login'), get_setting('fenda.furk_password')
...
def clear_media_results_database():
	...
	dbcur.execute("SELECT id FROM maincache WHERE id LIKE 'fenda_FURK_SEARCH_%'")
	try:
		furk_results = [str(i[0]) for i in dbcur.fetchall()]
		if not furk_results: results.append('success')
		dbcur.execute("DELETE FROM maincache WHERE id LIKE 'fenda_FURK_SEARCH_%'")
		for i in furk_results: clear_property(i)
		results.append('success')
	except: results.append('failed')
	dbcur.execute("SELECT id FROM maincache WHERE id LIKE 'fenda_FURK_SEARCH_DIRECT_%'")
	try:
		furk_results = [str(i[0]) for i in dbcur.fetchall()]
		if not furk_results: results.append('success')
		dbcur.execute("DELETE FROM maincache WHERE id LIKE 'fenda_FURK_SEARCH_DIRECT_%'")
		for i in furk_results: clear_property(i)
		results.append('success')
	except: results.append('failed')
