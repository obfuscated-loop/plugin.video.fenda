...
class FurkAPI:
    ...
    def get_api(self):
        ...
        if not api_key:
            try:
                user_name, user_pass = get_setting(
                    'fenda.furk_login'), get_setting('fenda.furk_password')
    ...
    def file_protect(self, item_id, is_protected):
        try:
            url = base_url + \
                file_protect_url % (self.api_key, item_id, is_protected)
            return self._get(url)
        except:
            return
    ...
    def _process_files(self, url):
        ...
        if not self.check_status(result):
            return None
    ...
    def _process_tfiles(self, url):
        result = self._get(url)
        if not self.check_status(result) or result['found_files'] != '1':
            return None
