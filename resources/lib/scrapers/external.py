...
season_display, show_display, remain_str, pack_display = ls(
    32537), ls(32089), ls(32676), '%s (%s)'
pack_check = (season_display, show_display)
debrid_runners = {'Real-Debrid': ('Real-Debrid', RD_check), 'Premiumize.me': (
    'Premiumize.me', PM_check), 'AllDebrid': ('AllDebrid', AD_check)}
...
class source:
    def __init__(self, meta, source_dict, debrid_torrents, debrid_hosters, internal_scrapers, prescrape_sources, progress_dialog, disabled_ext_ignored=False):
        ...
        self.sleep_time = 100
        self.finish_early = get_setting('fenda.search.finish.early') == 'true'
        self.timeout = 60 if disabled_ext_ignored else int(
            get_setting('fenda.results.timeout', '20'))
    ...
    def results(self, info):
        if not self.source_dict:
            return
        try:
            self.media_type, self.tmdb_id, self.orig_title = info['media_type'], str(
                info['tmdb_id']), info['title']
            self.season, self.episode, self.total_seasons = info[
                'season'], info['episode'], info['total_seasons']
            ...
            if self.media_type == 'movie':
                ...
                self.data = {
                    'imdb': info['imdb_id'], 'title': self.title, 'aliases': aliases, 'year': self.year}
            else:
                try:
                    self.season_divider = [int(x['episode_count']) for x in self.meta['season_data'] if int(
                        x['season_number']) == int(self.meta['season'])][0]
                except:
    ...
    def get_sources(self):
        def _scraperDialog():
            ...
            while not self.progress_dialog.iscanceled() and not monitor.abortRequested():
                try:
                    alive_threads = [x.getName()
                                     for x in self.threads if x.is_alive()]
                    ...
                    percent = (max((time.time() - start_time), 0) /
                               float(self.timeout))*100
        ...
        if self.media_type == 'movie':
            ...
            self.source_dict = [
                i for i in self.source_dict if i[1].hasEpisodes]
            self.season_packs, self.show_packs = pack_enable_check(
                self.meta, self.season, self.episode)
            if self.season_packs:
                self.source_dict = [(i[0], i[1], '') for i in self.source_dict]
                pack_capable = [
                    i for i in self.source_dict if i[1].pack_capable]
                if pack_capable:
                    self.source_dict.extend(
                        [(i[0], i[1], ls(32537)) for i in pack_capable])
                    if self.show_packs:
                        self.source_dict.extend(
                            [(i[0], i[1], ls(32089)) for i in pack_capable])
    ...
    def process_movie_threads(self):
        for i in self.source_dict:
            ...
            threaded_object = Thread(target=self.get_movie_source, args=(
                provider, module), name=provider)
    ...
    def process_episode_threads(self):
        for i in self.source_dict:
            ...
            try:
                ...
                pack_arg = ''
            if pack_arg:
                provider_display = pack_display % (i[0], i[2])
            else:
                provider_display = provider
            threaded_object = Thread(target=self.get_episode_source, args=(
                provider, module, pack_arg), name=provider_display)
            threaded_object.start()
            self.threads_append(threaded_object)
        self.threads_completed = True
    ...
    def get_movie_source(self, provider, module):
        ...
        sources = _cache.get(provider, self.media_type,
                             self.tmdb_id, self.title, self.year, '', '')
        if sources == None:
            ...
            _cache.set(provider, self.media_type, self.tmdb_id,
                       self.title, self.year, '', '', sources, expiry_hours)
        if sources:
    ...
    def get_episode_source(self, provider, module, pack):
        ...
        sources = _cache.get(provider, self.media_type,
                             self.tmdb_id, self.title, self.year, s_check, e_check)
        if sources == None:
            if pack == show_display:
                expiry_hours = self.show_expiry
                sources = module().sources_packs(self.data, self.host_dict,
                                                 search_series=True, total_seasons=self.total_seasons)
            ...
            if not sources:
                expiry_hours = 1
            _cache.set(provider, self.media_type, self.tmdb_id, self.title,
                       self.year, s_check, e_check, sources, expiry_hours)
        if sources:
            if pack == season_display:
                sources = [i for i in sources if not 'episode_start' in i or i['episode_start']
                           <= self.episode <= i['episode_end']]
            elif pack == show_display:
                sources = [
                    i for i in sources if i['last_season'] >= self.season]
            if not self.background:
                self.process_quality_count(sources)
            self.sources.extend(sources)
    ...
    def process_results(self):
        self.processed_hosters = self.process_hosters(
            [i for i in self.final_sources if not 'hash' in i])
        self.non_torrent_sources += self.processed_hosters
        if not self.background and self.non_torrent_sources:
            self.process_quality_count_final(self.non_torrent_sources)
        self.processed_torrents = self.process_torrents(
            [i for i in self.final_sources if 'hash' in i])
            ...
            for i in sources:
                try:
                    ...
                    if 'hash' in i:
                        ...
                        i['hash'] = str(_hash)
                    display_name = clean_file_name(normalize(i['name'].replace(
                        'html', ' ').replace('+', ' ').replace('-', ' ')))
                    if 'name_info' in i:
                        quality, extraInfo = get_file_info(
    ...
    def process_hosters(self, hoster_sources):
        if not hoster_sources or not self.debrid_hosters:
            return []
        hoster_results = []
        for item in self.debrid_hosters:
            for k, v in item.items():
                hoster_results.extend(
                    [dict(i, **{'debrid': k}) for i in hoster_sources if i['source'].lower() in v])
    ...
    def process_torrents(self, torrent_sources):
        ...
        def _debrid_check_dialog():
            ...
            while not self.progress_dialog.iscanceled() and not monitor.abortRequested():
                try:
                    remaining_debrids = [
                        x.getName() for x in torrent_check_threads if x.is_alive() is True]
                    current_progress = max((time.time() - start_time), 0)
                    line1 = ', '.join(remaining_debrids).upper()
                    percent = int((current_progress/float(timeout))*100)
                    self.progress_dialog.update_scraper(
                        self.final_sd, self.final_720p, self.final_1080p, self.final_4k, self.final_total, line1, percent)
                    sleep(self.sleep_time)
        ...
        torrent_check_threads = [Thread(
            target=_process, args=debrid_runners[item], name=item) for item in self.debrid_torrents]
        if torrent_check_threads:
            [i.start() for i in torrent_check_threads]
            if self.background:
                [i.join() for i in torrent_check_threads]
            else:
