...
class source:
    def __init__(self, scrape_provider, scraper_name, folder_path):
        ...
        self.assignment_test_string = 'Fenda_FOLDERS_%s_%s' % (
            self.scrape_provider, '%s')
    ...
    def results(self, info):
        try:
            ...
            self.media_type, title, self.year = info.get(
                'media_type'), info.get('title'), int(info.get('year'))
            ...
            ) if self.media_type == 'episode' else self._year_query_list()
            ...
            def _process():
                for item in self.scrape_results:
                    try:
                        ...
                        if filter_title and not item[3] and not check_title(title, file_name, aliases, self.year, self.season, self.episode):
                            continue
    ...
    def _make_dirs(self, folder_info):
        ...
        for i in dirs:
            folder_files_append((i, 'folder', assigned_content))
        for i in files:
            folder_files_append((i, 'file', assigned_content))
        return folder_files
    ...
    def _scraper_worker(self, folder_results):
        scraper_threads = list(make_thread_list(
            self._scrape_directory, folder_results))
