...
class source:
    ...
    def results(self, info):
        try:
            if not enabled_debrids_check('rd'):
            ...
            self.media_type, title, self.tmdb_id = info.get(
                'media_type'), info.get('title'), info.get('tmdb_id')
            self.year, self.season, self.episode = int(
                info.get('year')), info.get('season'), info.get('episode')
            ...
            def _process():
                for item in self.scrape_results:
                    try:
                        ...
                        if filter_title and not item['assigned_content'] and not check_title(title, file_name, aliases, self.year, self.season, self.episode):
                            continue
                        display_name = clean_file_name(file_name).replace(
                            'html', ' ').replace('+', ' ').replace('-', ' ')
                        file_dl, size = item['url_link'], round(
                            float(item['bytes'])/1073741824, 2)
                        direct_debrid_link = item.get(
            ...
            from modules.kodi_utils import logger
            logger('Fenda real-debrid scraper Exception', str(e))
        internal_results(self.scrape_provider, self.sources)
        return self.sources

    def _gather_assigned_content(self):
        pre_assigned_content = gather_assigned_content("'Fenda_RD_%'")
    ...
    def _scrape_cloud(self):
        try:
            try:
                ...
                my_cloud_files = [
                    i for i in my_cloud_files if i['status'] == 'downloaded']
            except:
                return self.sources
            ...
            for item in my_cloud_files:
                ...
                elif not self.folder_query in folder_name:
                    continue
                else:
                    if self.media_type == 'movie' and not any(x in normalized for x in year_query_list):
                        continue
                    results_append((item['id'], False))
            ...
            for i in self.folder_results:
                threads_append(Thread(target=self._scrape_folders, args=(i,)))
    ...
    def _scrape_folders(self, folder_info):
        try:
            ...
            contents = [i for i in folder_files['files'] if i['selected']
            ...
            for item in contents:
                ...
                if self.media_type == 'episode' and not seas_ep_filter(self.season, self.episode, normalized):
                    continue
    ...
    def _scrape_downloads(self):
        try:
            ...
            my_downloads = [
                i for i in my_downloads if i['download'].lower().endswith(tuple(extensions))]
            scrape_results_append = self.scrape_results.append
            year_query_list = self._year_query_list()
            for item in my_downloads:
                normalized = normalize(item['filename'])
                folder_name = clean_title(normalized)
                if not self.folder_query in folder_name:
                    continue
                if self.media_type == 'movie':
                    if not any(x in normalized for x in year_query_list):
                        continue
