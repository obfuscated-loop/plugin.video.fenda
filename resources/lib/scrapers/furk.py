...
class source:
    ...
    def results(self, info):
        try:
            ...
            self.media_type, title, self.year, self.season, self.episode = info.get('media_type'), info.get(
                'title'), int(info.get('year')), info.get('season'), info.get('episode')
            self.search_title = clean_file_name(
                title).replace(' ', '+').replace('&', 'and')
            files = Furk.search(self._search_name(),
                                info.get('expiry_times')[0])
            if not files:
                return internal_results(self.scrape_provider, self.sources)
            cached_files = [i for i in files if i.get('type') not in (
                'default', 'audio', '') and i.get('is_ready') == '1']
                ...
                for item in cached_files:
                    try:
                        if self.media_type == 'movie':
                            ...
                            files_num_video = int(item['files_num_video'])
                        if files_num_video > 3:
                            package, season_compare, size = 'true', 'pack', float(
                                round(float(int(item['size']))/1073741824, 2))/files_num_video
                        else:
                            package, season_compare, size = 'false', self.season, round(
                                float(int(item['size']))/1073741824, 2)
                        file_name = normalize(item['name'])
                        if any(x in file_name.lower() for x in extras):
                            continue
                        if filter_title and not check_title(title, file_name, aliases, self.year, season_compare, self.episode):
                            continue
                        display_name = clean_file_name(file_name).replace(
                            'html', ' ').replace('+', ' ').replace('-', ' ')
                        file_id, file_dl = item['id'], item['url_dl']
                        video_quality, details = get_file_info(name_info=release_info_format(
                            file_name), default_quality=self._quality_estimate(int(item.get('width', 0))))
            ...
            from modules.kodi_utils import logger
            logger('Fenda furk scraper Exception', str(e))
    ...
    def _search_name(self):
        if self.media_type == 'movie':
            return '@name+%s+%d+|+%d+|+%d' % (self.search_title, self.year-1, self.year, self.year+1)
        else:
            return '@name+%s+@files+%s+|+%s+|+%s+|+%s+|+%s+|+%s+|+%s' % self.tvshow_query()
    ...
    def tvshow_query(self):
        return (self.search_title, 's%de%02d' % (self.season, self.episode), 's%02de%02d' % (self.season, self.episode), '%dx%02d' % (self.season, self.episode),
                '%02dx%02d' % (self.season, self.episode), '"season %d episode %d"' % (
                    self.season, self.episode), '"season %d episode %02d"' % (self.season, self.episode),
