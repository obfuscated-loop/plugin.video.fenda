...
class source:
    ...
    def results(self, info):
        try:
            ...
            files = EasyNews.search(
                self._search_name(), info.get('expiry_times')[0])
            if not files:
            ...
            def _process():
                for item in files:
                    try:
                        if item.get('short_vid', False):
                            continue
                        file_name = normalize(item['name'])
                        if any(x in file_name.lower() for x in extras):
                            continue
                        if filter_title and not check_title(title, file_name, self.aliases, self.year, self.season, self.episode):
                            continue
                        if filter_lang and not any(i in lang_filters for i in item['language']):
                            continue
                        display_name = clean_file_name(file_name).replace(
                            'html', ' ').replace('+', ' ').replace('-', ' ')
                        url_dl, size = item['url_dl'], round(
    ...
    def _search_name(self):
        if self.media_type == 'movie':
            return '%s %d' % (self.search_title, self.year)
        else:
