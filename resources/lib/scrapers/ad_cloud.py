...
class source:
    ...
    def results(self, info):
        try:
            if not enabled_debrids_check('ad'):
            ...
            def _process():
                for item in self.scrape_results:
                    try:
                        ...
                        if filter_title and not item['assigned_content'] and not check_title(title, file_name, self.aliases, self.year, self.season, self.episode):
                            continue
                        display_name = clean_file_name(file_name).replace(
                            'html', ' ').replace('+', ' ').replace('-', ' ')
                        file_dl, size = item['link'], round(
    ...
    def _scrape_cloud(self):
        try:
            try:
                ...
                my_cloud_files = [
                    i for i in my_cloud_files if i['statusCode'] == 4]
            except:
                return self.sources
            threads = []
    ...
    def _scrape_folders(self, folder_info):
        try:
            ...
            links = [i for i in links if i['filename'].lower().endswith(
                tuple(extensions))]
            for item in links:
                normalized = normalize(item['filename'])
