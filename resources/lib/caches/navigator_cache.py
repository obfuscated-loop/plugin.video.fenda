...
prop_dict = {'default': 'fenda_%s_default', 'edited': 'fenda_%s_edited',
             'shortcut_folder': 'fenda_%s_shortcut_folder'}
timeout = 60
...
main_menus = {'RootList': root_list,
              'MovieList': movie_list, 'TVShowList': tvshow_list}
main_menu_items = {'RootList': {'name': 32457, 'iconImage': 'fenda', 'mode': 'navigator.main', 'action': 'RootList'},
...
class NavigatorCache:
    ...
    def get_memory_cache(self, list_name, list_type):
        try:
            return eval(get_property(self._get_list_prop(list_type) % list_name))
        except:
            return None

    def set_memory_cache(self, list_name, list_type, list_contents):
        set_property(self._get_list_prop(list_type) %
                     list_name, repr(list_contents))
