    ...
    try:
        ...
        for item in files:
            if not item in current_dbs:
                try:
                    delete_file(databases_path + item)
                except:
                    pass
    except:
        pass
...
def clean_databases(current_time=None, database_check=True, silent=False):
    ...
    functions_list = ((external_db, command_base % ('results_data', 'expires')),
                      ...
                      (metacache_db, command_base %
                       ('function_cache', 'expires')),
                      (metacache_db, command_base %
                       ('season_metadata', 'expires')),
...
def limit_metacache_database(max_size=60):
    with open_file(metacache_db) as f:
        s = f.size()
...
def clear_cache(cache_type, silent=False):
    ...
    if cache_type == 'meta':
        ...
        for item in ('pm_cloud', 'rd_cloud', 'ad_cloud', 'folders'):
    ...
    for count, cache_type in enumerate(caches, 1):
        try:
            progressDialog.update(line % (ls(32816), cache_type[1]), int(
                float(count) / float(len(caches)) * 100))
