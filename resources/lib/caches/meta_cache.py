...
class MetaCache:
    def get(self, media_type, id_type, media_id, current_time=None):
        ...
        try:
            ...
            if meta is None:
                ...
                cache_data = dbcur.execute(
                    GET_MOVIE_SHOW % id_type, (media_type, media_id)).fetchone()
                if cache_data:
    ...
    def get_season(self, prop_string):
        ...
        try:
            ...
            if meta is None:
                dbcur = self._connect_database()
                cache_data = dbcur.execute(
                    GET_SEASON, (prop_string,)).fetchone()
                if cache_data:
                    meta, expiry = eval(cache_data[0]), cache_data[1]
                    if expiry < current_time:
    ...
    def set(self, media_type, id_type, meta, expiration=30, current_time=None):
        try:
            ...
            dbcur.execute(SET_MOVIE_SHOW, (media_type, string(meta_get('tmdb_id')), meta_get(
        ...
        try:
            ...
            if cache_data:
                if cache_data[2] > current_time:
                    ...
                    dbcur.execute(DELETE_FUNCTION, (prop_string,))
        except:
            pass
        return result
    ...
    def delete_all(self):
        try:
            dbcon = database.connect(
                metacache_db, timeout=240, isolation_level=None)
            dbcur = dbcon.cursor()
