...
class ExternalProvidersCache(object):
    ...
    def get(self, source, media_type, tmdb_id, title, year, season, episode):
        result = None
        try:
            self._execute(SELECT_RESULTS, (source, media_type,
                          tmdb_id, title, year, season, episode))
            cache_data = self.dbcur.fetchone()
            if cache_data:
                if cache_data[1] > self._get_timestamp(self.time):
                    result = eval(cache_data[0])
                else:
                    self.delete(source, media_type, title, year,
                                tmdb_id, season, episode, dbcon)
        except:
            pass
        return result
    ...
    def set(self, source, media_type, tmdb_id, title, year, season, episode, results, expire_time):
        try:
            ...
            self._execute(INSERT_RESULTS, (source, media_type, tmdb_id, title,
                          year, season, episode, repr(results or []), int(expires)))
        except:
            pass

    def delete(self, source, media_type, tmdb_id, title, season, episode):
        try:
            self._execute(DELETE_RESULTS, (source, media_type,
    ...
    def _connect_database(self):
        self.dbcon = database.connect(
            external_db, timeout=timeout, isolation_level=None)
