...
fenda_str, trakt_str, watched_str, unwatched_str, season_str = ls(32036), ls(32037), ls(32642), ls(32643), ls(32537)
...
def mark_tvshow(params):
	...
	else: heading = heading % fenda_str
...
def clear_local_bookmarks():
	try:
		...
		dbcur = set_PRAGMAS(dbcon)
		file_ids = dbcur.execute("SELECT idFile FROM files WHERE strFilename LIKE 'plugin.video.fenda%'").fetchall()
