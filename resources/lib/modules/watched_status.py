...
fenda_str, trakt_str, watched_str, unwatched_str, season_str = ls(
    32036), ls(32037), ls(32642), ls(32643), ls(32537)
indicators_dict = {0: WATCHED_DB, 1: TRAKT_DB}
finished_show_check = ('Ended', 'Canceled')
progress_db_string = 'fenda_hidden_progress_items'


def get_hidden_progress_items(watched_indicators):
    try:
        if watched_indicators == 0:
            return main_cache.get(progress_db_string) or []
        else:
...
def hide_unhide_progress_items(params):
    ...
    if action == 'hide':
        current_items.append(tmdb_id)
    else:
        current_items.remove(tmdb_id)  # unhide
...
def get_recently_watched(media_type, short_list=1, dummy1=None):
    ...
    if media_type == 'movie':
        data = sorted([{'media_id': i[0], 'title': i[1], 'last_played': i[2]} for i in get_watched_info_movie(
            watched_indicators)], key=lambda x: x['last_played'], reverse=True)
    else:
        if short_list:
...
def get_progress_percent(bookmarks, tmdb_id, season='', episode=''):
    try:
        percent = str(
            round(float(detect_bookmark(bookmarks, tmdb_id, season, episode)[0])))
    except:
        percent = None
    return percent
...
def get_bookmarks(watched_indicators, media_type):
    try:
        ...
        result = dbcur.execute(
            "SELECT media_id, resume_point, curr_time, season, episode, resume_id FROM progress WHERE db_type = ?", (media_type,))
...
def erase_bookmark(media_type, tmdb_id, season='', episode='', refresh='false'):
    try:
        ...
        if watched_indicators == 1:
            sleep(1000)
            trakt_progress('clear_progress', media_type,
                           tmdb_id, 0, season, episode, resume_id)
        dbcon = make_database_connection(get_database())
        dbcur = set_PRAGMAS(dbcon)
        dbcur.execute("DELETE FROM progress where db_type = ? and media_id = ? and season = ? and episode = ?",
...
def batch_erase_bookmark(watched_indicators, insert_list, action):
    try:
        ...
        if watched_indicators == 1:
            def _process():
                ...
                for i in insert_list:
                    try:
                        resume_point, curr_time, resume_id = detect_bookmark(
                            bookmarks, tmdb_id, i[2], i[3])
                    except:
                        continue
                    try:
                        sleep(1100)
                        trakt_progress('clear_progress',
                                       i[0], i[1], 0, i[2], i[3], resume_id)
                    except:
                        pass
        ...
        dbcur = set_PRAGMAS(dbcon)
        dbcur.executemany(
            "DELETE FROM progress where db_type = ? and media_id = ? and season = ? and episode = ?", modified_list)
    except:
        pass
...
def set_bookmark(params):
    try:
        media_type, tmdb_id, curr_time, total_time = params.get('media_type'), params.get(
            'tmdb_id'), params.get('curr_time'), params.get('total_time')
        refresh = False if params.get(
            'from_playback', 'false') == 'true' else True
        title, season, episode = params.get(
            'title'), params.get('season'), params.get('episode')
        adjusted_current_time = float(curr_time) - 5
        resume_point = round(adjusted_current_time/float(total_time)*100, 1)
        watched_indicators = watched_indicators_function()
        if watched_indicators == 1:
            if trakt_official_status(media_type) == False:
                return
            else:
                trakt_progress('set_progress', media_type, tmdb_id,
                               resume_point, season, episode, refresh_trakt=True)
        else:
...
def get_watched_info_movie(watched_indicators):
    ...
    try:
        ...
        dbcur = set_PRAGMAS(dbcon)
        dbcur.execute(
            "SELECT media_id, title, last_played FROM watched_status WHERE db_type = ?", ('movie',))
        info = dbcur.fetchall()
    except:
        pass
    return info
...
def get_watched_info_tv(watched_indicators):
    ...
    try:
        ...
        dbcur = set_PRAGMAS(dbcon)
        dbcur.execute(
            "SELECT media_id, season, episode, title, last_played FROM watched_status WHERE db_type = ?", ('episode',))
        info = dbcur.fetchall()
    except:
        pass
    return info
...
def get_in_progress_movies(dummy_arg, page_no):
    ...
    dbcur = set_PRAGMAS(dbcon)
    dbcur.execute(
        "SELECT media_id, title, last_played FROM progress WHERE db_type = ?", ('movie',))
    data = dbcur.fetchall()
    data = [{'media_id': i[0], 'title': i[1], 'last_played': i[2]}
            for i in data if not i[0] == '']
    if lists_sort_order('progress') == 0:
        data = sort_for_article(data, 'title', ignore_articles())
    else:
        data = sorted(data, key=lambda x: x['last_played'], reverse=True)
    return data
...
def get_in_progress_tvshows(dummy_arg, page_no):
    def _process(item):
        ...
        meta = metadata.tvshow_meta(
            'tmdb_id', tmdb_id, meta_user_info, get_datetime())
        watched_status = get_watched_status_tvshow(
            watched_info, tmdb_id, meta.get('total_aired_eps'))
        status = meta.get('status', '')
        if watched_status[0] == 0:
            data_append(item)
        elif include_watched_airing and status not in finished_show_check:
            data_append(item)
    data, duplicates = [], set()
    ...
    prelim_data = [{'media_id': i[0], 'title': i[3], 'last_played': i[4]}
                   for i in watched_info if not (i[0] in duplicates or duplicates_add(i[0]))]
    hidden_items = get_hidden_progress_items(watched_indicators)
    prelim_data = [i for i in prelim_data if not int(
        i['media_id']) in hidden_items]
...
def get_in_progress_episodes():
    ...
    dbcur = set_PRAGMAS(dbcon)
    dbcur.execute(
        'SELECT media_id, season, episode, resume_point, last_played, title FROM progress WHERE db_type = ?', ('episode',))
    data = dbcur.fetchall()
    if lists_sort_order('progress') == 0:
        data = sort_for_article(data, 5, ignore_articles())
    else:
        data.sort(key=lambda k: k[4], reverse=True)
    episode_list = [{'media_ids': {'tmdb': i[0]}, 'season': int(
        i[1]), 'episode': int(i[2]), 'resume_point': float(i[3])} for i in data]
    return episode_list
...
def get_watched_items(media_type, page_no):
    ...
    if media_type == 'tvshow':
        def _process(item):
            ...
            meta = metadata.tvshow_meta(
                'tmdb_id', tmdb_id, meta_user_info, get_datetime())
            playcount = get_watched_status_tvshow(
                watched_info, tmdb_id, meta.get('total_aired_eps'))[0]
            status = meta.get('status', '')
            if playcount == 1:
                if exclude_still_airing and status not in finished_show_check:
                    return
                data_append(item)
        watched_info = get_watched_info_tv(watched_indicators)
        meta_user_info = metadata_user_info()
...
def get_watched_status_tvshow(watched_info, tmdb_id, aired_eps):
    ...
    try:
        watched = min(
            len([i for i in watched_info if i[0] == tmdb_id]), aired_eps)
...
def get_watched_status_season(watched_info, tmdb_id, season, aired_eps):
    playcount, overlay, watched, unwatched = 0, 4, 0, aired_eps
    try:
        watched = min(len([i for i in watched_info if i[0] ==
                      tmdb_id and i[1] == season]), aired_eps)
        unwatched = aired_eps - watched
        if watched >= aired_eps and not aired_eps == 0:
            playcount, overlay = 1, 5
    except:
...
def get_watched_status_episode(watched_info, tmdb_id, season='', episode=''):
    try:
        watched = [i for i in watched_info if i[0] ==
                   tmdb_id and (i[1], i[2]) == (season, episode)]
        if watched:
            return 1, 5
        else:
            return 0, 4
...
def mark_movie(params):
    ...
    refresh, from_playback = params.get('refresh', 'true') == 'true', params.get(
        'from_playback', 'false') == 'true'
    if from_playback:
        refresh = False
    tmdb_id, title, year = params.get(
        'tmdb_id'), params.get('title'), params.get('year')
    watched_indicators = watched_indicators_function()
    if watched_indicators == 1:
        if from_playback == 'true' and trakt_official_status(media_type) == False:
            sleep(1000)
        elif not trakt_watched_status_mark(action, 'movies', tmdb_id):
            return notification(32574)
        clear_trakt_collection_watchlist_data('watchlist', media_type)
    watched_status_mark(watched_indicators, media_type,
                        tmdb_id, action, title=title)
    refresh_container(refresh)


def mark_tvshow(params):
    title, year, action, tmdb_id, icon = params.get('title', ''), params.get(
        'year', ''), params.get('action'), params.get('tmdb_id'), params.get('icon', None)
    try:
    ...
    if watched_indicators == 1:
        if not trakt_watched_status_mark(action, 'shows', tmdb_id, tvdb_id):
            return notification(32574)
        clear_trakt_collection_watchlist_data('watchlist', 'tvshow')
        heading = heading % trakt_str
    else:
        heading = heading % fenda_str
    ...
    for count, item in enumerate(season_data, 1):
        ...
        for ep in ep_data:
            ...
            display = '%s - S%.2dE%.2d' % (title,
                                           int(season_number), int(ep_number))
            progressDialog.update(display, int(float(count)/float(total)*100))
            episode_date, premiered = adjust_premiered_date(
                ep['premiered'], date_offset())
            if episode_date and current_date < episode_date:
                continue
            insert_append(make_batch_insert(action, 'episode', tmdb_id,
                          season_number, ep_number, last_played, title))
...
def mark_season(params):
    ...
    insert_list = []
    insert_append = insert_list.append
    action, title, year, tmdb_id, icon = params.get('action'), params.get(
        'title'), params.get('year'), params.get('tmdb_id'), params.get('icon')
    try:
        tvdb_id = int(params.get('tvdb_id', '0'))
    except:
        tvdb_id = 0
    ...
    heading = watched_str if action == 'mark_as_watched' else unwatched_str
    if watched_indicators == 1:
        if not trakt_watched_status_mark(action, 'season', tmdb_id, tvdb_id, season):
            return notification(32574)
        clear_trakt_collection_watchlist_data('watchlist', 'tvshow')
        heading = heading % trakt_str
    else:
    ...
    for count, item in enumerate(ep_data, 1):
        ...
        display = '%s - %s %s - E%.2d' % (title,
                                          season_str, season_number, ep_number)
        episode_date, premiered = adjust_premiered_date(
            item['premiered'], date_offset())
        if episode_date and current_date < episode_date:
            continue
        progressDialog.update(display, int(
            float(count)/float(len(ep_data))*100))
        insert_append(make_batch_insert(action, 'episode', tmdb_id,
                      season_number, ep_number, last_played, title))
    batch_watched_status_mark(watched_indicators, insert_list, action)
    progressDialog.close()
...
def mark_episode(params):
    ...
    try:
        ...
        tvdb_id = 0
    season, episode, title = int(params.get('season')), int(
        params.get('episode')), params.get('title')
    watched_indicators = watched_indicators_function()
    if season == 0:
        notification(32490)
        return
    if watched_indicators == 1:
        if from_playback == 'true' and trakt_official_status(media_type) == False:
            sleep(1000)
        elif not trakt_watched_status_mark(action, media_type, tmdb_id, tvdb_id, season, episode):
            return notification(32574)
        clear_trakt_collection_watchlist_data('watchlist', 'tvshow')
    watched_status_mark(watched_indicators, media_type,
                        tmdb_id, action, season, episode, title)
    refresh_container(refresh)
...
def watched_status_mark(watched_indicators, media_type='', tmdb_id='', action='', season='', episode='', title=''):
    try:
        ...
        dbcur = set_PRAGMAS(dbcon)
        if action == 'mark_as_watched':
            dbcur.execute("INSERT OR REPLACE INTO watched_status VALUES (?, ?, ?, ?, ?, ?)",
                          (media_type, tmdb_id, season, episode, last_played, title))
        elif action == 'mark_as_unwatched':
            dbcur.execute("DELETE FROM watched_status WHERE (db_type = ? and media_id = ? and season = ? and episode = ?)",
                          (media_type, tmdb_id, season, episode))
        erase_bookmark(media_type, tmdb_id, season, episode)
    except:
        notification(32574)
...
def batch_watched_status_mark(watched_indicators, insert_list, action):
    try:
        ...
        if action == 'mark_as_watched':
            dbcur.executemany(
                "INSERT OR IGNORE INTO watched_status VALUES (?, ?, ?, ?, ?, ?)", insert_list)
        elif action == 'mark_as_unwatched':
            dbcur.executemany(
                "DELETE FROM watched_status WHERE (db_type = ? and media_id = ? and season = ? and episode = ?)", insert_list)
...
def make_batch_insert(action, media_type, tmdb_id, season, episode, last_played, title):
    if action == 'mark_as_watched':
        return (media_type, tmdb_id, season, episode, last_played, title)
    else:
