...
def imdb_sort_list():
	# From Exodus code
	sort, sort_order = int(get_setting('fenda.imdb_lists.sort_type', '0')), int(get_setting('fenda.imdb_lists.sort_direction', '0'))
...
def download_github_zip(repo, file, destination):
	...
	try:
		url = 'https://github.com/obfuscated-loop/%s/raw/main/%s.zip' % (repo, file)
