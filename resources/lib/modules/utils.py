...
days_translate = {'Monday': 32971, 'Tuesday': 32972, 'Wednesday': 32973,
                  'Thursday': 32974, 'Friday': 32975, 'Saturday': 32976, 'Sunday': 32977}
...
def jsondate_to_datetime(jsondate_object, resformat, remove_time=False):
    if remove_time:
        datetime_object = datetime_workaround(
            jsondate_object, resformat).date()
    else:
...
def make_day(today, date, date_format, use_words=True):
    ...
    if use_words:
        if day_diff == -1:
            ...
            day = ls(days_translate[date.strftime('%A')])
    return day
...
def date_difference(current_date, compare_date, difference_tolerance, allow_postive_difference=False):
    try:
        ...
        if not allow_postive_difference and difference > 0:
            return False
        else:
...
def calculate_age(born, str_format, died=None):
    ...
    if not died:
        ...
        today = datetime_workaround(died, str_format)
...
def remove_accents(obj):
    try:
        ...
        obj = ''.join(c for c in unicodedata.normalize(
            'NFD', obj) if unicodedata.category(c) != 'Mn')
    except:
        pass
    return obj
...
def regex_from_to(text, from_string, to_string, excluding=True):
    if excluding:
        r = re.search(r"(?i)" + from_string +
                      r"([\S\s]+?)" + to_string, text).group(1)
    else:
        r = re.search(
            r"(?i)(" + from_string + r"[\S\s]+?" + to_string + ")", text).group(1)
    return r
...
def sort_for_article(_list, _key, ignore_articles):
    if not ignore_articles:
        _list.sort(key=lambda k: k[_key])
    else:
        _list.sort(key=lambda k: re.sub(
            r'(^the |^a |^an )', '', k[_key].lower()))
    return _list
...
def sort_list(sort_key, sort_direction, list_data, ignore_articles):
    try:
        ...
        if sort_key == 'votes':
            return sorted(list_data, key=lambda x: x[x['type']].get('votes', 0), reverse=reverse)
        if sort_key == 'random':
            return sorted(list_data, key=lambda k: random())
        return list_data
    except:
        return list_data
...
def imdb_sort_list():
    # From Exodus code
    sort, sort_order = int(get_setting('fenda.imdb_lists.sort_type', '0')), int(
        get_setting('fenda.imdb_lists.sort_direction', '0'))
    if sort == 0:
        imdb_sort = 'list_order'  # Default
    elif sort == 1:
        imdb_sort = 'alpha'  # Alphabetical
    elif sort == 2:
        imdb_sort = 'user_rating'  # IMDb Rating
    elif sort == 3:
        imdb_sort = 'moviemeter'  # Popularity
    elif sort == 4:
        imdb_sort = 'your_rating'  # Your Rating
    elif sort == 5:
        imdb_sort = 'num_votes'  # Number Of Ratings
    elif sort == 6:
        imdb_sort = 'release_date'  # Release Date
    elif sort == 7:
        imdb_sort = 'runtime'  # Runtime
    elif sort == 8:
        imdb_sort = 'date_added'  # Date Added
