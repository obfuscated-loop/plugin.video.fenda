...
addon_fanart = addon_object.getSetting('addon_fanart') or path_join(addon_path, 'resources', 'media', 'fenda_fanart.png')
...
def end_directory(handle, cacheToDisc=None):
	if cacheToDisc == None: cacheToDisc = get_setting('fenda.kodi_menu_cache') == 'true'
...
def volume_checker():
	# 0% == -60db, 100% == 0db
	try:
		if get_setting('fenda.playback.volumecheck_enabled', 'false') == 'false' or get_visibility('Player.Muted'): return
		from modules.utils import string_alphanum_to_num
		max_volume = min(int(get_setting('fenda.playback.volumecheck_percent', '50')), 100)
...
def make_settings_props():
	...
	try:
		...
		for item in all_settings(): set_property('fenda.%s' % item[0], item[1])
