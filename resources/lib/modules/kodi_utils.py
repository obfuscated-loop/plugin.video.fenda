...
player, xbmc_player, numeric_input, xbmc_monitor, translatePath, xbmc_actor = xbmc.Player(
), xbmc.Player, 1, xbmc.Monitor, xbmcvfs.translatePath, xbmc.Actor
...
monitor, window, dialog, progressDialog, progressDialogBG = xbmc_monitor(), Window(
    10000), xbmcgui.Dialog(), xbmcgui.DialogProgress(), xbmcgui.DialogProgressBG()
...
window_xml_selection_actions, window_xml_closing_actions, window_xml_context_actions = (
    7, 100), (9, 10, 13, 92), (101, 108, 117)
...
addon_icon = path_join(addon_path, 'resources', 'media', 'fenda_icon.png')
addon_fanart = addon_object.getSetting('addon_fanart') or path_join(
    addon_path, 'resources', 'media', 'fenda_fanart.png')
databases_path = path_join(userdata_path, 'databases/')
colorpalette_path = path_join(userdata_path, 'color_palette2/')
colorpalette_zip_path = path_join(
    addon_path, 'resources', 'media', 'color_palette2.zip')
custom_xml_path = path_join(addon_path, 'resources', 'skins',
                            'Custom', '%s', 'resources', 'skins', 'Default', '1080i', '%s')
...
int_window_prop, pause_services_prop, suppress_sett_dict_prop, highlight_prop = 'fenda.internal_results.%s', 'fenda.pause_services', 'fenda.suppress_settings_dict', 'fenda.main_highlight'
pause_settings_prop, current_skin_prop, use_skin_fonts_prop, custom_info_prop = 'fenda.pause_settings', 'fenda.current_skin', 'fenda.use_skin_fonts', 'fenda.custom_info_dialog'
custom_context_main_menu_prop, custom_context_prop, sett_addoninfo_active_prop = 'fenda.custom_context_main_menu', 'fenda.custom_context_menu', 'fenda.setting_addoninfo_active'
...
movie_extras_buttons_defaults = [('extras.movie.button10', 'movies_play'), ('extras.movie.button11', 'show_trailers'), ('extras.movie.button12', 'show_trakt_manager'),
                                 ('extras.movie.button13', 'show_images'), ('extras.movie.button14',
                                                                            'show_extrainfo'), ('extras.movie.button15', 'show_genres'),
...
tvshow_extras_buttons_defaults = [('extras.tvshow.button10', 'tvshow_browse'), ('extras.tvshow.button11', 'show_trailers'), ('extras.tvshow.button12', 'show_trakt_manager'),
                                  ('extras.tvshow.button13', 'show_images'), ('extras.tvshow.button14',
                                                                              'show_extrainfo'), ('extras.tvshow.button15', 'show_genres'),
...
tmdb_dict_removals = ('adult', 'backdrop_path', 'genre_ids', 'original_language', 'original_title',
                      'overview', 'popularity', 'vote_count', 'video', 'origin_country', 'original_name')
trakt_dict_removals = ('collected_at', 'released')
view_ids = ('view.main', 'view.movies', 'view.tvshows', 'view.seasons',
...
default_highlights = (('hoster.identify', 'FF0166FF'), ('torrent.identify', 'FFFF00FE'), ('provider.rd_colour', 'FF3C9900'), ('provider.pm_colour', 'FFFF3300'),
                      ('provider.ad_colour', 'FFE6B800'), ('provider.furk_colour',
                                                           'FFE6002E'), ('provider.easynews_colour', 'FF00B3B2'),
                      ('provider.debrid_cloud_colour', 'FF7A01CC'), ('provider.folders_colour',
                                                                     'FFB36B00'), ('scraper_4k_highlight', 'FFFF00FE'),
                      ('scraper_1080p_highlight', 'FFE6B800'), ('scraper_720p_highlight', 'FF3C9900'), (
                          'scraper_SD_highlight', 'FF0166FF'), ('scraper_single_highlight', 'FF008EB2'),
...
def end_directory(handle, cacheToDisc=None):
    if cacheToDisc == None:
        cacheToDisc = get_setting('fenda.kodi_menu_cache') == 'true'
...
def addon(addon_id='plugin.video.fenda'):
...
def external():
    return 'fenda' not in get_infolabel('Container.PluginName')
...
def disable_enable_addon(addon_name='plugin.video.fenda'):
    try:
        execute_JSON(json.dumps({'jsonrpc': '2.0', 'id': 1, 'method': 'Addons.SetAddonEnabled', 'params': {
                     'addonid': addon_name, 'enabled': False}}))
        execute_JSON(json.dumps({'jsonrpc': '2.0', 'id': 1, 'method': 'Addons.SetAddonEnabled', 'params': {
                     'addonid': addon_name, 'enabled': True}}))
    except:
        pass
...
def jsonrpc_get_directory(directory, properties=['title', 'file', 'thumbnail']):
    command = {'jsonrpc': '2.0', 'id': 1, 'method': 'Files.GetDirectory', 'params': {
        'directory': directory, 'media': 'files', 'properties': properties}}
    try:
        results = [i for i in get_jsonrpc(command).get('files') if i['file'].startswith(
            'plugin://') and i['filetype'] == 'directory']
    except:
        results = None
    return results
...
def jsonrpc_get_addons(_type, properties=['thumbnail', 'name']):
    command = {'jsonrpc': '2.0', 'method': 'Addons.GetAddons',
               'params': {'type': _type, 'properties': properties}, 'id': '1'}
...
def progress_dialog(heading='', icon=addon_icon):
    ...
    progress_dialog = create_window(
        ('windows.progress', 'Progress'), 'progress.xml', heading=heading, icon=icon)
...
def confirm_dialog(heading='', text=32580, ok_label=32839, cancel_label=32840, default_control=11):
    ...
    if isinstance(text, int):
        text = local_string(text)
    if isinstance(ok_label, int):
        ok_label = local_string(ok_label)
    if isinstance(cancel_label, int):
        cancel_label = local_string(cancel_label)
    kwargs = {'heading': heading, 'text': text, 'ok_label': ok_label,
              'cancel_label': cancel_label, 'default_control': default_control}
...
def show_text(heading, text=None, file=None, font_size='small', kodi_log=False):
    ...
    if kodi_log:
        confirm = confirm_dialog(
            text=32855, ok_label=32824, cancel_label=32828)
        if confirm == None:
            return
        if confirm:
            text = [i for i in text if any(
                x in i.lower() for x in ('exception', 'error', '[test]'))]
    text = ''.join(text)
...
def choose_view(view_type, content):
    ...
    listitem.setArt({'icon': settings_icon, 'poster': settings_icon,
                    'thumb': settings_icon, 'fanart': addon_fanart, 'banner': settings_icon})
    info_tag = listitem.getVideoInfoTag()
    info_tag.setPlot(' ')
...
def volume_checker():
    # 0% == -60db, 100% == 0db
    try:
        if get_setting('fenda.playback.volumecheck_enabled', 'false') == 'false' or get_visibility('Player.Muted'):
            return
        from modules.utils import string_alphanum_to_num
        max_volume = min(
            int(get_setting('fenda.playback.volumecheck_percent', '50')), 100)
...
def toggle_language_invoker():
    ...
    current_addon_setting = get_setting('fenda.reuse_language_invoker', 'true')
    new_value = invoker_switch_dict[current_addon_setting]
    if not confirm_dialog(text=local_string(33018) % (current_addon_setting.upper(), new_value.upper())):
        return
    if new_value == 'true' and not confirm_dialog(text=33019):
        return
    root = mdParse(addon_xml)
    root.getElementsByTagName("reuselanguageinvoker")[
        0].firstChild.data = new_value
...
def upload_logfile(params):
    ...
    kwargs = {'items': json.dumps(list_items), 'heading': local_string(
        33147), 'narrow_window': 'true'}
    log_file = select_dialog(log_files, **kwargs)
    if log_file == None:
        return
    log_name, log_file = log_file
    if not confirm_dialog(heading=log_name, text=32580):
        return
    show_busy_dialog()
...
def open_settings(query, addon='plugin.video.fenda'):
    hide_busy_dialog()
    if query:
        try:
...
def get_setting(setting_id, fallback=''):
    return get_property(setting_id) or addon_object.getSetting(setting_id.replace('fenda.', '')) or fallback
...
def make_settings_props():
    if get_property(sett_addoninfo_active_prop) == 'true':
        return
...
def all_settings():
    try:
        settings = [(item.getAttribute('id'), item.firstChild.data if item.firstChild and item.firstChild.data is not None else '')
