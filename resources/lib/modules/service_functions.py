...
fenda_str, window_top_str, listitem_property_str = ls(32036).upper(), 'Window.IsTopMost(%s)', 'ListItem.Property(%s)'
...
class DatabaseMaintenance:
	def run(self):
		...
		else: return logger(fenda_str, 'Database Maintenance Service Finished - Not Run')
...
class ReuseLanguageInvokerCheck:
	def run(self):
		...
		addon_xml = translate_path('special://home/addons/plugin.video.fenda/addon.xml')
		current_addon_setting = get_setting('fenda.reuse_language_invoker', None)
		if current_addon_setting is None: return logger(fenda_str, 'ReuseLanguageInvokerCheck Service Error. No current setting detected. Finished')
		...
		while not monitor.abortRequested():
			try:
				...
				if status in ('success', 'no account'):
					logger(fenda_str, trakt_service_string % ('Success', success_line_dict[status]))
					if trakt_sync_refresh_widgets():
						kodi_refresh()
						logger(fenda_str, trakt_service_string % ('Widgets Refresh', 'Setting Activated. Widget Refresh Performed'))
					else: logger(fenda_str, trakt_service_string % ('Widgets Refresh', 'Setting Disabled. Skipping Widget Refresh'))
				elif status == 'failed': logger(fenda_str, trakt_service_string % ('Failed. Error from Trakt', next_update_string))
				else: logger(fenda_str, trakt_service_string % ('Success. No Changes Needed', next_update_string))# 'not needed'
			except Exception as e: logger(fenda_str, trakt_service_string % ('Failed', 'The following Error Occured: %s' % str(e)))
		...
		while not abort_requested():
			...
			while not any([context_visible, info_visible]) and not abort_requested():
				...
				if not external() or get_infolabel(listitem_property_str % 'fenda.external') == 'true':
					run_custom = True
					custom_context_params = get_infolabel(listitem_property_str % 'fenda.options_params')
					custom_main_context_params = get_infolabel(listitem_property_str % 'fenda.context_main_menu_params')
					custom_info_params = get_infolabel(listitem_property_str % 'fenda.extras_params')
...
class PremiumExpiryCheck:
	def run(self):
		...
		try:
			self.run_check()
			if self.make_message(): ok_dialog('Fenda - Premium Accounts Expiring...', '\n'.join(self.message))
			self.expired_premium = [i for i in self.days_remaining if i[1] == 0]
			if self.expired_premium and confirm_dialog(heading='Fenda - Premium Accounts Expired', text='Disable Expired Accounts?', ok_label=32839, cancel_label=32840): self.revoke()
		except: pass
	...
	def run(self, sender, method, data):
		if sender == 'xbmc':
			if method in ('GUI.OnScreensaverActivated', 'System.OnSleep'):
				...
				logger('OnNotificationActions', 'PAUSING Fenda Services Due to Device Sleep')
			elif method in ('GUI.OnScreensaverDeactivated', 'System.OnWake'):
				clear_property(pause_services_prop)
				logger('OnNotificationActions', 'UNPAUSING Fenda Services Due to Device Awake')
