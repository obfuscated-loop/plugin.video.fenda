...
trakt_sync_interval, trakt_sync_refresh_widgets, auto_start_fenda = settings.trakt_sync_interval, settings.trakt_sync_refresh_widgets, settings.auto_start_fenda
...
fenda_str, window_top_str, listitem_property_str = ls(
    32036).upper(), 'Window.IsTopMost(%s)', 'ListItem.Property(%s)'
...
class InitializeDatabases:
    def run(self):
        logger(fenda_str, 'InitializeDatabases Service Starting')
        check_databases()
        return logger(fenda_str, 'InitializeDatabases Service Finished')


class CheckSettings:
    def run(self):
        logger(fenda_str, 'CheckSettingsFile Service Starting')
        if not path_exists(userdata_path):
            make_directories(userdata_path)
        addon_object.setSetting('dummy_setting', 'foo')
        return logger(fenda_str, 'CheckSettingsFile Service Finished')


class DatabaseMaintenance:
    def run(self):
        logger(fenda_str, 'Database Maintenance Service Starting')
        time = datetime.datetime.now()
        current_time = self._get_timestamp(time)
        due_clean = int(get_setting('fenda.database.maintenance.due', '0'))
        if due_clean == 0:
            next_clean = str(int(self._get_timestamp(
                time + datetime.timedelta(days=3))))
            set_setting('database.maintenance.due', next_clean)
            return logger(fenda_str, 'Database Maintenance Service First Run - Skipping')
        if current_time >= due_clean:
            clean_databases(current_time, database_check=False, silent=True)
            next_clean = str(int(self._get_timestamp(
                time + datetime.timedelta(days=3))))
            set_setting('database.maintenance.due', next_clean)
            return logger(fenda_str, 'Database Maintenance Service Finished')
        else:
            return logger(fenda_str, 'Database Maintenance Service Finished - Not Run')
...
class FirstRunActions:
    def run(self):
        logger(fenda_str, 'CheckUpdateActions Service Starting')
        addon_version, settings_version = self.remove_alpha(addon_object.getAddonInfo(
            'version')), self.remove_alpha(addon_object.getSetting('version_number'))
        if addon_version != settings_version:
            logger(fenda_str, 'CheckUpdateActions Running Update Actions....')
            addon_object.setSetting('version_number', addon_version)
            self.update_action(addon_version)
        if get_setting('first_use', 'false') == 'true':
            lines = ('By default, Fenda has [B]Local Folders[/B], [B]Debrid Cloud[/B], [B]Furk[/B] and [B]Easynews[/B] scrapers.',
                     'These scrapers are designed for use on media you have purchased and organized through these services.',
                     'Fenda is [B]unable[/B] to search online for sources outside of these services.',
                     'If you wish to use an external scraper package within Fenda, you must select a module at[CR][B]Fenda Settings->Providers->Choose External Scrapers Module.[/B]',
    ...
    def remove_alpha(self, string):
        try:
            result = ''.join(c for c in string if (c.isdigit() or c == '.'))
        except:
            result = ''
        return result
...
class ReuseLanguageInvokerCheck:
    def run(self):
        logger(fenda_str, 'ReuseLanguageInvokerCheck Service Starting')
        addon_xml = translate_path(
            'special://home/addons/plugin.video.fenda/addon.xml')
        current_addon_setting = get_setting(
            'fenda.reuse_language_invoker', None)
        if current_addon_setting is None:
            return logger(fenda_str, 'ReuseLanguageInvokerCheck Service Error. No current setting detected. Finished')
        root = mdParse(addon_xml)
        invoker_instance = root.getElementsByTagName(
            'reuselanguageinvoker')[0].firstChild
...
class TraktMonitor:
    def run(self):
        ...
        wait_time = 30 * 60
        success_line_dict = {'success': 'Trakt Update Performed',
                             'no account': '(Unauthorized) Trakt Update Performed'}
        while not monitor.abortRequested():
            try:
                while is_playing() or get_property(pause_services_prop) == 'true':
                    wait_for_abort(10)
                ...
                if status in ('success', 'no account'):
                    logger(fenda_str, trakt_service_string %
                           ('Success', success_line_dict[status]))
                    if trakt_sync_refresh_widgets():
                        kodi_refresh()
                        logger(fenda_str, trakt_service_string % (
                            'Widgets Refresh', 'Setting Activated. Widget Refresh Performed'))
                    else:
                        logger(fenda_str, trakt_service_string % (
                            'Widgets Refresh', 'Setting Disabled. Skipping Widget Refresh'))
                elif status == 'failed':
                    logger(fenda_str, trakt_service_string %
        ...
        try:
            ...
            pass
        return logger(fenda_str, 'TraktMonitor Service Finished')
    ...
    def run(self):
        ...
        while not abort_requested():
            ...
            while not any([context_visible, info_visible]) and not abort_requested():
                ...
                custom_main_context = get_property(
                    custom_context_main_menu_prop) == 'true'
                custom_info = get_property(custom_info_prop) == 'true'
                if not any([custom_context, custom_main_context, custom_info]):
                    self.wait_for_abort(5)
                    continue
                ...
                if get_property(pause_services_prop) == 'true' or is_playing():
                    self.wait_for_abort(2)
                    continue
                if not external() or get_infolabel(listitem_property_str % 'fenda.external') == 'true':
                    run_custom = True
                    custom_context_params = get_infolabel(
                        listitem_property_str % 'fenda.options_params')
                    custom_main_context_params = get_infolabel(
                        listitem_property_str % 'fenda.context_main_menu_params')
                    custom_info_params = get_infolabel(
                        listitem_property_str % 'fenda.extras_params')
                    self.wait_for_abort(0.25)
                else:
                    run_custom = False
                    self.wait_for_abort(1)
                    continue
                context_visible, info_visible = get_visibility(
                    window_top_str % contextmenu_str), get_visibility(window_top_str % movieinformation_str)
            try:
                if run_custom and any([custom_context_params, custom_main_context_params, custom_info_params]):
                    if info_visible:
                        if custom_info and custom_info_params:
                            self.run_custom_action(
                                custom_info_params, movieinformation_str)
                    else:
                        if all([custom_context, custom_context_params != '']) or all([custom_main_context, custom_main_context_params != '']):
                            self.run_custom_action(
                                custom_context_params or custom_main_context_params, contextmenu_str)
                else:
                    self.wait_for_abort(1)
            except:
                self.wait_for_abort(2)
        ...
        try:
            ...
            pass
        return logger(fenda_str, 'CustomActions Service Finished')
...
class CustomFonts:
    def run(self):
        ...
        try:
            ...
            pass
        return logger(fenda_str, 'CustomFonts Service Finished')


class ClearSubs:
    def run(self):
        logger(fenda_str, 'Clear Subtitles Service Starting')
        sub_formats = ('.srt', '.ssa', '.smi', '.sub', '.idx', '.nfo')
        subtitle_path = 'special://temp/%s'
        files = list_dirs(translate_path('special://temp/'))[1]
        for i in files:
            if i.startswith('FendaSubs_') or i.endswith(sub_formats):
                delete_file(translate_path(subtitle_path % i))
        return logger(fenda_str, 'Clear Subtitles Service Finished')


class AutoRun:
    def run(self):
        logger(fenda_str, 'AutoRun Service Starting')
        if auto_start_fenda():
            run_addon()
        return logger(fenda_str, 'AutoRun Service Finished')
...
class PremiumExpiryCheck:
    def run(self):
        if not check_premium():
            return
        logger(fenda_str, 'Premium Expiry Check Service Starting')
        self.message, self.days_remaining = [], []
        try:
            self.run_check()
            if self.make_message():
                ok_dialog('Fenda - Premium Accounts Expiring...',
                          '\n'.join(self.message))
            self.expired_premium = [
                i for i in self.days_remaining if i[1] == 0]
            if self.expired_premium and confirm_dialog(heading='Fenda - Premium Accounts Expired', text='Disable Expired Accounts?', ok_label=32839, cancel_label=32840):
                self.revoke()
        except:
            pass
        logger(fenda_str, 'Premium Expiry Check Service Finished')
    ...
    def run_check(self):
        ...
        active_functions = [Thread(target=self.process, args=(
            premium_check_function_dict[i], i)) for i in self.active_accounts()]
        [i.start() for i in active_functions]
        [i.join() for i in active_functions]

    def active_accounts(self):
        active_accounts = [i[0] for i in [
            ('Real-Debrid', 'rd'), ('Premiumize.me', 'pm'), ('AllDebrid', 'ad')] if enabled_debrids_check(i[1])]
        if settings.furk_active():
            active_accounts.append('Furk')
        if settings.easynews_active():
            active_accounts.append('Easynews')
    ...
    def process(self, function, name):
        expiry = function()
        if expiry <= 7:
            self.days_remaining.append((name, expiry))
    ...
    def revoke(self):
        ...
        for item in self.expired_premium:
            set_setting(premium_check_setting_dict[item[0]], 'false')
    ...
    def run(self, sender, method, data):
        if sender == 'xbmc':
            if method in ('GUI.OnScreensaverActivated', 'System.OnSleep'):
                ...
                logger('OnNotificationActions',
                       'PAUSING Fenda Services Due to Device Sleep')
            elif method in ('GUI.OnScreensaverDeactivated', 'System.OnWake'):
                clear_property(pause_services_prop)
                logger('OnNotificationActions',
                       'UNPAUSING Fenda Services Due to Device Awake')
