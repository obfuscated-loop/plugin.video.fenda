...
backup_resolutions = {'poster': 'w780', 'fanart': 'w1280',
                      'still': 'original', 'profile': 'h632', 'clearlogo': 'original'}
default_custom_artwork = {'custom_poster': '',
                          'custom_fanart': '', 'custom_clearlogo': ''}
writer_credits = ('Author', 'Writer', 'Screenplay', 'Characters')
alt_titles_check, trailers_check, finished_show_check, empty_value_check = (
    'US', 'GB', 'UK', ''), ('Trailer', 'Teaser'), ('Ended', 'Canceled'), ('', 'None', None)
...
def movie_meta(id_type, media_id, user_info, current_date, current_time=None):
    if id_type == 'trakt_dict':
        if media_id.get('tmdb', None):
            id_type, media_id = 'tmdb_id', media_id['tmdb']
        elif media_id.get('imdb', None):
            id_type, media_id = 'imdb_id', media_id['imdb']
        else:
            id_type, media_id = None, None
    if media_id == None:
        return None
    extra_fanart_enabled, fanart_client_key = user_info[
        'extra_fanart_enabled'], user_info['fanart_client_key']
    language = user_info['language']
    meta, custom_artwork = metacache_get(
        'movie', id_type, media_id, current_time)
    if meta:
        if not meta.get('fanart_added', False) and extra_fanart_enabled:
            fanart_data, all_fanart_images = fanarttv_get(
                'movies', language, meta['tmdb_id'], fanart_client_key)
            ...
            metacache_set('movie', id_type, meta, movie_expiry(
                current_date, meta), current_time)
        return meta
    try:
        tmdb_api = user_info['tmdb_api']
        if id_type in ('tmdb_id', 'imdb_id'):
        ...
        if not data:
            ...
            if id_type == 'tmdb_id':
                meta = {'tmdb_id': media_id, 'imdb_id': 'tt0000000',
                        'tvdb_id': '0000000', 'fanart_added': True, 'blank_entry': True}
            else:
                meta = {'tmdb_id': '0000000', 'imdb_id': media_id,
                        'tvdb_id': '0000000', 'fanart_added': True, 'blank_entry': True}
        ...
        if language != 'en':
            if data['overview'] in empty_value_check:
                ...
                if 'videos' in data:
                    ...
                    if all_trailers:
                        try:
                            trailer_test = [i for i in all_trailers if i['site']
                                            == 'YouTube' and i['type'] in trailers_check][0]
                        except:
                            trailer_test = False
                    else:
                        trailer_test = False
        ...
        if extra_fanart_enabled:
            fanart_data, all_fanart_images = fanarttv_get(
                'movies', language, data['id'], fanart_client_key)
        else:
            fanart_data, all_fanart_images = None, None
        image_resolution = user_info.get(
            'image_resolution', backup_resolutions)
        data_get = data.get
        ...
        rating, votes = data_get(
            'vote_average', ''), data_get('vote_count', '')
        plot, tagline, premiered = data_get('overview', ''), data_get(
            'tagline', ''), data_get('release_date', '')
        ...
        if poster_path:
            default_poster = tmdb_image_url % (
                image_resolution['poster'], poster_path)
        else:
            default_poster = ''
        try:
            extra_poster = images_get('poster')[0]
        except:
            extra_poster = ''
        poster = default_poster or extra_poster
        if default_poster and not default_poster in images['poster']:
            images['poster'].insert(0, default_poster)
        backdrop_path = data_get('backdrop_path', None)
        if backdrop_path:
            default_fanart = tmdb_image_url % (
                image_resolution['fanart'], backdrop_path)
        else:
            default_fanart = ''
        try:
            extra_fanart = images_get('fanart')[0]
        except:
            extra_fanart = ''
        fanart = default_fanart or extra_fanart
        if default_fanart and not default_fanart in images['fanart']:
            images['fanart'].insert(0, default_fanart)
        try:
            clearlogo = images_get('clearlogo')[0]
        except:
            clearlogo = ''
        title, original_title = data_get('title'), data_get('original_title')
        try:
            english_title = [i['data']['title'] for i in data_get(
                'translations')['translations'] if i['iso_639_1'] == 'en'][0]
        except:
            english_title = None
        try:
            year = str(data_get('release_date').split('-')[0])
        except:
            year = ''
        try:
        ...
        if companies:
            if len(companies) == 1:
                studio = [i['name'] for i in companies][0]
            else:
                try:
                    studio = [i['name'] for i in companies if i['logo_path']
                              not in empty_value_check][0] or [i['name'] for i in companies][0]
                except:
                    pass
        ...
        if release_dates:
            try:
                mpaa = [x['certification'] for i in release_dates['results'] for x in i['release_dates']
        ...
        if credits:
            ...
            if all_cast:
                try:
                    cast = [{'name': i['name'], 'role': i['character'], 'thumbnail': tmdb_image_url % (image_resolution['profile'], i['profile_path'])if i['profile_path'] else ''}
                            for i in all_cast]
                except:
                    pass
            ...
            if crew:
                try:
                    writer = ', '.join(
                        [i['name'] for i in crew if i['job'] in writer_credits])
                except:
                    pass
                try:
                    director = [i['name']
                                for i in crew if i['job'] == 'Director'][0]
                except:
                    pass
        ...
        if alternative_titles:
            ...
            alternative_titles = [
                i['title'] for i in alternatives if i['iso_3166_1'] in alt_titles_check]
        else:
            alternative_titles = []
        ...
        if videos:
            ...
            try:
                trailer = [youtube_url % i['key'] for i in all_trailers if i['site']
                           == 'YouTube' and i['type'] in trailers_check][0]
            except:
                pass
        status, homepage = data_get(
            'status', 'N/A'), data_get('homepage', 'N/A')
        belongs_to_collection = data_get('belongs_to_collection')
        if belongs_to_collection:
            ei_collection_name, ei_collection_id = belongs_to_collection[
        ...
        try:
            ...
            ei_revenue = '$0'
        if fanart_data:
            images.update({'poster': images['poster'] + all_fanart_images['poster'], 'fanart': images['fanart'] + all_fanart_images['fanart'],
                           'clearlogo': images['clearlogo'] + all_fanart_images['clearlogo'], 'banner': all_fanart_images['banner'],
                           'clearart': all_fanart_images['clearart'], 'landscape': all_fanart_images['landscape'], 'discart': all_fanart_images['discart'],
                           'keyart': all_fanart_images['keyart']})
        extra_info = {'status': status, 'budget': ei_budget, 'revenue': ei_revenue,
                      'homepage': homepage, 'collection_name': ei_collection_name, 'collection_id': ei_collection_id}
...
def tvshow_meta(id_type, media_id, user_info, current_date, current_time=None):
    ...
    language = user_info['language']
    meta, custom_artwork = metacache_get(
        'tvshow', id_type, media_id, current_time)
    if meta:
        if not meta.get('fanart_added', False) and extra_fanart_enabled:
            fanart_data, all_fanart_images = fanarttv_get(
                'tv', language, meta['tvdb_id'], fanart_client_key)
            meta.update(fanart_data)
            all_images = meta['images']
            meta['images'].update({'poster': all_images['poster'] + all_fanart_images['poster'], 'fanart': all_images['fanart'] + all_fanart_images['fanart'],
                                   'clearlogo': all_images['clearlogo'] + all_fanart_images['clearlogo'], 'banner': all_fanart_images['banner'],
                                   'clearart': all_fanart_images['clearart'], 'landscape': all_fanart_images['landscape']})
            metacache_set('tvshow', id_type, meta, tvshow_expiry(
                current_date, meta), current_time)
        return meta
    try:
        tmdb_api = user_info['tmdb_api']
        ...
        if not data:
            ...
            if id_type == 'tmdb_id':
                ...
                meta = {'tmdb_id': '0000000', 'imdb_id': 'tt0000000',
                        'tvdb_id': media_id, 'fanart_added': True, 'blank_entry': True}
            metacache_set('tvshow', id_type, meta,
                          EXPIRES_1_DAYS, current_time)
            return meta
        if language != 'en':
            if data['overview'] in empty_value_check:
                media_id, id_type = data['id'], 'tmdb_id'
        ...
        if extra_fanart_enabled:
            fanart_data, all_fanart_images = fanarttv_get(
                'tv', language, data['external_ids']['tvdb_id'], fanart_client_key)
        else:
            fanart_data, all_fanart_images = None, None
        image_resolution = user_info.get(
            'image_resolution', backup_resolutions)
        ...
        tmdb_id, imdb_id, tvdb_id = data_get('id', ''), external_ids.get(
            'imdb_id', ''), external_ids.get('tvdb_id', 'None')
        rating, votes = data_get(
            'vote_average', ''), data_get('vote_count', '')
        plot, tagline, premiered = data_get('overview', ''), data_get(
            'tagline', ''), data_get('first_air_date', '')
        season_data, total_seasons = data_get(
            'seasons'), data_get('number_of_seasons')
        extra_images = data_get('images', {})
        if extra_images:
            images['poster'] = [tmdb_image_url % (image_resolution['poster'], (i['file_path'] if i['file_path'].endswith('jpg')
                                                                               else i['file_path'].replace(i['file_path'].split('.')[-1], 'jpg'))) for i in extra_images['posters']]
        ...
        try:
            english_title = [i['data']['name'] for i in data_get(
                'translations')['translations'] if i['iso_639_1'] == 'en'][0]
        except:
            english_title = None
        try:
            year = str(data_get('first_air_date').split('-')[0]) or ''
        except:
            year = ''
        try:
            duration = min(data_get('episode_run_time'))*60
        except:
            duration = 0
        try:
            genre = ', '.join([i['name'] for i in data_get('genres')])
        ...
        if networks:
            if len(networks) == 1:
                studio = [i['name'] for i in networks][0]
            else:
                try:
                    studio = [i['name'] for i in networks if i['logo_path']
                              not in empty_value_check][0] or [i['name'] for i in networks][0]
                except:
                    pass
        production_countries = data_get('production_countries', None)
        if production_countries:
        ...
        if content_ratings:
            try:
                mpaa = [i['rating'] for i in content_ratings['results']
                        if i['iso_3166_1'] == user_info['mpaa_region']][0]
            except:
                pass
        elif release_dates:
            try:
                mpaa = [i['release_dates'][0]['certification']
                        for i in release_dates['results'] if i['iso_3166_1'] == user_info['mpaa_region']][0]
            except:
                pass
        credits = data_get('credits')
        if credits:
            ...
            if all_cast:
                try:
                    cast = [{'name': i['name'], 'role': i['character'],
        ...
        if videos:
            ...
            try:
                ...
                pass
        status, _type, homepage = data_get(
            'status', 'N/A'), data_get('type', 'N/A'), data_get('homepage', 'N/A')
        ...
        if created_by:
            ...
            ei_created_by = 'N/A'
        ei_next_ep, ei_last_ep = data_get(
            'next_episode_to_air', None), data_get('last_episode_to_air', None)
        if ei_last_ep and not status in finished_show_check:
            ...
            total_aired_eps = data_get('number_of_episodes')
        if fanart_data:
            images.update({'poster': images['poster'] + all_fanart_images['poster'], 'fanart': images['fanart'] + all_fanart_images['fanart'],
                           'clearlogo': images['clearlogo'] + all_fanart_images['clearlogo'], 'banner': all_fanart_images['banner'],
                           'clearart': all_fanart_images['clearart'], 'landscape': all_fanart_images['landscape']})
        extra_info = {'status': status, 'type': _type, 'homepage': homepage, 'created_by': ei_created_by,
...
def movieset_meta(media_id, user_info, current_time=None):
    ...
    extra_fanart_enabled, fanart_client_key = user_info[
        'extra_fanart_enabled'], user_info['fanart_client_key']
    meta, custom_artwork = metacache_get(
        'movie_set', id_type, media_id, current_time)
    if meta:
        if not meta.get('fanart_added', False) and extra_fanart_enabled:
            fanart_data, all_fanart_images = fanarttv_get(
                'movies', '', meta['tmdb_id'], fanart_client_key)
            meta.update(fanart_data)
            metacache_set('movie_set', id_type, meta,
                          EXPIRES_30_DAYS, current_time)
        return meta
    try:
        tmdb_api = user_info['tmdb_api']
        ...
        if not data:
            ...
            meta = {'tmdb_id': media_id,
                    'fanart_added': True, 'blank_entry': True}
            metacache_set('movie_set', id_type, meta,
                          EXPIRES_1_DAYS, current_time)
            return meta
        if extra_fanart_enabled:
            fanart_data, all_fanart_images = fanarttv_get(
                'movies', '', data['id'], fanart_client_key)
        else:
            fanart_data, all_fanart_images = None, None
        image_resolution = user_info.get(
            'image_resolution', backup_resolutions)
        data_get = data.get
        title, tmdb_id, plot = data_get('name'), data_get(
            'id'), data_get('overview', '')
        poster_path = data_get('poster_path', None)
        if poster_path:
        ...
        backdrop_path = data_get('backdrop_path', None)
        if backdrop_path:
            fanart = tmdb_image_url % (
                image_resolution['fanart'], backdrop_path)
        else:
            fanart = ''
        parts = data_get('parts')
        meta = {'tmdb_id': tmdb_id, 'title': title, 'plot': plot, 'poster': poster,
                'fanart': fanart, 'parts': parts, 'imdb_id': 'None', 'tvdb_id': 'None'}
        meta.update(fanart_data or default_fanart_movies)
        metacache_set('movie_set', id_type, meta,
                      EXPIRES_30_DAYS, current_time)
    except:
...
def episodes_meta(season, meta, user_info):
    def _process():
        for ep_data in details:
            ...
            title, plot, premiered = ep_data_get('name'), ep_data_get(
                'overview'), ep_data_get('air_date')
            season, episode = ep_data_get(
                'season_number'), ep_data_get('episode_number')
            try:
                duration = ep_data_get('runtime')*60
            except:
                duration = 30*60
            rating, votes, still_path = ep_data_get('vote_average'), ep_data_get(
                'vote_count'), ep_data_get('still_path', None)
            if still_path:
                thumb = tmdb_image_url % (still_resolution, still_path)
            else:
                thumb = None
            guest_stars_list = ep_data_get('guest_stars', None)
            if guest_stars_list:
                try:
                    guest_stars = [{'name': i['name'], 'role': i['character'], 'thumbnail': tmdb_image_url % (profile_resolution, i['profile_path'])
    ...
    if data:
        return data
    try:
        if meta['status'] in finished_show_check or meta['total_seasons'] > int(season):
            expiration = EXPIRES_182_DAYS
        else:
            expiration = EXPIRES_4_DAYS
        image_resolution = user_info.get(
            'image_resolution', backup_resolutions)
        still_resolution, profile_resolution = image_resolution['still'], image_resolution['profile']
        details = season_episodes_details(
            media_id, season, user_info['language'], user_info['tmdb_api'])['episodes']
        data = list(_process())
...
def all_episodes_meta(meta, user_info, include_specials=False):
    ...
    def _get_tmdb_episodes(season):
        try:
            data.extend(episodes_meta(season, meta, user_info))
        except:
            pass
    try:
        data = []
        ...
        threads = [Thread(target=_get_tmdb_episodes, args=(i,))
                   for i in seasons]
...
def english_translation(media_type, media_id, user_info):
    ...
    translations = english_translation(
        media_type, media_id, user_info['tmdb_api'])
    try:
        english = [i['data'][key]
                   for i in translations if i['iso_639_1'] == 'en'][0]
    except:
        english = ''
    return english
...
def movie_expiry(current_date, meta):
    try:
        difference = subtract_dates(current_date, jsondate_to_datetime(
            meta['premiered'], date_format, remove_time=True))
        if difference < 0:
            expiration = abs(difference) + 1
        elif difference <= 14:
            expiration = EXPIRES_7_DAYS
        elif difference <= 30:
            expiration = EXPIRES_14_DAYS
        else:
            expiration = EXPIRES_30_DAYS
    except:
        return EXPIRES_7_DAYS
    return max(expiration, EXPIRES_7_DAYS)


def tvshow_expiry(current_date, meta):
    try:
        if meta['status'] in finished_show_check:
            expiration = EXPIRES_182_DAYS
        else:
            data = subtract_dates(jsondate_to_datetime(
                meta['extra_info']['next_episode_to_air']['air_date'], date_format, remove_time=True), current_date) - EXPIRES_1_DAYS
            if data <= 1:
                expiration = EXPIRES_1_DAYS
            else:
                expiration = data
    except:
        expiration = EXPIRES_4_DAYS
