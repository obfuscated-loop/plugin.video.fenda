...
def runner(params):
    ...
    if action == 'image':
        ...
        if provider == 'furk':
            try:
                t_files = sources.furkPacks(
                    params['file_name'], params['file_id'], download=True)
                pack_choices = [dict(params, **{'pack_files': {'link': item['url_dl'],
                                     'filename': item['name'], 'size': item['size']}}) for item in t_files]
                icon = 'furk'
            except:
                return notification(32692)
        else:
            try:
                debrid_files, debrid_function = sources.debridPacks(
                    provider, params['name'], params['magnet_url'], params['info_hash'], download=True)
                pack_choices = [
                    dict(params, **{'pack_files': item}) for item in debrid_files]
                icon = {'Real-Debrid': 'realdebrid', 'Premiumize.me': 'premiumize',
                        'AllDebrid': 'alldebrid'}[provider]
            except:
                return notification(32692)
...
def download_threads_manager(threads, image):
    ...
    for thread in threads:
        while len([x for x in threads if x.is_alive()]) >= 2:
            sleep(2000)
        thread.start()
            ...
            if english_title:
                ...
                try:
                    ...
                    english_title = english_translation(
                        media_type, meta['tmdb_id'], metadata_user_info())
                    if english_title:
                        title = english_title
                    else:
...
class Downloader:
    ...
    def run(self):
        self.download_prep()
        if not self.action == 'meta.pack':
            show_busy_dialog()
        self.get_url_and_headers()
        if self.url in (None, 'None', ''):
            return self.return_notification(_notification=32692)
        self.get_filename()
        self.get_extension()
        if not self.download_check():
            return self.return_notification(_ok_dialog=32490)
        if not self.confirm_download():
            return self.return_notification(_notification=32736)
    ...
    def download_prep(self):
        if 'meta' in self.params:
            ...
            self.image = self.meta_get('custom_poster') or self.meta_get(
                art_provider[0]) or self.meta_get(art_provider[1]) or poster_empty
    ...
    def download_runner(self):
        self.final_destination = os.path.join(
            self.final_destination, self.final_name + self.extension)
        self.start_download()
    ...
    def get_url_and_headers(self):
        ...
        if url in (None, 'None', ''):
            if self.action == 'meta.single':
                ...
                if source.get('scrape_provider', '') == 'easynews':
                    source['url_dl'] = source['down_url']
    ...
    def get_download_folder(self):
        ...
        for level in levels:
            try:
                make_directory(os.path.abspath(
                    os.path.join(self.down_folder, level)))
            except:
                pass
    ...
    def get_destination_folder(self):
        if self.action == 'image':
            ...
            if self.action == 'meta.single':
                ...
                folder_rootname = self.params_get(
            ...
            if self.media_type == 'episode':
                ...
                self.final_destination = os.path.join(
                    inter, 'Season %02d' % int(self.season))
            else:
                self.final_destination = os.path.join(
                    self.down_folder, folder_rootname)
        else:
            self.final_destination = self.down_folder
    ...
    def get_filename(self):
        if self.final_name:
            ...
            final_name = os.path.splitext(urlparse(name).path)[
                0].split('/')[-1]
            ...
            if clean_title(self.title).lower() in file_name.lower():
                final_name = os.path.splitext(urlparse(name_url).path)[
                    0].split('/')[-1]
            else:
                try:
                    final_name = self.name.translate(
                        None, r'\/:*?"<>|').strip('.')
                except:
                    final_name = os.path.splitext(urlparse(name_url).path)[
                        0].split('/')[-1]
    ...
    def download_check(self):
        ...
        if not self.resp:
            return False
        try:
            self.content = int(self.resp.headers['Content-Length'])
        except:
            self.content = 0
        try:
            self.resumable = 'bytes' in self.resp.headers['Accept-Ranges'].lower(
            )
        except:
    ...
    def return_notification(self, _notification=None, _ok_dialog=None):
        hide_busy_dialog()
        if _notification:
            notification(_notification, 2500)
        elif _ok_dialog:
            ok_dialog(text=_ok_dialog)
        else:
            return
