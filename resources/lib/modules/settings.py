...
paginate_dict = {True: 'fenda.paginate.limit_widgets', False: 'fenda.paginate.limit_addon'}
...
def store_resolved_torrent_to_cloud(debrid_service, pack):
	setting_value = int(get_setting('fenda.store_resolved_torrent.%s' % debrid_service.lower(), '0'))
	return setting_value in (1, 2) if pack else setting_value == 1

def enabled_debrids_check(debrid_service):
	if not get_setting('fenda.%s.enabled' % debrid_service) == 'true': return False
	return authorized_debrid_check(debrid_service)

def authorized_debrid_check(debrid_service):
	if get_setting('fenda.%s.token' % debrid_service) in (None, ''): return False
	return True
...
def autoplay_next_episode():
	if auto_play('episode') and get_setting('fenda.autoplay_next_episode', 'false') == 'true': return True
	else: return False

def autoscrape_next_episode():
	if not auto_play('episode') and get_setting('fenda.autoscrape_next_episode', 'false') == 'true': return True
	else: return False
...
def auto_nextep_settings(play_type):
	...
	window_percentage = 100 - int(get_setting('fenda.%s_next_window_percentage' % play_type, '95'))
	use_chapters = get_setting('fenda.%s_use_chapters' % play_type, 'true') == 'true'
	...
	if play_type == 'autoplay':
		...
		default_action = default_action_dict[get_setting('fenda.autoplay_default_action', '1')] if alert_method == 0 else 'cancel'
...
def furk_active():
	if get_setting('fenda.provider.furk', 'false') == 'true':
		if not get_setting('fenda.furk_api_key'):
			furk_status = False if '' in (get_setting('fenda.furk_login'), get_setting('fenda.furk_password')) else True
...
def easynews_active():
	if get_setting('fenda.provider.easynews', 'false') == 'true':
		easynews_status = False if '' in (get_setting('fenda.easynews_user'), get_setting('fenda.easynews_password')) else True
...
def extras_enabled_menus():
	setting = get_setting('fenda.extras.enabled', '2000,2050,2051,2052,2053,2054,2055,2056,2057,2058,2059,2060,2061,2062,2063')
	if setting in ('', None, 'noop', []): return []
	return [int(i) for i in setting.split(',')]

def check_prescrape_sources(scraper, media_type):
	if scraper in prescrape_scrapers_tuple: return get_setting('fenda.check.%s' % scraper) == 'true'
	if get_setting('fenda.check.%s' % scraper) == 'true' and auto_play(media_type): return True
	else: return False
...
def easynews_language_filter():
	...
	if enabled: filters = get_setting('fenda.easynews.lang_filters').split(', ')
	else: filters = []
	return enabled, filters

def results_sort_order():
	sort_direction = -1 if get_setting('fenda.results.size_sort_direction') == '0' else 1
	return (
...
def active_internal_scrapers():
	...
	active = [i.split('.')[1] for i in settings if get_setting('fenda.%s' % i) == 'true']
	return active
...
def scraping_settings():
	...
	if highlight_type in (0, 1):
		...
		easynews_highlight = get_setting('fenda.provider.easynews_colour', 'FF00B3B2')
		debrid_cloud_highlight = get_setting('fenda.provider.debrid_cloud_colour', 'FF7A01CC')
		folders_highlight = get_setting('fenda.provider.folders_colour', 'FFB36B00')
		if highlight_type == 0:
			hoster_highlight = get_setting('fenda.hoster.identify', 'FF0166FF')
			torrent_highlight = get_setting('fenda.torrent.identify', 'FFFF00FE')
		else:
		...
		highlight_1080P = get_setting('fenda.scraper_1080p_highlight', 'FFE6B800')
		highlight_720P = get_setting('fenda.scraper_720p_highlight', 'FF3C9900')
...
def max_threads():
	if not get_setting('fenda.limit_concurrent_threads', 'false') == 'true': return 60
...
def watched_indicators():
	if get_setting('fenda.trakt.user') == '': return 0
...
def nextep_display_settings():
	include_airdate = get_setting('fenda.nextep.include_airdate', 'false') == 'true'
...
def nextep_content_settings():
	...
	include_unaired = get_setting('fenda.nextep.include_unaired', 'false') == 'true'
	include_unwatched = int(get_setting('fenda.nextep.include_unwatched', '0'))
	sort_airing_today_to_top = get_setting('fenda.nextep.sort_airing_today_to_top', 'false') == 'true'
