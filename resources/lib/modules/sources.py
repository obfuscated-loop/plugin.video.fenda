...
from modules.player import FendaPlayer
...
internal_include_list = ['easynews', 'furk',
                         'pm_cloud', 'rd_cloud', 'ad_cloud']
external_exclude_list = ['furk', 'easynews',
                         'gdrive', 'library', 'filepursuit', 'plexshare']
sd_check = ('SD', 'CAM', 'TELE', 'SYNC')
rd_info, pm_info, ad_info = ('apis.real_debrid_api', 'RealDebridAPI'), (
    'apis.premiumize_api', 'PremiumizeAPI'), ('apis.alldebrid_api', 'AllDebridAPI')
...
quality_ranks = {'4K': 1, '1080p': 2, '720p': 3,
                 'SD': 4, 'SCR': 5, 'CAM': 5, 'TELE': 5}
cloud_scrapers, folder_scrapers = (
    'rd_cloud', 'pm_cloud', 'ad_cloud'), ('folder1', 'folder2', 'folder3', 'folder4', 'folder5')
default_internal_scrapers = (
    'furk', 'easynews', 'rd_cloud', 'pm_cloud', 'ad_cloud', 'folders')
...
class Sources():
    ...
    def playback_prep(self, params=None):
        ...
        self.play_type, self.background, self.prescrape = params_get('play_type', ''), params_get(
            'background', 'false') == 'true', params_get('prescrape', self.prescrape) == 'true'
        self.random, self.random_continual = params_get(
            'random', 'false') == 'true', params_get('random_continual', 'false') == 'true'
        if self.play_type:
            ...
            self.autoplay_nextep, self.autoscrape_nextep = autoplay_next_episode(
        ...
        self.nextep_settings, self.disable_autoplay_next_episode = params_get(
            'nextep_settings', {}), params_get('disable_autoplay_next_episode', 'false') == 'true'
        self.ignore_scrape_filters = params_get(
            'ignore_scrape_filters', 'false') == 'true'
        self.disabled_ext_ignored = params_get(
            'disabled_ext_ignored', self.disabled_ext_ignored) == 'true'
        self.default_ext_only = params_get(
            'default_ext_only', self.default_ext_only) == 'true'
        self.folders_ignore_filters = get_setting(
            'fenda.results.folders_ignore_filters', 'false') == 'true'
        self.media_type, self.tmdb_id, self.ep_name, self.plot = params_get(
            'media_type'), params_get('tmdb_id'), params_get('ep_name'), params_get('plot')
        self.custom_title, self.custom_year = params_get(
            'custom_title', None), params_get('custom_year', None)
        self.custom_season, self.custom_episode = params_get(
            'custom_season', None), params_get('custom_episode', None)
        if 'autoplay' in self.params:
            self.autoplay = self.params.get('autoplay', 'false') == 'true'
        else:
            self.autoplay = auto_play(self.media_type)
        if 'season' in self.params:
            self.season = int(params_get('season'))
        else:
            self.season = ''
        if 'episode' in self.params:
            self.episode = int(params_get('episode'))
        else:
            self.episode = ''
        if 'meta' in self.params:
            self.meta = json.loads(params_get('meta'))
        else:
            self._grab_meta()
        self.active_internal_scrapers = active_internal_scrapers()
        if not 'external' in self.active_internal_scrapers and (self.disabled_ext_ignored or self.default_ext_only):
        ...
        self.include_prerelease_results, self.ignore_results_filter, self.limit_resolve = include_prerelease_results(
        ), ignore_results_filter(), limit_resolve()
        self.filter_hevc, self.filter_hdr = filter_status(
            'hevc'), filter_status('hdr')
        self.filter_dv, self.filter_av1, self.filter_audio = filter_status(
            'dv'), filter_status('av1'), 3
        ...
        self.sort_function, self.display_uncached_torrents, self.quality_filter = results_sort_order(
        ), display_uncached_torrents(), self._quality_filter()
        self.hybrid_allowed, self.filter_size_method = self.filter_hdr in (
            0, 2), int(get_setting('fenda.results.filter_size_method', '0'))
        self.include_unknown_size = get_setting(
            'fenda.results.include.unknown.size', 'false') == 'true'
        self.include_3D_results = get_setting(
            'fenda.include_3d_results', 'true') == 'true'
        self._update_meta()
        self._search_info()
        if self.autoscrape:
    ...
    def collect_results(self):
        ...
        if self.providers:
            for i in self.providers:
                threads_append(Thread(target=self.activate_providers,
                               args=(i[0], i[1], False), name=i[2]))
    ...
    def process_results(self, results):
        ...
        self.uncached_torrents = self.sort_results(
            [i for i in results if 'Uncached' in i.get('cache_provider', '')])
        if not self.display_uncached_torrents:
            results = [i for i in results if not i in self.uncached_torrents]
        if self.ignore_scrape_filters:
            ...
            results = self.sort_results(results)
            results = self._special_filter(
                results, self.audio_filter_key, self.filter_audio)
            results = self._special_filter(
                results, self.hdr_filter_key, self.filter_hdr)
            results = self._special_filter(
                results, self.dolby_vision_filter_key, self.filter_dv)
            results = self._special_filter(
                results, self.av1_filter_key, self.filter_av1)
            results = self._special_filter(
                results, self.hevc_filter_key, self.filter_hevc)
            results = self._sort_first(results)
        return results
    ...
    def filter_results(self, results):
        if self.folders_ignore_filters:
            folder_results = [
                i for i in results if i['scrape_provider'] == 'folders']
        ...
        if not self.include_3D_results:
            results = [i for i in results if not '3D' in i['extraInfo']]
        if self.filter_size_method:
            min_size = string_to_float(get_setting(
                'fenda.results.size.min_manual', '0'), '0') / 1000
            ...
            if self.filter_size_method == 1:
                ...
                max_size = ((0.125 * (0.90 * string_to_float(get_setting(
                    'fenda.results.size.auto', '20'), '20'))) * duration)/1000
            elif self.filter_size_method == 2:
                max_size = string_to_float(get_setting(
                    'fenda.results.size.manual', '10000'), '10000') / 1000
            results = [i for i in results if i['scrape_provider']
                       == 'folders' or min_size <= i['size'] <= max_size]
    ...
    def sort_results(self, results):
        def _add_keys(item):
            ...
            if provider == 'external':
                account_type = item['debrid'].lower()
            else:
                account_type = provider.lower()
            item['provider_rank'] = self._get_provider_rank(account_type)
            item['quality_rank'] = self._get_quality_rank(
                item.get('quality', 'SD'))
        for item in results:
            _add_keys(item)
    ...
    def prepare_internal_scrapers(self):
        ...
        active_internal_scrapers = [
            i for i in self.active_internal_scrapers if not i in self.remove_scrapers]
        if self.prescrape and not self.active_external and all([check_prescrape_sources(i, self.media_type) for i in active_internal_scrapers]):
            return False
        if 'folders' in active_internal_scrapers:
            self.folder_info = [i for i in self.get_folderscraper_info(
            ) if settings.source_folders_directory(self.media_type, i[1])]
            if self.folder_info:
                self.active_folders = True
                self.internal_scraper_names = [
                    i for i in active_internal_scrapers if not i == 'folders'] + [i[0] for i in self.folder_info]
            else:
                self.internal_scraper_names = [
    ...
    def activate_providers(self, module_type, function, prescrape):
        sources = self._get_module(
            module_type, function).results(self.search_info)
        if not sources:
            return
        if prescrape:
    ...
    def activate_debrid_info(self):
        ...
        self.debrid_torrent_enabled = debrid_type_enabled(
            'torrent', self.debrid_enabled)
        self.debrid_hoster_enabled = debrid_valid_hosts(
            debrid_type_enabled('hoster', self.debrid_enabled))

    def activate_external_providers(self):
        if not self.debrid_torrent_enabled and not self.debrid_hoster_enabled:
            return self.disable_external(32854)
        self.ext_folder, self.ext_name = external_scraper_info()
        if not self.ext_folder or not self.ext_name:
            return self.disable_external(33007)
        if not self.import_external_scrapers():
            return self.disable_external(33009)
        exclude_list = []
        if not self.debrid_torrent_enabled:
            exclude_list.extend(self.external_scraper_names('torrents'))
        elif not self.debrid_hoster_enabled:
            exclude_list.extend(self.external_scraper_names('hosters'))
        self.external_providers = self.external_sources()
        if not self.external_providers:
            self.disable_external(33008)
        if exclude_list:
            self.external_providers = [
                i for i in self.external_providers if not i[0] in exclude_list]
    ...
    def external_scraper_names(self, folder):
        if folder == 'torrents':
            return [i for i in self.ext_sources.torrent_providers if not i in external_exclude_list]
        elif folder == 'hosters':
            return [i for i in self.ext_sources.hoster_providers if not i in external_exclude_list]
        else:
            return [i for i in self.ext_sources.all_providers if not i in external_exclude_list]
    ...
    def external_sources(self):
        try:
            if self.disabled_ext_ignored:
                active_sources = self.ext_sources.total_providers['torrents']
            elif self.default_ext_only:
                active_sources = [
            ...
            sourceDict = self.legacy_external_sources()
        return sourceDict

    def legacy_external_sources(self):
        try:
            sourceDict = manual_function_import(self.ext_name, 'sources')(
        ...
        def _scraperDialog():
            ...
            while not self.progress_dialog.iscanceled() and not monitor.abortRequested():
                try:
                    ...
                    percent = int(
                        (current_progress/float(scraper_timeout))*100)
    ...
    def _process_post_results(self):
        ...
        if self.orig_results and not self.background:
            if self.display_uncached_torrents and not self.autoplay:
                return self.play_source(self.uncached_torrents)
            if self.ignore_results_filter == 0:
                return self._no_results()
            if self.ignore_results_filter == 1 or confirm_dialog(heading=self.meta.get('rootname', ''), text=32021):
    ...
    def _search_info(self):
        title, year, season, episode, ep_name = self.get_search_title(
        ), self.get_search_year(), self.get_season(), self.get_episode(), self.get_ep_name()
        aliases = make_alias_dict(self.meta, title)
        expiry_times = get_cache_expiry(
            self.media_type, self.meta, self.season)
    ...
    def get_search_title(self):
        ...
        if custom_title:
            search_title = custom_title
        else:
            if get_setting('fenda.meta_language') == 'en':
                ...
                if english_title:
                    ...
                    try:
                        ...
                        english_title = metadata.english_translation(
                            media_type, self.meta['tmdb_id'], meta_user_info)
                        if english_title:
                            search_title = english_title
                        else:
                            search_title = self.meta['original_title']
                    except:
                        pass
                if not search_title:
                    search_title = self.meta['original_title']
    ...
    def get_search_year(self):
        ...
        if custom_year:
            ...
            if self.active_external and get_setting('fenda.search.enable.yearcheck', 'false') == 'true':
    ...
    def get_season(self):
        season = self.meta.get(
            'custom_season', None) or self.meta.get('season')
        try:
            season = int(season)
        except:
            season = None
    ...
    def get_episode(self):
        episode = self.meta.get(
            'custom_episode', None) or self.meta.get('episode')
        try:
            episode = int(episode)
        except:
            episode = None
    ...
    def _sort_first(self, results):
        try:
            sort_first_scrapers = []
            if 'folders' in self.all_scrapers and sort_to_top('folders'):
                sort_first_scrapers.append('folders')
            sort_first_scrapers.extend(
                [i for i in self.all_scrapers if i in cloud_scrapers and sort_to_top(i)])
            if not sort_first_scrapers:
                return results
            sort_first = [
                i for i in results if i['scrape_provider'] in sort_first_scrapers]
            sort_first.sort(key=lambda k: (self._sort_folder_to_top(
                k['scrape_provider']), k['quality_rank']))
    ...
    def _sort_uncached_torrents(self, results):
        uncached = [i for i in results if 'Uncached' in i.get(
            'cache_provider', '')]
    ...
    def _special_filter(self, results, key, enable_setting):
        if key == self.hevc_filter_key and enable_setting in (0, 2):
            hevc_max_quality = self._get_quality_rank(get_setting('fenda.filter_hevc.%s' % (
                'max_autoplay_quality' if self.autoplay else 'max_quality'), '4K'))
        ...
        if enable_setting == 1:
            if key == self.dolby_vision_filter_key and self.hybrid_allowed:
                results = [i for i in results if all(x in i['extraInfo'] for x in (
                    key, self.hdr_filter_key)) or not key in i['extraInfo']]
            else:
                results = [i for i in results if not key in i['extraInfo']]
            ...
            results = [i for i in results if not any(
                x in i['extraInfo'] for x in key)]
        return results
    ...
    def _grab_meta(self):
        ...
        if self.media_type == 'movie':
            self.meta = metadata.movie_meta(
                'tmdb_id', self.tmdb_id, meta_user_info, get_datetime())
        else:
            self.meta = metadata.tvshow_meta(
                'tmdb_id', self.tmdb_id, meta_user_info, get_datetime())
            episodes_data = metadata.episodes_meta(
                self.season, self.meta, meta_user_info)
            try:
                episode_data = [
                    i for i in episodes_data if i['episode'] == self.episode][0]
                art_providers = get_art_provider()
                thumb = episode_data.get('thumb', None) or self.meta.get('custom_fanart') or self.meta.get(
                    art_providers[2]) or self.meta.get(art_providers[3]) or ''
    ...
    def _clear_properties(self):
        for item in default_internal_scrapers:
            clear_property(int_window_prop % item)
        if self.active_folders:
            for item in self.folder_info:
                clear_property(int_window_prop % item[0])

    def _make_progress_dialog(self):
        self.progress_dialog = create_window(
            ('windows.sources', 'SourcesPlayback'), 'sources_playback.xml', meta=self.meta)
    ...
    def furkPacks(self, name, file_id, download=False):
        ...
        if download:
            return t_files
        list_items = [{'line1': '%.2f GB | %s' % (float(
            item['size'])/1073741824, clean_file_name(item['name']).upper())} for item in t_files]
        kwargs = {'items': json.dumps(
            list_items), 'heading': name, 'enumerate': 'true', 'narrow_window': 'true'}
        ...
        self._kill_progress_dialog()
        return FendaPlayer().run(link, 'video')

    def debridPacks(self, debrid_provider, name, magnet_url, info_hash, download=False):
        show_busy_dialog()
        debrid_info = {'Real-Debrid': 'rd_browse', 'Premiumize.me': 'pm_browse',
                       'AllDebrid': 'ad_browse'}[debrid_provider]
        ...
        list_items = [{'line1': '%.2f GB | %s' % (float(
            item['size'])/1073741824, clean_file_name(item['filename']).upper())} for item in debrid_files]
        kwargs = {'items': json.dumps(
            list_items), 'heading': name, 'enumerate': 'true', 'narrow_window': 'true'}
        chosen_result = select_dialog(debrid_files, **kwargs)
        if chosen_result is None:
            return None
        link = self.resolve_internal_sources(
            debrid_info, chosen_result['link'], '')
    ...
    def play_file(self, results, source={}):
        ...
        try:
            hide_busy_dialog()
            url = None
            results = [i for i in results if not 'Uncached' in i.get(
                'cache_provider', '')]
            if not source:
                source = results[0]
            items = [source]
            ...
            for count, item in enumerate(items, 1):
                ...
                if provider == 'easynews' and self.monitor_playback and self.easynews_max_retries:
                    for retry in range(1, self.easynews_max_retries + 1):
                        resolve_item = dict(item)
                        resolve_item['resolve_display'] = '%s[CR]%s[CR]%s' % (
                            '%02d. [B]%s (RETRYx%s)[/B]' % (count, provider_text, retry), extra_info, display_name)
                        processed_items_append(resolve_item)
    ...
    def get_playback_percent(self):
        if self.media_type == 'episode' and any((self.random, self.random_continual)):
            return 0.0
        percent = get_progress_percent(get_bookmarks(settings.watched_indicators(
        ...
        if action == 'start_over':
            erase_bookmark(self.media_type, self.tmdb_id,
                           self.season, self.episode)
            return 0.0
        return float(percent)

    def get_resume_status(self, percent):
        if settings.auto_resume(self.media_type):
            return float(percent)
    ...
    def autoscrape_nextep_handler(self):
        ...
        if player.isPlaying():
            ...
            if action == 'cancel':
                ...
                if not results:
                    return notification(33092, 3000)
                if action == 'play':
                    player.stop()
                else:
            ...
            if 'cache_provider' in item:
                ...
                if self.meta['media_type'] == 'episode':
                    if hasattr(self, 'search_info'):
                        title, season, episode, pack = self.search_info['title'], self.search_info[
                            'season'], self.search_info['episode'], 'package' in item
                    else:
                        title, season, episode, pack = self.get_ep_name(
                        ), self.get_season(), self.get_episode(), 'package' in item
                else:
                    title, season, episode, pack = self.get_search_title(), None, None, False
                if cache_provider in debrid_providers:
                    url = self.resolve_cached_torrents(
                        cache_provider, item['url'], item['hash'], title, season, episode, pack)
            elif item.get('scrape_provider', None) in default_internal_scrapers:
                url = self.resolve_internal_sources(
                    item['scrape_provider'], item['id'], item['url_dl'], item.get('direct_debrid_link', False))
            elif item.get('debrid') in debrid_providers and not item['source'].lower() == 'torrent':
                url = self.resolve_debrid(
                    item['debrid'], item['provider'], item['url'])
            else:
                url = item['url']
        except:
            pass
    ...
    def resolve_cached_torrents(self, debrid_provider, item_url, _hash, title, season, episode, pack):
        ...
        store_to_cloud = settings.store_resolved_torrent_to_cloud(
            debrid_provider, pack)
        try:
    ...
    def resolve_uncached_torrents(self, debrid_provider, item_url, pack):
        if not confirm_dialog(text=ls(32831) % debrid_provider.upper()):
            return None
        debrid_function = self.debrid_importer(debrid_provider)
        try:
            debrid_function().add_uncached_torrent(item_url, pack)
        except:
            return notification(32490, 3500)
    ...
    def resolve_internal_sources(self, scrape_provider, item_id, url_dl, direct_debrid_link=False):
        ...
        try:
            if direct_debrid_link or scrape_provider == 'folders':
                ...
                url = resolve_furk(
                    item_id, self.meta['media_type'], self.meta['season'], self.meta['episode'])
                ...
                if any(i in scrape_provider for i in ('rd_', 'ad_')):
                    ...
                    if '_cloud' in scrape_provider:
                        item_id = debrid_function().get_item_details(item_id)[
                            'link']
