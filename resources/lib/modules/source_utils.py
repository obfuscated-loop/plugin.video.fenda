...
RES_4K = ('.4k', 'hd4k', '4khd', '.uhd', 'ultrahd', 'ultra.hd',
          'hd2160', '2160hd', '2160', '2160p', '216o', '216op')
RES_1080 = ('1080', '1080p', '1080i', 'hd1080', '1080hd', 'hd1080p', 'm1080p',
            'fullhd', 'full.hd', '1o8o', '1o8op', '108o', '108op', '1o80', '1o80p')
RES_720 = ('720', '720p', '720i', 'hd720', '720hd', 'hd720p', '72o', '72op')
CAM = ('.cam.', 'camrip', 'hdcam', '.hd.cam',
       'hqcam', '.hq.cam', 'cam.rip', 'dvdcam')
SCR = ('.scr.', 'screener', 'dvdscr', 'dvd.scr', '.r5', '.r6')
TELE = ('.tc.', '.ts.', 'tsrip', 'hdts', 'hdtc', '.hd.tc',
        'dvdts', 'telesync', 'tele.sync', 'telecine', 'tele.cine')
VIDEO_3D = ('.3d.', '.sbs.', '.hsbs', 'sidebyside', 'side.by.side',
            'stereoscopic', '.tab.', '.htab.', 'topandbottom', 'top.and.bottom')
...
CODEC_MPEG = ('.mpg', '.mp2', '.mpeg', '.mpe', '.mpv',
              '.mp4', '.m4p', '.m4v', 'msmpeg', 'mpegurl')
...
DOLBY_DIGITALPLUS = ('dolby.digital.plus', 'dolbydigital.plus', 'dolbydigitalplus',
                     'dd.plus.', 'ddplus', '.ddp.', 'ddp2', 'ddp5', 'ddp7', 'eac3', '.e.ac3')
DOLBY_DIGITALEX = ('.dd.ex.', 'ddex', 'dolby.ex.',
                   'dolby.digital.ex.', 'dolbydigital.ex.')
DOLBYDIGITAL = ('dd2.', 'dd5', 'dd7', 'dolby.digital',
                'dolbydigital', '.ac3', '.ac.3.', '.dd.')
DTSX = ('.dts.x.', 'dtsx')
DTS_HDMA = ('hd.ma', 'hdma')
DTS_HD = ('dts.hd.', 'dtshd')
...
SUBS = ('subita', 'subfrench', 'subspanish',
        'subtitula', 'swesub', 'nl.subs', 'subbed')
ADS = ('1xbet', 'betwin')
...
EXTRAS = ('sample', 'extra', 'extras', 'deleted', 'unused', 'footage', 'inside', 'blooper',
...
audio_filter_choices = (('DOLBY DIGITAL', 'DD'), ('DOLBY DIGITAL PLUS', 'DD+'), ('DOLBY DIGITAL EX', 'DD-EX'), ('DOLBY ATMOS', 'ATMOS'), ('DOLBY TRUEHD', 'TRUEHD'),
                        ('DTS', 'DTS'), ('DTS-HD MASTER AUDIO', 'DTS-HD MA'), ('DTS-X',
                                                                               'DTS-X'), ('DTS-HD', 'DTS-HD'), ('AAC', 'AAC'), ('OPUS', 'OPUS'), ('MP3', 'MP3'),
...
def make_alias_dict(meta, title):
    ...
    country_codes = set([i.replace('GB', 'UK')
                        for i in meta.get('country_codes', [])])
...
def pack_enable_check(meta, season, episode):
    try:
        ...
        if status in ('Ended', 'Canceled'):
            return True, True
        adjust_hours, current_date, meta_user_info = date_offset(
        ), get_datetime(), metadata_user_info()
        episodes_data = episodes_meta(season, meta, meta_user_info)
        unaired_episodes = [adjust_premiered_date(i['premiered'], adjust_hours)[
            0] for i in episodes_data]
...
def seas_ep_filter(season, episode, release_title, split=False, return_match=False):
    ...
    release_title = re.sub(r'[^A-Za-z0-9-]+', '.',
                           unquote(release_title).replace('\'', '')).lower()
    string1 = r'(s<<S>>[.-]?e[p]?[.-]?<<E>>[.-])'
    ...
    string_list_append(string1.replace(
        '<<S>>', season_fill).replace('<<E>>', episode_fill))
    string_list_append(string1.replace(
        '<<S>>', str_season).replace('<<E>>', episode_fill))
    string_list_append(string1.replace(
        '<<S>>', season_fill).replace('<<E>>', str_episode))
    string_list_append(string1.replace(
        '<<S>>', str_season).replace('<<E>>', str_episode))
    string_list_append(string2.replace(
        '<<S>>', season_fill).replace('<<E>>', episode_fill))
    string_list_append(string2.replace(
        '<<S>>', str_season).replace('<<E>>', episode_fill))
    string_list_append(string2.replace(
    ...
    string_list_append(string2.replace(
        '<<S>>', str_season).replace('<<E>>', str_episode))
    string_list_append(string3.replace('<<S>>', season_fill).replace(
        '<<E1>>', str_ep_minus_1.zfill(2)).replace('<<E2>>', episode_fill))
    string_list_append(string3.replace('<<S>>', season_fill).replace(
        '<<E1>>', episode_fill).replace('<<E2>>', str_ep_plus_1.zfill(2)))
    string_list_append(string4.replace(
        '<<S>>', season_fill).replace('<<E>>', episode_fill))
    string_list_append(string4.replace(
        '<<S>>', str_season).replace('<<E>>', episode_fill))
...
def find_season_in_release_title(release_title):
    release_title = re.sub(r'[^A-Za-z0-9-]+', '.',
                           unquote(release_title).replace('\'', '')).lower()
    match = None
    regex_list = [r's(\d+)', r's\.(\d+)', r'(\d+)x',
                  r'(\d+)\.x', r'season(\d+)', r'season\.(\d+)']
    ...
    try:
        ...
        for i in UNWANTED_TAGS:
            if releasetitle_startswith(i):
                ...
                pattern = r'\%s' % i if i_startswith(
                    '[') or i_startswith('+') else r'%s' % i
                release_title = re.sub(
                    r'^%s' % pattern, '', release_title, 1, re.I)
        ...
        if hdlr:
            ...
            release_title = release_title.replace(year, '').replace('(', '').replace(
                ')', '').replace('&', 'and').rstrip('.-').rstrip('.').rstrip('-').replace(':', '')
...
def get_release_quality(release_info):
    if any(i in release_info for i in SCR):
        return 'SCR'
    if any(i in release_info for i in CAM):
        return 'CAM'
    if any(i in release_info for i in TELE):
        return 'TELE'
    if any(i in release_info for i in RES_720):
        return '720p'
    if any(i in release_info for i in RES_1080):
        return '1080p'
    if any(i in release_info for i in RES_4K):
        return '4K'
    return None
...
def get_info(title):
    ...
    if any(i in title for i in VIDEO_3D):
        info_append('[B]3D[/B]')
    if '.sdr' in title:
        info_append('SDR')
    elif any(i in title for i in DOLBY_VISION):
        info_append('[B]D/VISION[/B]')
    elif any(i in title for i in HDR):
        info_append('[B]HDR[/B]')
    elif all(i in title for i in ('2160p', 'remux')):
        info_append('[B]HDR[/B]')
    if '[B]D/VISION[/B]' in info:
        if any(i in title for i in HDR_TRUE) or 'hybrid' in title:
            info_append('[B]HDR[/B]')
        if '[B]HDR[/B]' in info:
            info_append('[B]HYBRID[/B]')
    if any(i in title for i in CODEC_H264):
        info_append('AVC')
    elif '.av1.' in title:
        info_append('[B]AV1[/B]')
    elif any(i in title for i in CODEC_H265):
        info_append('[B]HEVC[/B]')
    elif any(i in info for i in ('[B]HDR[/B]', '[B]D/VISION[/B]')):
        info_append('[B]HEVC[/B]')
    elif any(i in title for i in CODEC_XVID):
        info_append('XVID')
    elif any(i in title for i in CODEC_DIVX):
        info_append('DIVX')
    if any(i in title for i in REMUX):
        info_append('REMUX')
    if any(i in title for i in BLURAY):
        info_append('BLURAY')
    elif any(i in title for i in DVD):
        info_append('DVD')
    elif any(i in title for i in WEB):
        info_append('WEB')
    elif 'hdtv' in title:
        info_append('HDTV')
    elif 'pdtv' in title:
        info_append('PDTV')
    elif any(i in title for i in HDRIP):
        info_append('HDRIP')
    if 'atmos' in title:
        info_append('ATMOS')
    if any(i in title for i in DOLBY_TRUEHD):
        info_append('TRUEHD')
    if any(i in title for i in DOLBY_DIGITALPLUS):
        info_append('DD+')
    elif any(i in title for i in DOLBY_DIGITALEX):
        info_append('DD-EX')
    elif any(i in title for i in DOLBYDIGITAL):
        info_append('DD')
    if 'aac' in title:
        info_append('AAC')
    elif 'mp3' in title:
        ...
        info_append('OPUS')
    if any(i in title for i in DTSX):
        info_append('DTS-X')
    elif any(i in title for i in DTS_HDMA):
        info_append('DTS-HD MA')
    elif any(i in title for i in DTS_HD):
        info_append('DTS-HD')
    elif '.dts' in title:
        info_append('DTS')
    if any(i in title for i in AUDIO_8CH):
        info_append('8CH')
    elif any(i in title for i in AUDIO_7CH):
        info_append('7CH')
    elif any(i in title for i in AUDIO_6CH):
        info_append('6CH')
    elif any(i in title for i in AUDIO_2CH):
        info_append('2CH')
    if '.wmv' in title:
        info_append('WMV')
    elif any(i in title for i in CODEC_MPEG):
        info_append('MPEG')
    elif '.avi' in title:
        info_append('AVI')
    elif any(i in title for i in CODEC_MKV):
        info_append('MKV')
    if any(i in title for i in MULTI_LANG):
        info_append('MULTI-LANG')
    if any(i in title for i in ADS):
        info_append('ADS')
    if any(i in title for i in SUBS):
        info_append('SUBS')
...
def get_cache_expiry(media_type, meta, season):
    try:
        ...
        if media_type == 'movie':
            ...
            if difference == 0:
                single_expiry = expiry_3hrs
            elif difference <= 7:
                single_expiry = expiry_1day
            elif difference <= 14:
                single_expiry = expiry_2days
            elif difference <= 21:
                single_expiry = expiry_3days
            elif difference <= 30:
                single_expiry = expiry_4days
            elif difference <= 60:
                single_expiry = expiry_7days
            else:
                single_expiry = expiry_14days
            season_expiry, show_expiry = 0, 0
        else:
            recently_ended = False
            extra_info = meta['extra_info']
            ended = extra_info['status'] in ('Ended', 'Canceled')
            premiered = adjust_premiered_date(
                meta['premiered'], date_offset())[0]
            difference = subtract_dates(current_date, premiered)
            last_episode_to_air = jsondate_to_datetime(
                extra_info['last_episode_to_air']['air_date'], '%Y-%m-%d', remove_time=True)
            last_ep_difference = subtract_dates(
                current_date, last_episode_to_air)
            ...
            if not ended or recently_ended:
                ...
                if meta['total_seasons'] == season:
                    if last_ep_difference <= 7:
                        season_expiry = expiry_3days
                    else:
                        season_expiry = expiry_10days
                else:
                    season_expiry = expiry_30days
                show_expiry = expiry_10days
            else:
                single_expiry, season_expiry, show_expiry = expiry_10days, expiry_30days, expiry_30days
    except:
        single_expiry, season_expiry, show_expiry = expiry_3days, expiry_3days, expiry_10days
...
def gather_assigned_content(string):
    ...
    if results:
        ...
        results = [i for i in results if i[1].get(
            'media_type') in ('movie', 'episode')]
        return results
    else:
        return []
...
def test_assigned_content(string, assigned_content):
    if not assigned_content:
        return None
    try:
        result = [i[1].get('rootname').upper()
                  for i in assigned_content if i[0] == string][0]
        return result
    except:
        return None
