...
class FendaPlayer(xbmc_player):
    ...
    def check_playback_start(self):
        ...
        while self.playback_successful is None:
            ...
            self.sources_object.progress_dialog.update_resolver(
        ...
        try:
            ...
            if self.media_type == 'episode':
                ...
                if any((play_random_continual, play_random, disable_autoplay_next_episode)):
                    ...
                    self.autoplay_nextep, self.autoscrape_nextep = self.sources_object.autoplay_nextep, self.sources_object.autoscrape_nextep
            else:
    ...
    def make_listing(self):
        ...
        if self.is_generic:
            ...
            self.tmdb_id, self.imdb_id, self.tvdb_id = self.meta_get(
                'tmdb_id', ''), self.meta_get('imdb_id', ''), self.meta_get('tvdb_id', '')
            self.media_type, self.title, self.year = self.meta_get(
                'media_type'), self.meta_get('title'), self.meta_get('year')
            self.season, self.episode = self.meta_get(
                'season', ''), self.meta_get('episode', '')
            self.auto_resume, self.fanart_enabled = auto_resume(
                self.media_type), get_fanart_data()
            ...
            poster = self.meta_get('custom_poster') or self.meta_get(
                poster_main) or self.meta_get(poster_backup) or poster_empty
            fanart = self.meta_get('custom_fanart') or self.meta_get(
                fanart_main) or self.meta_get(fanart_backup) or fanart_empty
            ...
            if self.fanart_enabled:
                banner = self.meta_get(
                    'custom_banner') or self.meta_get('banner') or ''
                clearart = self.meta_get(
                    'custom_clearart') or self.meta_get('clearart') or ''
                landscape = self.meta_get(
                    'custom_landscape') or self.meta_get('landscape') or ''
            else:
                banner, clearart, landscape = '', '', ''
            duration, plot, genre, trailer, mpaa = self.meta_get('duration'), self.meta_get(
                'plot'), self.meta_get('genre', ''), self.meta_get('trailer'), self.meta_get('mpaa')
            rating, votes = self.meta_get('rating'), self.meta_get('votes')
            premiered, studio, tagline = self.meta_get(
                'premiered'), self.meta_get('studio', ''), self.meta_get('tagline')
            director, writer, cast, country = self.meta_get('director', ''), self.meta_get(
                'writer', ''), self.meta_get('cast', []), self.meta_get('country', '')
            listitem.setLabel(self.title)
            if self.media_type == 'movie':
                if self.fanart_enabled:
                    discart = self.meta_get(
                        'custom_discart') or self.meta_get('discart') or ''
                    keyart = self.meta_get(
                        'custom_keyart') or self.meta_get('keyart') or ''
                else:
                    discart, keyart = '', ''
                ...
                info_tag.setCast([xbmc_actor(
                    name=item['name'], role=item['role'], thumbnail=item['thumbnail']) for item in cast])
            else:
                ...
                info_tag.setStudios((studio or '',))
                info_tag.setUniqueIDs({'imdb': self.imdb_id, 'tmdb': str(
                    self.tmdb_id), 'tvdb': str(self.tvdb_id)})
                info_tag.setIMDBNumber(self.imdb_id)
                info_tag.setGenres(genre.split(', '))
                info_tag.setWriters(writer.split(', '))
                info_tag.setDirectors(director.split(', '))
    ...
    def media_watched_marker(self, force_watched=False):
        ...
        try:
            if self.current_point >= self.set_watched or force_watched:
                if self.media_type == 'movie':
                    watched_function = mark_movie
                else:
                    watched_function = mark_episode
                ...
                Thread(target=self.run_media_progress, args=(
                    watched_function, watched_params)).start()
            else:
                clear_property('fenda.random_episode_history')
                if self.current_point >= self.set_resume:
                    ...
                    Thread(target=self.run_media_progress, args=(
                        set_bookmark, progress_params)).start()
        except:
            pass
    ...
    def run_subtitles(self):
        ...
        try:
            Thread(target=Subtitles().get, args=(self.title, self.imdb_id,
                   self.season or None, self.episode or None)).start()
        except:
            pass
    ...
    def info_next_ep(self):
        ...
        try:
            ...
            ) if nextep_settings['use_chapters'] else None
            percentage = 100 - \
                final_chapter if final_chapter else nextep_settings['window_percentage']
            ...
            self.nextep_settings = {'use_window': use_window, 'window_time': window_time,
                                    'default_action': default_action, 'play_type': play_type}
        except:
            pass
    ...
    def set_playback_properties(self):
        try:
            trakt_ids = {'tmdb': self.tmdb_id, 'imdb': self.imdb_id,
            ...
            if self.playing_filename:
                set_property('subs.player_filename', self.playing_filename)
        except:
            pass
...
class Subtitles(xbmc_player):
    def __init__(self):
        ...
        self.auto_enable = get_setting('fenda.subtitles.auto_enable')
        self.subs_action = get_setting('fenda.subtitles.subs_action')
        self.language = get_setting('fenda.subtitles.language_primary')
        self.quality = ['bluray', 'hdrip', 'brrip', 'bdrip', 'dvdrip',
                        'webdl', 'webrip', 'webcap', 'web', 'hdtv', 'hdrip']
    ...
    def get(self, query, imdb_id, season, episode, secondary_search=False):
        ...
        def _searched_subs():
            ...
            result = self.os.search(
                query, imdb_id, self.language, season, episode)
            ...
            if self.subs_action == '1':
                self.pause()
                choices = [i for i in result if i['SubLanguageID']
                           == self.language and i['SubSumCD'] == '1']
                if len(choices) == 0:
                    return False
                dialog_list = ['[B]%s[/B] | [I]%s[/I]' %
                               (i['SubLanguageID'].upper(), i['MovieReleaseName']) for i in choices]
                list_items = [{'line1': item} for item in dialog_list]
                kwargs = {'items': json.dumps(list_items), 'heading': video_path.replace(
                    '%20', ' '), 'enumerate': 'true', 'narrow_window': 'true'}
                ...
                try:
                    chosen_sub = [i for i in result if i['MovieReleaseName'].lower() in video_path.lower(
                    ) and i['SubLanguageID'] == self.language and i['SubSumCD'] == '1'][0]
                except:
                    pass
                if not chosen_sub:
            ...
            subtitle = self.os.download(
                download_url, subtitle_path, temp_zip, temp_path, final_path)
            sleep(1000)
            return subtitle
        ...
        if subtitle:
            return self.setSubtitles(subtitle)
        if secondary_search:
            return _notification(32793)
        secondary_language = get_setting('fenda.subtitles.language_secondary')
        if secondary_language in (self.language, None, 'None', ''):
            return _notification(32793)
