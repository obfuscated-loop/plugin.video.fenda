...
main_list_name_dict = {'RootList': ls(
    32457), 'MovieList': ls(32028), 'TVShowList': ls(32029)}
pos_str, top_pos_str, top_str, exists_str, addq_str, eadd_str, erename_str = ls(
    32707), ls(32708), ls(32709), ls(32727), ls(32728), ls(33140), ls(32137)
...
class MenuEditor:
    def __init__(self, params):
        ...
        if 'menu_item' in self.params:
            self.menu_item = json.loads(self.params.get('menu_item'))
        else:
            self.menu_item = dict(parse_qsl(get_infolabel(
                'ListItem.FileNameAndPath').replace('plugin://plugin.video.fenda/?', '')))
        self.menu_item_get = self.menu_item.get
        self.window_function = activate_window if external() else container_update

    def edit_menu(self):
        active_list, position = self.params_get(
            'active_list'), int(self.params_get('position', '0'))
        ...
        external_list_item, shortcut_folder = self.menu_item_get(
            'external_list_item', 'False') == 'True', self.menu_item_get('shortcut_folder', 'False') == 'True'
        ...
        if not shortcut_folder:
            append((add_org_str % menu_name_translated_display,
                   self.add_original_external))
        append((fol_add_str % menu_name_translated_display,
               self.shortcut_folder_add_item))
        ...
        if get_setting('fenda.trakt.user', ''):
            append((trak_add_str % list_name, self.add_trakt))
        append((res_str % list_name, self.restore))
        append((upd_str % list_name, self.check_update_list))
        if not external_list_item:
            append((reload_str % menu_name_translated_display, self.reload_menu_item))
        ...
        if function == None:
            return
        self.params = {'active_list': active_list, 'list_name': list_name, 'menu_name': menu_name,
                       'menu_name_translated': menu_name_translated, 'position': position}
        self.params_get = self.params.get
        return function()

    def edit_menu_shortcut_folder(self):
        listing = [(eadd_str, 'add'), ('Rename', 'rename'), (emove_str,
                                                             'move'), (eremove_str, 'remove'), (eclear_str, 'clear_all')]
        list_items = [{'line1': i[0]} for i in listing]
        kwargs = {'items': json.dumps(list_items), 'narrow_window': 'true'}
    ...
    def edit_menu_external(self):
        mode, list_type, action = self.menu_item_get('mode'), self.menu_item_get(
            'list_type'), self.menu_item_get('action')
        listing = []
        append = listing.append
        if mode == 'navigator.build_shortcut_folder_list':
            ...
            append((self._remove_bold(eeremove_str), self.remove_search_history),
                   (self._remove_bold(eeclear_str), self.clear_search_history))
            ...
            if mode == 'furk.furk_t_file_browser':
                if self.params_get('display_mode') == 'search':
                    append((self._remove_bold(furk_add_str), self.furk_add_file))
                else:
                    append((self._remove_bold(furk_remove_str),
                           self.furk_remove_file))
                    if self.params_get('is_protected') == '0':
                        append((self._remove_bold(furk_p_str),
                               self.furk_protect_file))
                    elif self.params_get('is_protected') == '1':
                        append((self.remove_bold(furk_up_str),
                               self.furk_unprotect_file))
        elif mode in ('real_debrid.browse_rd_cloud', 'alldebrid.browse_ad_cloud') or self.params_get('service') == 'FOLDERS':
            append((self._remove_bold(cloud_link_str), self.add_link_to_debrid_folders),
                   (self._remove_bold(cloud_unlink_str), self.remove_link_to_debrid_folders))
        else:
            append((self._remove_bold(eefadde_str), self.add_external))
            append((self._remove_bold(eefadd_str), self.shortcut_folder_add_item))
            if action == 'tmdb_movies_sets':
                append((self._remove_bold(eesets_str),
                       self.movie_sets_to_collection))
            elif mode == 'trakt.list.build_trakt_list':
                if list_type == 'user_lists':
                    if not self.menu_item_get('user') == 'Trakt Official':
                        append((self._remove_bold(trlike_str),
                               self.trakt_like_list))
                        append((self._remove_bold(trunlike_str),
                               self.trakt_unlike_list))
                ...
                append((self._remove_bold(eerem_disc_str),
                       self.remove_single_discover_history))
                append((self._remove_bold(eeclear_str),
                       self.remove_all_discover_history))
        append((tools_str, self.open_tools))
        append((settings_str, self.open_settings))
        list_items = [{'line1': i[0]} for i in listing]
        kwargs = {'items': json.dumps(list_items), 'narrow_window': 'true'}
    ...
    def browse(self):
        ...
        list_name = main_list_name_dict[active_list]
        try:
            choice_items = self._get_removed_items(active_list)
        except:
    ...
    def move(self):
        ...
        choice_items = [i for i in list_items if str(
            i['name']) != str(self.params_get('menu_name'))]
        current_position = self.params_get('position')
        new_position = self._menu_select(choice_items, self.params_get(
            'menu_name_translated'), multi_line='true', position_list=True)
        if new_position == None or new_position == current_position:
            return
    ...
    def remove(self):
        ...
        if len(list_items) == 1:
            return notification(32736, 1500)
        list_items = [i for i in list_items if str(
            i['name']) != str(self.params_get('menu_name'))]
        self._db_execute('set', active_list, list_items)
    ...
    def add_original(self):
        ...
        list_items = navigator_cache.currently_used_list(active_list)
        position = self._menu_select(
            list_items, menu_name_translated, multi_line='true', position_list=True)
        if position == None:
            return notification(32736, 1500)
    ...
    def add_original_external(self):
        ...
        position = self._menu_select(
            list_items, menu_name or menu_name_translated, multi_line='true', position_list=True)
        if position == None:
            return notification(32736, 1500)
        list_items.insert(position, self._add_external_info_to_item(
            self.menu_item, menu_name, False))
    ...
    def add_trakt(self):
        ...
        if trakt_selection == None:
            return notification(32736, 1500)
        active_list = self.params_get('active_list')
        list_items = navigator_cache.currently_used_list(active_list)
        menu_name = self._get_external_name_input(trakt_selection['name'])
        position = self._menu_select(
            list_items, menu_name or trakt_selection['name'], multi_line='true', position_list=True)
        if position == None:
            return notification(32736, 1500)
        trakt_selection.update(
            {'iconImage': 'trakt', 'mode': 'trakt.lists.build_trakt_list'})
        list_items.insert(position, self._add_external_info_to_item(
            trakt_selection, menu_name, False))
        self._db_execute('set', active_list, list_items)
    ...
    def restore(self):
        if not confirm_dialog():
            return notification(32736, 1500)
        active_list = self.params_get('active_list')
        self._db_execute('delete', active_list,
                         list_type='edited', refresh=False)
        self._db_execute('set', active_list,
                         main_menus[active_list], 'default')
    ...
    def reload_menu_item(self):
        ...
        try:
            new_item = [i for i in default if str(
                i['name']) == str(menu_name)][0]
        except:
            return notification(32760, 1500)
        list_items = [i for i in current_list if str(
            i['name']) != str(menu_name)]
    ...
    def check_update_list(self):
        ...
        if list_type == 'edited':
            self._db_execute('set', active_list, new_contents, 'default')
    ...
    def add_external(self):
        ...
        position = self._menu_select(
            list_items, menu_name, multi_line='true', position_list=True)
        if position == None:
            return notification(32736, 1500)
        list_items.insert(position, self._add_external_info_to_item(
            self.menu_item, menu_name))
        self._db_execute('set', choice_name, list_items, refresh=False)
    ...
    def shortcut_folder_contents_adjust(self):
        ...
        if self.action == 'clear_all':
            ...
            list_items = navigator_cache.get_shortcut_folder_contents(
                active_list)
            if self.action == 'add':
                return self.shortcut_folder_browse_for_content()
            if self.action == 'rename':
                try:
                    ...
                    if not new_item_name or new_item_name == item_name:
                        return
                ...
                new_position = self._menu_select(
                    choice_items, list_name, multi_line='true', position_list=True)
                if new_position == None or new_position == current_position:
                    return
                list_items.insert(
                    new_position, list_items.pop(current_position))
        self._db_execute('set', active_list, list_items,
    ...
    def shortcut_folder_make(self):
        ...
        if not list_name:
            return
        self._db_execute('make_new_folder', list_name,
                         list_type='shortcut_folder')

    def shortcut_folder_add_item(self):
        shortcut_folders = navigator_cache.get_shortcut_folders()
        if len(shortcut_folders) == 1:
            choice_name, choice_list = shortcut_folders[0]
        elif shortcut_folders:
            choice = self._menu_select(
                [{'name': i[0], 'iconImage': 'folder'} for i in shortcut_folders], '')
            if choice == None:
                return notification(32736, 1500)
            choice_name, choice_list = shortcut_folders[choice]
        else:
            if not confirm_dialog(text=32702, default_control=10):
                return notification(32736, 1500)
            self.shortcut_folder_make()
            try:
                choice_name, choice_list = navigator_cache.get_shortcut_folders()[
                    0]
            except:
                return notification(32736, 1500)
        list_items = choice_list
        name = self._remove_bold(self.params_get(
            'name') or self.params_get('menu_name_translated'))
        menu_name = self._get_external_name_input(name) or name
        icon_choice = self._icon_select(default_icon=self.params_get(
            'iconImage', None) or self.menu_item_get('iconImage') or 'folder')
        ...
        self._db_execute('set', choice_name, list_items,
                         'shortcut_folder', False)

    def shortcut_folder_add_to_main_menu(self):
        shortcut_folders = navigator_cache.get_shortcut_folders()
        if len(shortcut_folders) == 1:
            name = shortcut_folders[0][0]
        elif shortcut_folders:
            choice = self._menu_select(
                [{'name': i[0], 'iconImage': 'folder'} for i in shortcut_folders], '')
            if choice == None:
            ...
            self.shortcut_folder_make()
            try:
                name = navigator_cache.get_shortcut_folders()[0][0]
            except:
                return notification(32736, 1500)
        active_list = self.params_get('active_list')
        list_items = navigator_cache.currently_used_list(active_list)
        position = self._menu_select(list_items, self.params_get(
            'menu_name_translated'), multi_line='true', position_list=True)
        if position == None:
            return notification(32736, 1500)
        icon_choice = self._icon_select(default_icon='folder')
        chosen_folder = {'mode': 'navigator.build_shortcut_folder_list', 'name': name,
                         'iconImage': icon_choice, 'shortcut_folder': 'True', 'external_list_item': 'True'}
    ...
    def shortcut_folder_delete(self):
        ...
        main_menu_items_list = [
            (i, navigator_cache.currently_used_list(i)) for i in default_menu_items]
    ...
    def shortcut_folder_browse_for_content(self):
        ...
        choice_name, list_items = list_name, navigator_cache.get_shortcut_folder_contents(
            list_name)
        browsed_result = self._path_browser()
        if browsed_result == None:
            return
        self.menu_item = dict(
            parse_qsl(browsed_result['file'].replace('plugin://plugin.video.fenda/?', '')))
        name, icon = self._remove_bold(browsed_result['label']), self._get_icon_var(
            browsed_result['thumbnail'])
        menu_name = self._get_external_name_input(name) or name
    ...
    def trakt_like_list(self):
        ...
        params = {'user': self.menu_item_get(
            'user'), 'list_slug': self.menu_item_get('slug')}
    ...
    def movie_sets_to_collection(self):
        ...
        movie_sets_to_collection_choice(
            {'collection_id': self.menu_item_get('tmdb_id')})

    def remove_search_history(self):
        from modules.history import remove_from_search_history
        params = {'setting_id': self.menu_item_get(
            'setting_id'), 'query': self.menu_item_get('query')}
    ...
    def furk_protect_file(self):
        ...
        myfiles_protect_unprotect('protect', self.menu_item_get(
            'name'), self.menu_item_get('item_id'))

    def furk_unprotect_file(self):
        from indexers.furk import myfiles_protect_unprotect
        myfiles_protect_unprotect('unprotect', self.menu_item_get(
            'name'), self.menu_item_get('item_id'))
    ...
    def add_link_to_debrid_folders(self):
        ...
        link_folders_choice({'service': self.params_get(
            'service'), 'folder_id': folder_id, 'action': 'add'})

    def remove_link_to_debrid_folders(self):
        from indexers.dialogs import link_folders_choice
        folder_id = self.menu_item_get('id', None) or self.params_get('id')
        link_folders_choice({'service': self.params_get(
            'service'), 'folder_id': folder_id, 'action': 'remove'})
    ...
    def _menu_select(self, choice_items, menu_name, heading='', multi_line='false', position_list=False):
        def _builder():
            for item in choice_items:
                item_get = item.get
                line2 = pos_str % (menu_name, ls(
                    item_get('name', None) or item_get('list_name')) if position_list else '')
        ...
        if position_list:
            list_items.insert(
                0, {'line1': top_str, 'line2': top_pos_str % menu_name, 'icon': get_icon('top')})
        index_list = [list_items.index(i) for i in list_items]
        kwargs = {'items': json.dumps(
            list_items), 'heading': heading, 'multi_line': multi_line}
    ...
    def _icon_select(self, default_icon=''):
        ...
        icon_choice = select_dialog(
            all_icons, **kwargs) or default_icon or 'folder'
        return icon_choice
    ...
    def _get_removed_items(self, active_list):
        default_list_items, list_items = navigator_cache.get_main_lists(
            active_list)
    ...
    def _add_external_info_to_item(self, menu_item, menu_name=None, add_all_params=True):
        ...
        icon_choice = self._icon_select(
            default_icon=menu_item.get('iconImage', 'folder'))
    ...
    def _remove_active_shortcut_folder(self, main_menu_items_list, folder_name):
        for x in main_menu_items_list:
            try:
                match = [i for i in x[1] if str(
                    i['name']) == str(folder_name)][0]
    ...
    def _db_execute(self, db_action, list_name, list_contents=[], list_type='edited', refresh=True):
        if db_action == 'set':
            navigator_cache.set_list(list_name, list_type, list_contents)
        elif db_action == 'delete':
            navigator_cache.delete_list(list_name, list_type)
        elif db_action == 'make_new_folder':
            navigator_cache.set_list(
                list_name, 'shortcut_folder', list_contents)
