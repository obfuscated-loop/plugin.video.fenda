...
class MenuEditor:
	def __init__(self, params):
		...
		else: self.menu_item = dict(parse_qsl(get_infolabel('ListItem.FileNameAndPath').replace('plugin://plugin.video.fenda/?','')))
	...
	def edit_menu(self):
		...
		if get_setting('fenda.trakt.user', ''): append((trak_add_str % list_name, self.add_trakt))
	...
	def shortcut_folder_browse_for_content(self):
		...
		self.menu_item = dict(parse_qsl(browsed_result['file'].replace('plugin://plugin.video.fenda/?','')))
