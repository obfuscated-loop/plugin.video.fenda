...
tv_str, furk_str, easy_str, peop_str, imdb_str, coll_str = ls(
    32029), ls(32069), ls(32070), ls(32507), ls(32064), ls(33080)
clear_history_list = [(insert_string_4 % (delete_str, mov_str, search_str, hist_str), 'movie_queries'),
                      (insert_string_4 % (delete_str, tv_str,
                       search_str, hist_str), 'tvshow_queries'),
                      (insert_string_4 % (delete_str, peop_str,
                       search_str, hist_str), 'people_queries'),
                      (insert_string_5 % (delete_str, imdb_str, key_str,
                       mov_str, hist_str), 'keyword_imdb_movie_queries'),
                      (insert_string_5 % (delete_str, imdb_str, key_str,
                       tv_str, hist_str), 'keyword_imdb_tvshow_queries'),
                      (insert_string_5 % (delete_str, furk_str, vid_str,
                       search_str, hist_str), 'furk_video_queries'),
                      (insert_string_4 % (delete_str, easy_str,
                       search_str, hist_str), 'easynews_video_queries'),
...
def get_search_term(params):
    ...
    if search_type == 'media_title':
        mode, action, string = ('build_movie_list', 'tmdb_movies_search', 'movie_queries') if media_type == 'movie' else (
            'build_tvshow_list', 'tmdb_tv_search', 'tvshow_queries')
        ...
        url_params, string = {
            'mode': 'easynews.search_easynews'}, 'easynews_video_queries'
    elif search_type == 'tmdb_movie_sets':
        url_params, string = {'mode': 'build_movie_list',
                              'action': 'tmdb_movies_search_sets'}, 'tmdb_movie_sets_queries'
    ...
    action = 'ActivateWindow(Videos,%s,return)' if external(
    ) else 'Container.Update(%s)'
...
def remove_from_search_history(params):
    try:
        ...
        main_cache.set(params['setting_id'], result,
...
def clear_search_history():
    try:
        ...
        setting_id = select_dialog([item[1]
                                   for item in clear_history_list], **kwargs)
        if setting_id == None:
            return
