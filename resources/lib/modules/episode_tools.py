...
hidden_ind_str, hidden_str, heading, window_prop = ' [COLOR=red][B][%s][/B][/COLOR]', ls(
    32804).upper(), ls(32806), 'fenda.random_episode_history'
...
class EpisodeTools:
    ...
    def get_next_episode_info(self):
        try:
            ...
            current_season, current_episode = int(
                self.meta_get('season')), int(self.meta_get('episode'))
            curr_season_data = [
            ...
            episode = current_episode + \
                1 if current_episode < curr_season_data['episode_count'] else 1
            ep_data = episodes_meta(
                season, self.meta, settings.metadata_user_info())
            if not ep_data:
            ...
            if play_type == 'autoscrape_nextep':
                url_params['prescrape'] = 'false'
            if custom_title:
                url_params['custom_title'] = custom_title
            if 'custom_year' in self.meta:
                url_params['custom_year'] = self.meta_get('custom_year')
        except:
            url_params = 'error'
        return url_params
    ...
    def get_random_episode(self, continual=False, first_run=True):
        try:
            meta_user_info, adjust_hours, current_date = settings.metadata_user_info(
            ), settings.date_offset(), get_datetime()
            tmdb_id = self.meta_get('tmdb_id')
            tmdb_key = str(tmdb_id)
            try:
                episodes_data = [i for i in all_episodes_meta(self.meta, meta_user_info) if i['premiered'] and adjust_premiered_date(
                    i['premiered'], adjust_hours)[0] <= current_date]
            except:
                return None
            if continual:
                episode_list = []
                try:
                    ...
                    pass
                episodes_data = [
                    i for i in episodes_data if not i in episode_list]
            ...
            title, season, episode = self.meta['title'], int(
                chosen_episode['season']), int(chosen_episode['episode'])
            ...
            try:
                premiered = adjust_premiered_date(
                    chosen_episode['premiered'], adjust_hours)[1]
            except:
...
def build_next_episode_manager():
    def _process(item):
        try:
            ...
            if tmdb_id in hidden_list:
                display, action = title + hidden_ind_str % hidden_str, 'unhide'
            else:
                display, action = title, 'hide'
            url_params = {'mode': mode, 'action': action, 'media_type': 'shows',
                          'media_id': tmdb_id, 'section': 'progress_watched'}
            url = build_url(url_params)
            listitem.setLabel(display)
            listitem.setArt(
                {'poster': icon, 'fanart': addon_fanart, 'icon': icon})
    ...
    if indicators == 0:
        icon, mode = get_icon('folder'), 'hide_unhide_progress_items'
    else:
        icon, mode = get_icon('trakt'), 'trakt.hide_unhide_progress_items'
