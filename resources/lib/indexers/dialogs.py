...
single_ep_list = ('episode.progress', 'episode.recently_watched', 'episode.next_trakt',
                  'episode.next_fenda', 'episode.trakt_recently_aired', 'episode.trakt_calendar')
scraper_names = [ls(32118).upper(), ls(32069).upper(), ls(32070).upper(), ls(
    32098).upper(), ls(32097).upper(), ls(32099).upper(), ls(32108).upper()]
network_str, created_by_str, last_aired_str, next_aired_str, seasons_str, episodes_str, favorites_str = ls(
...
tagline_str, premiered_str, rating_str, votes_str, runtime_str, easy_serv_str = ls(
    32619), ls(32620), ls(32621), ls(32623), ls(32622), ls(32696)
genres_str, budget_str, revenue_str, director_str, writer_str = ls(
    32624), ls(32625), ls(32626), ls(32627), ls(32628)
title_str, year_str, season_str, episode_str = ls(32228), ls(
    32543), ls(32537), ls(32203).lower().capitalize()
...
def external_scraper_choice(params):
    try:
        ...
        return
    list_items = [{'line1': i['name'], 'icon': i['thumbnail']}
                  for i in results]
...
def auth_accounts_choice(params):
    ...
    if service == 'realdebrid':
        ...
        if active:
            trakt_api.trakt_revoke_authentication()
        else:
...
def audio_filters_choice(params={}):
    icon = get_icon('audio')
    list_items = [{'line1': item[0], 'line2': item[1], 'icon': icon}
                  for item in audio_filter_choices]
    try:
        preselect = [audio_filter_choices.index(
            item) for item in audio_filter_choices if item[1] in audio_filters()]
    except:
        preselect = []
    kwargs = {'items': json.dumps(list_items), 'heading': ls(
        32002), 'multi_choice': 'true', 'multi_line': 'true', 'preselect': preselect}
    selection = select_dialog([i[1] for i in audio_filter_choices], **kwargs)
    if selection == None:
        return
    if selection == []:
        set_setting('filter_audio', '')
    else:
...
def movie_sets_to_collection_choice(params):
    ...
    trakt_collection_ids = [i['media_ids']['tmdb'] for i in trakt_fetch_collection_watchlist(
        'collection', 'movies') if i['media_ids']['tmdb']]
    ...
    list_items = []
    for item in parts:
        tmdb_id, title, release_date, poster_path, release_date = item['id'], item[
            'title'], item['release_date'], item['poster_path'], item['release_date']
        if poster_path:
            icon = tmdb_image_url % poster_path
        else:
            icon = poster_empty
        ...
        if in_collection:
            ...
            line2 = ls(33083)
        list_items.append({'line1': line1, 'line2': line2, 'icon': icon, 'tmdb_id': tmdb_id,
                          'in_collection': in_collection, 'release_date': release_date})
    list_items.sort(key=lambda k: k['release_date'] or '2050')
    preselect = [list_items.index(i)
                 for i in list_items if not i['in_collection']]
    kwargs = {'items': json.dumps(list_items), 'heading': collection_name,
              'multi_choice': 'true', 'multi_line': 'true', 'preselect': preselect}
    ...
    try:
        result = add_to_collection(
            {'movies': [{'ids': {'tmdb': i}} for i in selection]}, multi=True)
    except:
        result = {'added': {'movies': 0}}
    hide_busy_dialog()
    ...
    if movies_added:
        ...
        if 'existing' in result and result['existing']['movies'] == movies_to_add:
            text = 32765
        else:
            text = 33047
        ok_dialog(text=text)
...
def link_folders_choice(params):
    ...
    def _builder():
        for item in results:
            try:
                ...
                if poster_path:
                    icon = tmdb_image_base % poster_path
                else:
                    icon = poster_empty
                function_list_append(
                    {'rootname': rootname, 'tmdb_id': tmdb_id})
    ...
    data = {'media_type': 'episode' if media_type == 'tvshow' else media_type,
            'rootname': chosen['rootname'], 'tmdb_id': str(chosen['tmdb_id'])}
        ...
        for i in start_list:
            try:
                if i == 1 and widget_content:
                    ...
                    first_alpha, last_alpha = title_key(first_entry, ignore).replace(
                        ' ', '')[0:3], title_key(last_entry, ignore).replace(' ', '')[0:3]
            ...
            yield {'line1': line1}
    try:
        total_pages, all_pages = int(params.get(
            'total_pages')), json.loads(params.get('all_pages'))
        if all_pages[0] == []:
            widget_content = True
        else:
            widget_content = False
        fenda_highlight = get_property(highlight_prop)
        start_list = [i for i in range(1, total_pages+1)]
        ignore, jump_to, current_page = ignore_articles(), int(
            params.get('jump_to_enabled', '0')), int(params.get('current_page'))
        list_items = list(_builder())
        kwargs = {'items': json.dumps(list_items), 'narrow_window': 'true'}
        new_page = select_dialog(start_list, **kwargs)
        if new_page == None or new_page == current_page:
            return
        if new_page == 1 and widget_content:
            function, paginate_start = container_refresh_input, '0'
        else:
            function, paginate_start = container_update, params.get(
                'paginate_start', '0')
...
def media_artwork_choice(meta, changed_artwork=False):
    meta = dict(meta)
    all_images = meta.get('images', {'poster': [], 'fanart': [], 'clearlogo': [
    ], 'banner': [], 'clearart': [], 'landscape': [], 'discart': [], 'keyart': []})
    if not metadata_user_info()['extra_fanart_enabled']:
        for k in ('banner', 'clearart', 'landscape', 'discart', 'keyart'):
            all_images.pop(k, None)
        all_images = [{k: [i for i in v if 'image.tmdb' in i]
                       for k, v in all_images.items()}][0]
    if all(i == [] for i in all_images.values()):
        return notification(33069, 2000)
    kwargs = {'images': all_images, 'meta': meta}
    custom_images = open_window(
        ('windows.artwork', 'SelectArtwork'), 'artwork.xml', **kwargs)
    if custom_images:
...
def trailer_choice(media_type, poster, tmdb_id, trailer_url, all_trailers=[]):
    if get_language() != 'en' and not trailer_url and not all_trailers:
        ...
        try:
            all_trailers = tmdb_media_videos(media_type, tmdb_id)['results']
        except:
            pass
        hide_busy_dialog()
    if all_trailers:
        if len(all_trailers) == 1:
            ...
            def _sort_trailers():
                official_trailers = [i for i in all_trailers if i['type']
                                     == 'Trailer' and i['name'].lower() == 'official trailer']
                other_official_trailers = [i for i in all_trailers if i['type'] ==
                                           'Trailer' and 'official' in i['name'].lower() and not i in official_trailers]
                other_trailers = [i for i in all_trailers if i['type'] ==
                                  'Trailer' and not i in official_trailers and not i in other_official_trailers]
                teaser_trailers = [
                    i for i in all_trailers if i['type'] == 'Teaser']
                full_trailers = official_trailers + \
                    other_official_trailers + other_trailers + teaser_trailers
                features = [i for i in all_trailers if not i in full_trailers]
                return full_trailers + features
            sorted_trailers = _sort_trailers()
            list_items = [{'line1': clean_file_name(
                i['name']), 'icon': poster} for i in sorted_trailers]
            kwargs = {'items': json.dumps(list_items)}
            video_id = select_dialog([i['key']
...
def random_choice(params):
    meta, poster, return_choice = params.get('meta'), params.get(
        'poster'), params.get('return_choice', 'false')
    meta = params.get('meta', None)
    list_items = [{'line1': ls(32541), 'icon': poster}, {
        'line1': ls(32542), 'icon': poster}]
...
def trakt_manager_choice(params):
    ...
    from apis import trakt_api
    if choice == 'Add':
        trakt_api.trakt_add_to_list(params)
    else:
        trakt_api.trakt_remove_from_list(params)
...
def playback_choice(params):
    ...
    if aliases:
        items += [{'line': ls(32212), 'function': 'scrape_with_aliases'}]
    ...
    def clear_caches():
        ...
        ExternalProvidersCache().delete_cache_single(
            media_type, str(meta['tmdb_id']))
        hide_busy_dialog()
    if choice == 'clear_and_rescrape':
        clear_caches()
        if media_type == 'movie':
            play_params = {'mode': 'playback.media', 'media_type': 'movie',
                           'tmdb_id': meta['tmdb_id'], 'autoplay': 'false'}
        else:
            play_params = {'mode': 'playback.media', 'media_type': 'episode',
                           'tmdb_id': meta['tmdb_id'], 'season': season, 'episode': episode, 'autoplay': 'false'}
    elif choice == 'scrape_with_default':
        if media_type == 'movie':
            play_params = {'mode': 'playback.media', 'media_type': 'movie', 'tmdb_id': meta['tmdb_id'],
                           'default_ext_only': 'true', 'prescrape': 'false', 'autoplay': 'false'}
        else:
            play_params = {'mode': 'playback.media', 'media_type': 'episode', 'tmdb_id': meta['tmdb_id'], 'season': season,
        ...
        if len(aliases) == 1:
            ...
            poster = meta.get('custom_poster') or meta.get(
                poster_main) or meta.get(poster_backup) or poster_empty
        ...
        if media_type in ('movie', 'movies'):
            play_params = {'mode': 'playback.media', 'media_type': 'movie',
                           'tmdb_id': meta['tmdb_id'], 'prescrape': 'false'}
        else:
            play_params = {'mode': 'playback.media', 'media_type': 'episode',
                           'tmdb_id': meta['tmdb_id'], 'season': season, 'episode': episode, 'prescrape': 'false'}
        if aliases:
            if len(aliases) == 1:
            ...
            if alias_title:
                custom_title = dialog.input(title_str, defaultt=alias_title)
            else:
        ...
        custom_year = dialog.input(
            year_str, type=numeric_input, defaultt=default_year)
        _process_params(default_year, custom_year, 'custom_year')
        if media_type == 'episode':
            custom_season = dialog.input(
                season_str, type=numeric_input, defaultt=season)
            _process_params(season, custom_season, 'custom_season')
            custom_episode = dialog.input(
                episode_str, type=numeric_input, defaultt=episode)
            ...
            if any(i in play_params for i in ('custom_season', 'custom_episode')):
                if autoplay_next_episode():
                    _process_params(
                        '', 'true', 'disable_autoplay_next_episode')
        all_choice = confirm_dialog(heading=meta.get(
            'rootname', ''), text=allscrapers_str, ok_label=32824, cancel_label=32828)
        if all_choice == None:
            return notification(32736, 2500)
        if not all_choice:
            default_choice = confirm_dialog(heading=meta.get(
                'rootname', ''), text=def_scrapers_str, ok_label=32824, cancel_label=32828)
            if default_choice == None:
                return notification(32736, 2500)
            if default_choice:
                _process_params('', 'true', 'default_ext_only')
        else:
            _process_params('', 'true', 'disabled_ext_ignored')
        disable_filters_choice = confirm_dialog(heading=meta.get(
            'rootname', ''), text=disable_filters_str, ok_label=32824, cancel_label=32828)
        if disable_filters_choice == None:
            return notification(32736, 2500)
...
def set_quality_choice(params):
    ...
    kwargs = {'items': json.dumps(
        list_items), 'multi_choice': 'true', 'preselect': preselect}
...
def extras_buttons_choice(params):
    media_type, button_dict, orig_button_dict = params.get('media_type', None), params.get(
    ...
    kwargs = {'items': json.dumps(list_items), 'heading': ls(
        33121), 'narrow_window': 'true'}
    choice = select_dialog(choices, **kwargs)
    if choice == None:
        return extras_buttons_choice({'button_dict': button_dict, 'orig_button_dict': orig_button_dict})
    button_name, button_label, button_setting = choice[1:]
    choices = [(v, k) for k, v in extras_button_label_values[media_type].items(
    ) if not v == button_label]
    choices = [i for i in choices if not i[0] == button_label]
    list_items = [{'line1': ls(i[0])} for i in choices]
    kwargs = {'items': json.dumps(list_items), 'heading': ls(
        33122) % button_name, 'narrow_window': 'true'}
    choice = select_dialog(choices, **kwargs)
    if choice == None:
    ...
    button_dict[button_setting] = {'button_action': button_action,
                                   'button_label': button_label, 'button_name': button_name}
    return extras_buttons_choice({'button_dict': button_dict, 'orig_button_dict': orig_button_dict, 'media_type': media_type})
...
def extras_ratings_choice(params={}):
    choices = [('Metacritic', 'Meta'), ('Tomato Rating Critic', 'Tom/Critic'),
               ('Tomato Rating User', 'Tom/User'), ('IMDb', 'IMDb'), ('TMDb', 'TMDb')]
    list_items = [{'line1': i[0]} for i in choices]
    current_settings = extras_enabled_ratings()
    try:
        preselect = [choices.index(i)
                     for i in choices if i[1] in current_settings]
    except:
        preselect = []
    kwargs = {'items': json.dumps(list_items), 'heading': ls(
        33135), 'multi_choice': 'true', 'preselect': preselect}
...
def extras_lists_choice(params={}):
    ...
    try:
        ...
        preselect = []
    kwargs = {'items': json.dumps(list_items), 'heading': ls(
        32046), 'multi_choice': 'true', 'preselect': preselect}
    selection = select_dialog(choices, **kwargs)
    if selection == []:
...
def set_language_filter_choice(params):
    ...
    kwargs = {'items': json.dumps(
        list_items), 'multi_choice': multi_choice, 'preselect': preselect}
    choice = select_dialog(fl, **kwargs)
    if choice == None:
        return
    if multi_choice == 'true':
        ...
        set_setting(filter_setting, choice)


def easynews_use_custom_farm_choice(params={}):
    from apis.easynews_api import clear_media_results_database
    new_setting = 'True' if get_setting(
        'fenda.easynews.use_custom_farm', 'False') == 'False' else 'False'
...
def enable_scrapers_choice(params={}):
    icon = params.get('icon', None) or get_icon('fenda')
    scrapers = ['external', 'furk', 'easynews',
                'rd_cloud', 'pm_cloud', 'ad_cloud', 'folders']
    cloud_scrapers = {'rd_cloud': 'rd.enabled',
                      'pm_cloud': 'pm.enabled', 'ad_cloud': 'ad.enabled'}
...
def folder_scraper_manager_choice(params):
    def _set_settings(refresh=True):
        ...
        if refresh:
            kodi_refresh()
    setting_id, media_type, folder_path = params['setting_id'], params['media_type'], params.get(
        'folder_path', '')
    display_name, default_name = params.get(
        'display_name', ''), params['default_name']
    ...
    if folder_path:
        confirm = confirm_dialog(heading=display_name, text=ls(
            32529) % default_name, ok_label=32531, cancel_label=32530)
        if confirm is None:
            return
        elif confirm:
            ...
            if not confirm_dialog(heading=display_name, text=32640, default_control=10):
                set_folder_path = False
    if set_folder_path:
        folder_path = dialog.browse(0, '', '')
        if not folder_path:
            if confirm_dialog(heading=display_name, text=32704):
                return folder_scraper_manager_choice(params)
            else:
                return
    display_name = dialog.input(ls(32115), defaultt=display_name or setting_id)
    if not display_name:
        if confirm_dialog(heading=display_name, text=32714):
            return folder_scraper_manager_choice(params)
        else:
            return
    _set_settings()
...
def set_subtitle_choice():
    ...
    choice = select_dialog([i[1] for i in choices], **kwargs)
    if choice:
        return set_setting('subtitles.subs_action', choice)
...
def favorites_choice(params):
    media_type, tmdb_id, title = params.get(
        'media_type'), params.get('tmdb_id'), params.get('title')
    from caches.favorites_cache import favorites
...
def highlight_color_choice(params={}):
    current_setting = get_setting('fenda.main_highlight')
    chosen_color = color_choice({'current_setting': current_setting})
    if chosen_color:
        manage_settings_reset()
...
def options_menu_choice(params, meta=None):
    ...
    def _builder():
        for item in listing:
            yield {'line1': item[0], 'line2': item[1] or item[0], 'icon': poster}
    params_get = params.get
    tmdb_id, content, poster, season_poster = params_get('tmdb_id', None), params_get(
        'content', None), params_get('poster', None), params_get('season_poster', None)
    is_external, from_extras = params_get('is_external') in (
        True, 'True', 'true'), params_get('from_extras', 'false') == 'true'
    season, episode, in_progress_menu = params_get('season', ''), params_get(
        'episode', ''), params_get('in_progress_menu', 'false') == 'true'
    if not content:
        content = container_content()[:-1]
    menu_type = content
    if content.startswith('episode.'):
        content = 'episode'
    if not meta:
        function = metadata.movie_meta if content == 'movie' else metadata.tvshow_meta
        meta = function('tmdb_id', tmdb_id,
                        metadata_user_info(), get_datetime())
    meta_get = meta.get
    rootname, title, year, imdb_id, tvdb_id = meta_get('rootname', None), meta_get(
        'title'), meta_get('year'), meta_get('imdb_id', None), meta_get('tvdb_id', None)
    ...
    if custom_context_menu and not from_extras:
        try:
            playcount = int(params_get('playcount', '0'))
        except:
            playcount = 0
        try:
            progress = int(params_get('progress', '0'))
        except:
            progress = 0
        if menu_type in ('movie', 'tvshow'):
            ...
            if menu_type == 'movie':
                if playcount:
                    ...
                    watched_action, watchedstr = 'mark_as_watched', strip_bold(
                        ls(32642)).replace(' %s', '')
                listing_append((watchedstr, '', 'mark_movie'))
                if progress:
                    listing_append(
                        (strip_bold(ls(32651)), '', 'clear_progress'))
            else:
                if not playcount:
                    listing_append((strip_bold(ls(32642)).replace(
                        ' %s', ''), '', 'mark_watched_tvshow'))
                if progress:
                    listing_append((strip_bold(ls(32643)).replace(
                        ' %s', ''), '', 'mark_unwatched_tvshow'))
            if not is_external:
                listing_append(
                    (strip_bold(ls(32649) if menu_type == 'movie' else ls(32650)), '', 'exit_menu'))
        else:
            listing_append(
                (strip_bold(ls(32645)).replace('...', ''), '', 'extras'))
            if menu_type == 'season':
    ...
    if menu_type in ('movie', 'tvshow'):
        ...
        if menu_type == 'tvshow':
            listing_append((ls(32613), ls(32004) % rootname, 'random'))
    if menu_type in ('movie', 'episode') or menu_type in single_ep_list:
        ...
        if menu_type == 'episode' or menu_type in single_ep_list:
            if autoplay_status == on_str:
                autoplay_next_status, autoplay_next_toggle = (
                    on_str, 'false') if autoplay_next_episode() else (off_str, 'true')
                listing_append((base_str1 % (ls(32178), ''), base_str2 %
                               autoplay_next_status, 'toggle_autoplay_next'))
            else:
    ...
    if menu_type in ('movie', 'tvshow') and not from_extras:
        listing_append((ls(32604) % (ls(32028) if menu_type == 'movie' else ls(
            32029)), ls(32497) % rootname, 'clear_media_cache'))
        listing_append((ls(33043), ls(33066) % rootname, 'set_media_artwork'))
    if menu_type in ('movie', 'episode') or menu_type in single_ep_list:
        listing_append((ls(32637), '', 'clear_scrapers_cache'))
    if in_progress_menu:
    ...
    kwargs = {'items': json.dumps(
        list_items), 'heading': heading, 'multi_line': 'true'}
    ...
    if choice == 'playback':
        ...
        close_all_dialog()
        mode, action = ('build_movie_list', 'tmdb_movies_recommendations') if menu_type == 'movie' else (
...
def extras_menu_choice(params):
    ...
    function = metadata.movie_meta if media_type == 'movie' else metadata.tvshow_meta
    meta = function('tmdb_id', params['tmdb_id'],
                    metadata_user_info(), get_datetime())
    if not stacked:
        hide_busy_dialog()
