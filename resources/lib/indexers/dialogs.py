...
single_ep_list = ('episode.progress', 'episode.recently_watched', 'episode.next_trakt', 'episode.next_fenda', 'episode.trakt_recently_aired', 'episode.trakt_calendar')
		...
		for i in start_list:
			try:
				if i == 1 and widget_content: line1 = from_widget_str
					...
					if i == current_page: line1 = '[COLOR %s][B]%s   |   %s[/B][/COLOR]' % (fenda_highlight, line1, current_page_str)
				else:
...
def trakt_manager_choice(params):
	if not get_setting('fenda.trakt.user', ''): return notification(32760, 3500)
...
def set_quality_choice(params):
	...
	fl = ['SD', '720p', '1080p', '4K']
	try: preselect = [fl.index(i) for i in get_setting('fenda.%s' % quality_setting).split(', ')]
	except: preselect = []
...
def set_language_filter_choice(params):
	...
	try: preselect = [fl.index(i) for i in get_setting('fenda.%s' % filter_setting).split(', ')]
	except: preselect = []
...
def easynews_use_custom_farm_choice(params={}):
	...
	new_setting = 'True' if get_setting('fenda.easynews.use_custom_farm', 'False') == 'False' else 'False'
...
def easynews_server_choice(params={}):
	...
	kwargs = {'items': json.dumps(list_items), 'heading': easy_serv_str, 'narrow_window': 'true'}
	port_choice = select_dialog(ports, **kwargs) or get_setting('fenda.easynews.port', '443')
...
def options_menu_choice(params, meta=None):
	...
	if menu_type in ('movie', 'tvshow'):
		if get_setting('fenda.trakt.user', ''): listing_append((ls(32198), '', 'trakt_manager'))
