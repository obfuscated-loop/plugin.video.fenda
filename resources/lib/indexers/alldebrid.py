...
default_ad_icon, fanart, set_view_mode = kodi_utils.get_icon(
    'alldebrid'), kodi_utils.addon_fanart, kodi_utils.set_view_mode
    ...
    def _builder():
        for count, item in enumerate(cloud_dict, 1):
            try:
                ...
                url_params = {'mode': 'alldebrid.browse_ad_cloud',
                              'id': folder_id, 'folder': json.dumps(item['links'])}
                link_folders_add = {'mode': 'link_folders_choice',
                                    'service': 'AD', 'folder_id': folder_id, 'action': 'add'}
                ...
                listitem.setArt({'icon': default_ad_icon, 'poster': default_ad_icon,
                                'thumb': default_ad_icon, 'fanart': fanart, 'banner': default_ad_icon})
                info_tag = listitem.getVideoInfoTag()
                info_tag.setMediaType('video')
                info_tag.setPlot(' ')
                listitem.setProperty('fenda.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': clean_folder_name, 'iconImage': default_ad_icon,
                                                                                  'service': 'AD'}))
                yield (url, listitem, True)
            except:
                pass
    try:
        cloud_dict = [i for i in AllDebrid.user_cloud()['magnets']
                      if i['statusCode'] == 4]
    except:
        cloud_dict = []
    if cloud_dict:
        assigned_content = gather_assigned_content("'Fenda_AD_%'")
    ...
    def _builder():
        for count, item in enumerate(links, 1):
            try:
                ...
                url_params = {'mode': 'alldebrid.resolve_ad',
                ...
                info_tag.setPlot(' ')
                info_tag.setMediaType('video')
                listitem.setProperty('fenda.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': default_ad_icon,
                                                                                  'action': 'cloud.alldebrid'}))
                yield (url, listitem, False)
            except:
                pass
    try:
        links = [i for i in json.loads(
            folder) if i['filename'].lower().endswith(tuple(extensions))]
    except:
        links = []
    handle = int(sys.argv[1])
    add_items(handle, list(_builder()))
