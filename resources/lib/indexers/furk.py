...
def search_furk(params):
    ...
    try:
        ...
        files = Furk.direct_search(
            query) if search_method == 'direct_search' else Furk.search(query)
        files = [i for i in files if int(
            i.get('files_num_video', '0')) > 0 and 'url_dl' in i]
    ...
    def _builder():
        for count, item in enumerate(files, 1):
            try:
                ...
                cm = []
                cm_append = cm.append
                name, size, info_hash = clean_file_name(
                    item_get('name')).upper(), item_get('size'), item_get('info_hash')
                item_id, url_dl, files_num_video, is_protected = item_get('id'), item_get(
                    'url_dl'), item_get('files_num_video'), item_get('is_protected')
                try:
                    thumb = [i for i in item_get('ss_urls', []) if i][0]
                except:
                    thumb = furk_icon
                display_size = str(round(float(size)/1048576000, 1))
                duration = int(int(item['duration'])/60.0)
                header, footer = (
                    '[COLOR=green]', '[/COLOR]') if is_protected == '1' else ('', '')
                display = '%02d %s| [B]%sGB | %s %s | %sMINS | %s[/B][I]%s[/I]' % (
                    count, header, display_size, files_num_video, files_str, duration, footer, name)
                download_archive = build_url(
                    {'mode': 'downloader', 'name': name, 'url': url_dl, 'action': 'archive', 'image': furk_icon})
                add_to_files = build_url(
                    {'mode': 'furk.add_to_files', 'item_id': item_id})
                remove_files = build_url(
                ...
                if is_protected == '0':
                    ...
                    cm_append((unprot_str, 'RunPlugin(%s)' % build_url(
                        {'mode': 'furk.myfiles_protect_unprotect', 'action': 'unprotect', 'name': name, 'item_id': item_id})))
                listitem = make_listitem()
                listitem.setLabel(display)
                listitem.addContextMenuItems(cm)
                listitem.setArt({'icon': thumb, 'poster': thumb,
                                'thumb': thumb, 'fanart': fanart, 'banner': furk_icon})
                info_tag = listitem.getVideoInfoTag()
                info_tag.setMediaType('video')
                info_tag.setPlot(' ')
                listitem.setProperty('fenda.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': furk_icon,
...
def furk_t_file_browser(params):
    def _builder():
        for count, item in enumerate(t_files, 1):
            try:
                cm = []
                url_params = {'mode': 'playback.video',
                              'url': item['url_dl'], 'obj': 'video'}
                ...
                if width > 1920:
                    ...
                    display_res = 'SD'
                display_name = '%02d | [B]%s[/B] | [B]%.2fGB | %sMINS | [/B][I]%s[/I]' % (
                    count, display_res, float(item['size'])/1048576000, int(int(item['length'])/60.0), name)
                listitem = make_listitem()
                ...
                pass
    handle = int(sys.argv[1])
    t_files = [i for i in Furk.t_files(params.get(
        'item_id')) if 'video' in i['ct'] and 'bitrate' in i]
    add_items(handle, list(_builder()))
    set_content(handle, 'files')
    end_directory(handle, False)
    set_view_mode('view.premium')
...
def add_to_files(item_id):
    if not confirm_dialog(text=32580):
        return
...
def resolve_furk(item_id, media_type, season, episode):
    ...
    url = None
    t_files = [i for i in Furk.t_files(item_id) if 'video' in i['ct'] and not any(
        x in i['name'].lower() for x in ('furk320', 'sample'))]
    if media_type == 'movie':
        try:
            url = [i['url_dl'] for i in t_files if 'is_largest' in i][0]
        except:
            pass
    else:
        try:
            url = [i['url_dl'] for i in t_files if seas_ep_filter(
                season, episode, normalize(i['name']))][0]
        except:
            pass
    return url
...
def account_info(params):
    try:
        ...
        month_time_left = float(
            accinfo['premium']['bw_month_time_left'])/60/60/24
        try:
            total_time_left = float(accinfo['premium']['time_left'])/60/60/24
        except:
            total_time_left = ''
        try:
            renewal_date = accinfo['premium']['to_dt']
        except:
            renewal_date = ''
        try:
            is_not_last_month = accinfo['premium']['is_not_last_month']
        except:
            is_not_last_month = ''
        try:
            bw_used_month = float(
                accinfo['premium']['bw_used_month'])/1073741824
        except:
            bw_used_month = ''
        try:
            bw_limit_month = float(
                accinfo['premium']['bw_limit_month'])/1073741824
        except:
            bw_limit_month = ''
        try:
            rem_bw_limit_month = bw_limit_month - bw_used_month
        except:
            rem_bw_limit_month = ''
