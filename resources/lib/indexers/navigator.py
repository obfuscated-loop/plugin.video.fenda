...
log_utils_str, tips_use_str, views_str, updates_str, fenda_str, all_str, cache_str, clean_str = ls(32777), ls(32518), ls(32510), ls(32196), ls(32036), ls(32129), ls(32524), ls(32526)
...
class Navigator:
	...
	def tools(self):
		self.add({'mode': 'open_settings', 'isFolder': 'false'}, settings_ins % fenda_str, 'settings')
	...
	def changelog_utils(self):
		...
		mh_str = '[B]%s[/B]: %s  [I](v.%s)[/I]' % (changelog_str.upper(), fenda_str, addon().getAddonInfo('version'))
		self.add({'mode': 'show_text', 'heading': mh_str, 'file': fenda_clogpath, 'font_size': 'large', 'isFolder': 'false'}, mh_str, 'lists', False)
		...
		def _builder():
			for item in folder_info:
				for media_type in ('movie', 'tvshow'):
					...
					display_name = get_setting('fenda.%s.display_name' % setting_id)
					if action == 'browse':
		...
		def _builder():
			for i in folders:
				try:
					...
					info_tag.setMediaType('video')
					info_tag.setPlot(' ')
					listitem.setProperty('fenda.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': folder_icon}))
					if self.is_home: listitem.setProperty('fenda.external', 'true')
		...
		def _process():
			for item_position, item in enumerate(contents):
				try:
					...
					info_tag.setPlot(' ')
					listitem.addContextMenuItems(cm)
					listitem.setProperty('fenda.context_main_menu_params', menu_editor_url)
	...
	def add(self, url_params, list_name, iconImage='folder', contextmenu_edit=True, original_image=False):
		...
		if contextmenu_edit:
			...
			listitem.addContextMenuItems(cm)
			listitem.setProperty('fenda.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': list_name, 'iconImage': iconImage}))
			if self.is_home: listitem.setProperty('fenda.external', 'true')
