...
trakt_icon, fanart, add_item, set_property = get_icon(
    'trakt'), kodi_utils.addon_fanart, kodi_utils.add_item, kodi_utils.set_property
...
add2menu_str, add2folder_str, likelist_str, unlikelist_str = ls(
    32730), ls(32731), ls(32776), ls(32783)
newlist_str, deletelist_str, nextpage_str, jump2_str = ls(
    32780), ls(32781), ls(32799), ls(32964)
...
def search_trakt_lists(params):
    def _builder():
        for item in lists:
            try:
                ...
                if list_key == 'officiallist':
                    continue
                item_count = list_info['item_count']
                if list_info['privacy'] in ('private', 'friends') or item_count == 0:
                    continue
                ...
                cm_append((add2menu_str, 'RunPlugin(%s)' % build_url(
                    {'mode': 'menu_editor.add_external', 'name': editor_display, 'iconImage': 'trakt'})))
                cm_append((add2folder_str, 'RunPlugin(%s)' % build_url(
                    {'mode': 'menu_editor.shortcut_folder_add_item', 'name': editor_display, 'iconImage': 'trakt'})))
                cm_append((likelist_str, 'RunPlugin(%s)' % build_url(
                    {'mode': 'trakt.trakt_like_a_list', 'user': user, 'list_slug': slug})))
                cm_append((unlikelist_str, 'RunPlugin(%s)' % build_url(
                    {'mode': 'trakt.trakt_unlike_a_list', 'user': user, 'list_slug': slug})))
                listitem = make_listitem()
                listitem.setLabel(display)
                listitem.setArt({'icon': trakt_icon, 'poster': trakt_icon,
                                'thumb': trakt_icon, 'fanart': fanart, 'banner': trakt_icon})
                info_tag = listitem.getVideoInfoTag()
                info_tag.setMediaType('video')
                info_tag.setPlot(' ')
                listitem.addContextMenuItems(cm)
                listitem.setProperty('fenda.context_main_menu_params', build_url(
                    {'mode': 'menu_editor.edit_menu_external', 'name': editor_display, 'iconImage': 'trakt'}))
    ...
    try:
        ...
        if pages > page:
            ...
            add_dir({'mode': mode, 'query': search_title, 'new_page': new_page},
                    nextpage_str % new_page, handle, 'item_next')
    except:
        pass
    ...
    def _process():
        for item in lists:
            try:
                ...
                cm = []
                cm_append = cm.append
                list_name, user, slug, item_count = item['name'], item['user'][
                    'ids']['slug'], item['ids']['slug'], item['item_count']
                list_name_upper = list_name.upper()
                url = build_url({'mode': 'trakt.list.build_trakt_list', 'user': user,
...
def get_trakt_trending_popular_lists(params):
    def _process():
        for _list in lists:
            try:
                ...
                if item['user']['private'] or item_count == 0:
                    continue
    ...
    try:
        ...
        add_items(handle, list(_process()))
        add_dir({'mode': 'trakt.list.get_trakt_trending_popular_lists', 'list_type': 'trending',
                'new_page': new_page}, nextpage_str % new_page, handle, 'item_next')
    except:
        pass
    set_content(handle, 'files')
    ...
    def _paginate_list(data, page_no, paginate_start):
        if paginate(is_home):
            ...
            data, all_pages, total_pages = paginate_list(
                data, page_no, limit, paginate_start)
            if is_home:
                paginate_start = limit
        else:
            all_pages, total_pages = '', 1
        return data, all_pages, total_pages, paginate_start
    handle, is_external, is_home, content, list_name = int(
        sys.argv[1]), external(), home(), 'movies', params.get('list_name')
    try:
        threads, item_list = [], []
        item_list_extend = item_list.extend
        user, slug, list_type = params.get('user'), params.get(
            'slug'), params.get('list_type')
        page_no, paginate_start = int(params.get('new_page', '1')), int(
            params.get('paginate_start', '0'))
        ...
        process_list, all_pages, total_pages, paginate_start = _paginate_list(
        ...
        movie_list = {'list': [(i['order'], i['media_ids']) for i in process_list if i['type']
                               == 'movie'], 'id_type': 'trakt_dict', 'custom_order': 'true'}
        tvshow_list = {'list': [(i['order'], i['media_ids']) for i in process_list if i['type']
                                == 'show'], 'id_type': 'trakt_dict', 'custom_order': 'true'}
        content = 'movies' if len(movie_list['list']) > len(
            tvshow_list['list']) else 'tvshows'
...
def build_trakt_movie_sets(params):
    ...
    try:
        add_items(handle, Movies({'list': [i['id'] for i in sort_for_article(
            trakt_fetch_movie_sets(), 'title', ignore_articles())]}).movie_sets_worker())
    except:
        pass
    set_content(handle, content)
    set_category(handle, params.get('category_name', ''))
