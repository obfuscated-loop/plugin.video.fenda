...
fenda_str, trakt_str, watched_str, unwatched_str, exit_str, nextpage_str, browse_str, jump2_str = ls(32036), ls(32037), ls(32642), ls(32643), ls(32650), ls(32799), ls(33137), ls(32964)
...
class TVShows:
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external:
				...
				if not any([x in folderpath for x in internal_nav_check]): set_property('fenda.exit_params', folderpath)
	...
	def build_tvshow_content(self, _position, _id):
		try:
			...
			set_properties({'fenda.extras_params': extras_params, 'fenda.options_params': options_params})
