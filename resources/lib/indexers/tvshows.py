...
trakt_main = ('trakt_tv_trending', 'trakt_tv_trending_recent',
              'trakt_recommendations', 'trakt_tv_most_watched')
trakt_personal = ('trakt_collection', 'trakt_watchlist',
                  'trakt_collection_lists')
imdb_all = ('imdb_watchlist', 'imdb_user_list_contents',
            'imdb_keywords_list_contents', 'imdb_featured', 'imdb_most_voted')
...
class TVShows:
    def __init__(self, params):
        ...
        self.category_name = self.params_get(
            'category_name', None) or self.params_get('name', None) or 32029
    ...
    def fetch_list(self):
        ...
        try:
            ...
            if page_no == 1 and not self.is_external:
                ...
                if not any([x in folderpath for x in internal_nav_check]):
                    set_property('fenda.exit_params', folderpath)
            ...
            if self.action in tmdb_main:
                ...
                self.list = [i['show']['ids'] for i in data]
                self.new_page = {'new_page': string(
                    page_no + 1), 'certification': self.params['certification']}
    ...
    def build_tvshow_content(self, _position, _id):
        try:
            ...
            if not meta or 'blank_entry' in meta:
                return
            meta_get = meta.get
            tmdb_id, total_seasons, total_aired_eps = meta_get(
                'tmdb_id'), meta_get('total_seasons'), meta_get('total_aired_eps')
            playcount, overlay, total_watched, total_unwatched = get_watched_function(
                self.watched_info, string(tmdb_id), total_aired_eps)
            try:
            ...
            rootname, trailer, title, year = meta_get('rootname'), meta_get(
                'trailer'), meta_get('title'), meta_get('year') or '2050'
            ...
            if self.fanart_enabled:
                ...
                landscape = meta_get('custom_landscape') or meta_get(
                    'landscape') or ''
            else:
            ...
            extras_params = build_url({'mode': 'extras_menu_choice', 'tmdb_id': tmdb_id,
                                      'media_type': 'tvshow', 'is_external': self.is_external})
            ...
            if progress:
                ...
                set_properties({'watchedepisodes': string(
                    total_watched), 'unwatchedepisodes': string(total_unwatched)})
            set_properties({'watchedprogress': string(progress), 'totalepisodes': string(
                total_aired_eps), 'totalseasons': string(total_seasons)})
            if self.is_external:
            ...
            set_properties({'fenda.extras_params': extras_params,
                           'fenda.options_params': options_params})
    ...
    def worker(self):
        ...
        ), watched_indicators(), include_year_in_title('tvshow')
        self.watched_info, self.all_episodes, self.open_extras = get_watched_info_function(
            self.watched_indicators), default_all_episodes(), extras_open_action('tvshow')
        ...
        self.is_folder, self.watched_title = False if self.open_extras else True, trakt_str if self.watched_indicators == 1 else fenda_str
        ...
        if self.custom_order:
            threads = list(make_thread_list_multi_arg(
                self.build_tvshow_content, self.list, self.max_threads))
