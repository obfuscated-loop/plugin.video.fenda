...
default_imdb_icon, fanart = kodi_utils.get_icon(
    'imdb'), kodi_utils.addon_fanart
    ...
    def _builder():
        for item in user_lists:
            try:
                ...
                title = item['title']
                url = build_url(
                    {'mode': mode, 'action': 'imdb_user_list_contents', 'list_id': item['list_id']})
                listitem = make_listitem()
                cm_append((ls(32730), 'RunPlugin(%s)' % build_url(
                    {'mode': 'menu_editor.add_external', 'name': title, 'iconImage': 'imdb'})))
                cm_append((ls(32731), 'RunPlugin(%s)' % build_url(
                    {'mode': 'menu_editor.shortcut_folder_add_item', 'name': title, 'iconImage': 'imdb'})))
                listitem.addContextMenuItems(cm)
                listitem.setLabel(title)
                listitem.setArt({'icon': default_imdb_icon, 'poster': default_imdb_icon,
                                'thumb': default_imdb_icon, 'fanart': fanart, 'banner': default_imdb_icon})
    ...
    def _builder():
        for count, keyword in enumerate(results, 1):
            try:
                ...
                listitem = make_listitem()
                cm_append((ls(32730), 'RunPlugin(%s)' % build_url(
                    {'mode': 'menu_editor.add_external', 'name': name, 'iconImage': 'imdb'})))
                cm_append((ls(32731), 'RunPlugin(%s)' % build_url(
                    {'mode': 'menu_editor.shortcut_folder_add_item', 'name': name, 'iconImage': 'imdb'})))
                listitem.addContextMenuItems(cm)
                listitem.setProperty('fenda.context_main_menu_params', build_url(
                    {'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': 'imdb'}))
