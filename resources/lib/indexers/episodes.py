...
def build_episode_list(params):
    def _process():
        for item in episodes_data:
            try:
                ...
                cm_append = cm.append
                item_get = item.get
                season, episode, ep_name = item_get(
                    'season'), item_get('episode'), item_get('title')
                episode_date, premiered = adjust_premiered_date_function(
                    item_get('premiered'), adjust_hours)
                playcount, overlay = get_watched_status(
                    watched_info, string(tmdb_id), season, episode)
                ...
                extras_params = build_url(
                    {'mode': 'extras_menu_choice', 'tmdb_id': tmdb_id, 'media_type': 'episode', 'is_external': is_external})
                play_options_params = build_url({'mode': 'playback_choice', 'media_type': 'episode',
                                                'poster': show_poster, 'meta': tmdb_id, 'season': season, 'episode': episode})
                url_params = build_url({'mode': 'playback.media', 'media_type': 'episode',
                ...
                if not unaired:
                    ...
                    if progress:
                        ...
                        cm_append((clearprog_str, run_plugin %
                                  clearprog_params))
                if is_external:
                    cm_append((refr_widg_str, run_plugin %
                              build_url({'mode': 'kodi_refresh'})))
                    set_properties({'fenda.external': 'true'})
                ...
                info_tag.setUniqueIDs(
                    {'imdb': imdb_id, 'tmdb': string(tmdb_id), 'tvdb': string(tvdb_id)})
                ...
                info_tag.setCast([xbmc_actor(name=item['name'], role=item['role'],
                                 thumbnail=item['thumbnail']) for item in cast + item_get('guest_stars', [])])
                ...
                pass
    handle, is_external, category_name = int(
        sys.argv[1]), external(), episodes_str
    item_list = []
    append = item_list.append
    meta_user_info, watched_indicators, show_unaired, adjust_hours = metadata_user_info(
    ), watched_indicators_info(), show_unaired_info(), date_offset_info()
    current_date, watched_info, bookmarks = get_datetime_function(), get_watched_info(
        watched_indicators), get_bookmarks(watched_indicators, 'episode')
    watched_title = trakt_str if watched_indicators == 1 else fenda_str
    fanart_enabled, hide_watched = meta_user_info[
        'extra_fanart_enabled'], is_external and meta_user_info['widget_hide_watched']
    meta = tv_meta_function('tmdb_id', params.get(
        'tmdb_id'), meta_user_info, current_date)
    meta_get = meta.get
    tmdb_id, tvdb_id, imdb_id, tvshow_plot, orig_title = meta_get('tmdb_id'), meta_get(
        'tvdb_id'), meta_get('imdb_id'), meta_get('plot'), meta_get('original_title')
    title, show_year, rootname, show_duration, show_status = meta_get('title'), meta_get(
        'year') or '2050', meta_get('rootname'), meta_get('duration'), meta_get('status')
    cast, mpaa, trailer, genre, studio, country = meta_get('cast', []), meta_get('mpaa'), string(
        meta_get('trailer')), meta_get('genre'), meta_get('studio'), meta_get('country')
    ...
    show_poster = meta_get('custom_poster') or meta_get(
        poster_main) or meta_get(poster_backup) or poster_empty
    show_fanart = meta_get('custom_fanart') or meta_get(
        fanart_main) or meta_get(fanart_backup) or fanart_empty
    show_clearlogo = meta_get('custom_clearlogo') or meta_get(
        clearlogo_main) or meta_get(clearlogo_backup) or ''
    if fanart_enabled:
        season_art = meta_get('season_art', {})
        show_banner = meta_get('custom_banner') or meta_get('banner') or ''
        show_clearart = meta_get(
            'custom_clearart') or meta_get('clearart') or ''
        show_landscape = meta_get(
            'custom_landscape') or meta_get('landscape') or ''
    else:
        show_banner, show_clearart, show_landscape = '', '', ''
    if season == 'all':
        episodes_data = all_episodes_meta_function(
            meta, meta_user_info, show_specials())
        season_poster = show_poster
    else:
        episodes_data = episodes_meta_function(season, meta, meta_user_info)
        try:
            poster_path = [i['poster_path'] for i in meta_get(
                'season_data') if i['season_number'] == int(season)][0]
            tmdb_poster = '%s%s%s' % (
                tmdb_poster_prefix, meta_user_info['image_resolution']['poster'], poster_path) if poster_path is not None else show_poster
        except:
...
def build_single_episode(list_type, params={}):
    def _get_category_name():
        ...
        if isinstance(cat_name, dict):
            cat_name = cat_name[params.get('recently_aired')]
        return cat_name

    def _process(_position, ep_data):
        try:
            ep_data_get = ep_data.get
            meta = tv_meta_function('trakt_dict', ep_data_get(
                'media_ids'), meta_user_info, current_date)
            if not meta:
                return
            meta_get = meta.get
            cm = []
            ...
            listitem = make_listitem()
            set_properties = listitem.setProperties
            orig_season, orig_episode = ep_data_get(
                'season'), ep_data_get('episode')
            unwatched = ep_data_get('unwatched', False)
            season_data = meta_get('season_data')
            if list_type_starts_with('next_'):
                if orig_episode == 0:
                    orig_episode, new_season = 1, False
                else:
                    try:
                        episode_count = [
                            i for i in season_data if i['season_number'] == orig_season][0]['episode_count']
                        if orig_episode >= episode_count:
                            orig_season, orig_episode, new_season = orig_season + 1, 1, True
                            if orig_season > meta_get('total_seasons'):
                                return
                        else:
                            orig_episode, new_season = orig_episode + 1, False
                    except:
                        return
            episodes_data = episodes_meta_function(
                orig_season, meta, meta_user_info)
            try:
            ...
            if not episode_date or current_date < episode_date:
                if list_type_starts_with('next_'):
                    if not episode_date:
                        return
                    if not nextep_include_unaired:
                        return
                    if episode_date and new_season and not date_difference_function(current_date, episode_date, 7):
                        return
                elif not show_unaired:
                    return
                unaired = True
            else:
                unaired = False
            tmdb_id, tvdb_id, imdb_id, title, show_year = meta_get('tmdb_id'), meta_get(
                'tvdb_id'), meta_get('imdb_id'), meta_get('title'), meta_get('year') or '2050'
            orig_title, rootname, trailer, genre, studio = meta_get('original_title'), meta_get(
                'rootname'), string(meta_get('trailer')), meta_get('genre'), meta_get('studio')
            cast, mpaa, tvshow_plot, show_status = meta_get('cast', []), meta_get(
                'mpaa'), meta_get('plot'), meta_get('status')
            show_poster = meta_get('custom_poster') or meta_get(
                poster_main) or meta_get(poster_backup) or poster_empty
            show_fanart = meta_get('custom_fanart') or meta_get(
                fanart_main) or meta_get(fanart_backup) or fanart_empty
            ...
            try:
                poster_path = [i['poster_path']
                               for i in season_data if i['season_number'] == int(season)][0]
                tmdb_poster = '%s%s%s' % (
                    tmdb_poster_prefix, meta_user_info['image_resolution']['poster'], poster_path) if poster_path is not None else show_poster
            except:
                tmdb_poster = show_poster
            if fanart_enabled:
                season_art = meta_get('season_art', {})
                show_banner = meta_get(
                    'custom_banner') or meta_get('banner') or ''
                show_clearart = meta_get(
                    'custom_clearart') or meta_get('clearart') or ''
                show_landscape = meta_get(
                    'custom_landscape') or meta_get('landscape') or ''
            ...
            (playcount, overlay), progress = get_watched_status(watched_info, string(
                tmdb_id), season, episode), get_progress_percent(bookmarks, tmdb_id, season, episode)
    ...
    handle, is_external, category_name = int(
        sys.argv[1]), external(), episodes_str
    item_list, unwatched = [], []
    resinsert, fenda_highlight = '', get_property('fenda.main_highlight')
    item_list_append = item_list.append
    display_title, date_format, art_keys, all_episodes = single_ep_display_title(
    ), single_ep_format(), get_art_provider(), default_all_episodes()
    meta_user_info, watched_indicators, show_unaired = metadata_user_info(
    ), watched_indicators_info(), show_unaired_info()
    current_date, adjust_hours, ignore_articles_setting = get_datetime_function(
    ), date_offset_info(), ignore_articles()
    watched_info, bookmarks, watched_title = get_watched_info(watched_indicators), get_bookmarks(
        watched_indicators, 'episode'), trakt_str if watched_indicators == 1 else fenda_str
    fanart_enabled, hide_watched, show_all_episodes = meta_user_info[
        'extra_fanart_enabled'], is_external and meta_user_info['widget_hide_watched'], all_episodes in (1, 2)
    ...
    if list_type == 'episode.next':
        ...
        nextep_unaired_color, nextep_unwatched_color = nextep_disp_settings[
            'unaired_color'], nextep_disp_settings['unwatched_color']
        nextep_include_airdate, nextep_include_unaired = nextep_disp_settings[
            'include_airdate'], nextep_settings['include_unaired']
    ...
    elif list_type == 'episode.recently_watched':
        data = get_recently_watched('episode')
    else:  # episode.trakt
        ...
        data = sorted(data, key=lambda i: (i['sort_title'], i.get(
            'first_aired', '2100-12-31')), reverse=True)
    ...
    if list_type_starts_with('next_'):
        def func(function):
            if sort_key == 'name':
                return title_key_function(function, ignore_articles_setting)
            elif sort_key == 'last_played':
                return jsondate_to_datetime_function(function, resformat)
            else:
                return function
        ...
        if nextep_settings['sort_airing_today_to_top']:
            ...
            airing_today = sorted(airing_today, key=lambda i: i.get(
                'first_aired', '2100-12-31'))
            remainder = [i for i in item_list if not i in airing_today]
            remainder = sorted(remainder, key=lambda i: func(
                i[sort_key]), reverse=sort_direction)
            ...
            item_list = sorted(item_list, key=lambda i: func(
                i[sort_key]), reverse=sort_direction)
        ...
        if list_type_compare in ('trakt_calendar', 'trakt_recently_aired'):
            if list_type_compare == 'trakt_calendar':
                ...
                reverse = True
            item_list = sorted(item_list, key=lambda i: i.get(
                'first_aired', '2100-12-31'), reverse=reverse)
