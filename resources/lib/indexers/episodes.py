...
string, fenda_str, trakt_str, watched_str, unwatched_str, season_str, episodes_str =  str, ls(32036), ls(32037), ls(32642), ls(32643), ls(32537), ls(32506)
...
def build_single_episode(list_type, params={}):
	...
	def _process(_position, ep_data):
		try:
			...
			if list_type_starts_with('next_'):
				...
				if nextep_include_airdate: airdate = '[COLOR %s][%s][/COLOR] ' % (fenda_highlight, display_premiered)
				else: airdate = ''
				...
				if unaired:
					...
					display = '[COLOR %s]%s][/COLOR]%s' % (fenda_highlight, displays[0], displays[1])
			else:
				color_tags = ('[COLOR %s]' % fenda_highlight, '[/COLOR]') if unaired else ('', '')
	...
	watched_info, bookmarks, watched_title = get_watched_info(watched_indicators), get_bookmarks(watched_indicators, 'episode'), trakt_str if watched_indicators == 1 else fenda_str
