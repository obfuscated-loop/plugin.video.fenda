...
default_icon, fanart, default_poster, default_cast = get_icon(
    'discover'), kodi_utils.addon_fanart, 'box_office', 'genre_family'
...
inc_str, ex_str, heading_base = '%s %s' % (ls(32188), '%s'), '%s %s' % (
    ls(32189), '%s'), '%s - %s' % (ls(32451), '%s')
add_menu_str, add_folder_str, remove_str, clear_str = ls(
    32730), ls(32731), ls(32698), ls(32699)
base_str, window_prop = '[B]%s:[/B]  [I]%s[/I]', 'fenda.%s_discover_params'
...
class Discover:
    def __init__(self, params):
        ...
        if self.media_type:
            self.window_prop = window_prop % self.media_type.upper()
        else:
            self.window_prop = ''
        try:
            self.discover_params = json.loads(get_property(self.window_prop))
        except:
    ...
    def movie(self):
        ...
        if not 'recommended' in self.value_names:
            ...
            self.add({'mode': 'discover.genres', 'media_type': 'movie', 'key': 'without_genres',
                     'list_name': base_str % (ex_str % ls(32470), self.gv('without_genres'))})
            self.add({'mode': 'discover.keywords', 'media_type': 'movie', 'key': 'with_keywords',
                     'list_name': base_str % (inc_str % ls(32657), self.gv('with_keywords'))})
            self.add({'mode': 'discover.keywords', 'media_type': 'movie', 'key': 'without_keywords',
                     'list_name': base_str % (ex_str % ls(32657), self.gv('without_keywords'))})
            self.add({'mode': 'discover.language', 'media_type': 'movie',
                     'list_name': base_str % (ls(32658), self.gv('language'))})
            self.add({'mode': 'discover.region', 'media_type': 'movie',
                     'list_name': base_str % (ls(32659), self.gv('region'))})
            self.add({'mode': 'discover.companies', 'media_type': 'movie',
                     'list_name': base_str % (ls(32660), self.gv('companies'))})
            ...
            self.add({'mode': 'discover.adult', 'media_type': 'movie',
                     'list_name': base_str % (inc_str % ls(32665), self.gv('adult'))})
        self._add_defaults()
        self._end_directory()
    ...
    def tvshow(self):
        ...
        if not 'recommended' in self.value_names:
            ...
            self.add({'mode': 'discover.genres', 'media_type': 'tvshow', 'key': 'without_genres',
                     'list_name': base_str % (ex_str % ls(32470), self.gv('without_genres'))})
            self.add({'mode': 'discover.keywords', 'media_type': 'tvshow', 'key': 'with_keywords',
                     'list_name': base_str % (inc_str % ls(32657), self.gv('with_keywords'))})
            self.add({'mode': 'discover.keywords', 'media_type': 'tvshow', 'key': 'without_keywords',
                     'list_name': base_str % (ex_str % ls(32657), self.gv('without_keywords'))})
            self.add({'mode': 'discover.language', 'media_type': 'tvshow',
                     'list_name': base_str % (ls(32658), self.gv('language'))})
        ...
        for item in results:
            ...
            try:
                year = item['release_date'].split(
                    '-')[0] if self.media_type == 'movie' else item['first_air_date'].split('-')[0]
            except:
                year = ''
            if year:
    ...
    def keywords(self):
        if self._action(self.key) in ('clear', None):
            return
        current_key_ids = self.discover_params['value_strings'].get(
            self.key, [])
        current_keywords = self.discover_params['value_names'].get(
            self.key, [])
        ...
        if keyword:
            try:
                ...
                keywords_choice = self._multiselect_dialog(heading_base % ('%s %s' % (
                    ls(32193), ls(32657))), [i['name'].upper() for i in result], result)
                ...
                pass
            values = ('%s%s' % (replace_value, ','.join(
                [i for i in current_key_ids])), ', '.join([i for i in current_keywords]))
    ...
    def genres(self):
        ...
        genres_choice = self._multiselect_dialog(heading_base % (heading % ls(32470)), [
                                                 i[0] for i in genre_list], genre_list)
    ...
    def region(self):
        ...
        region = self._selection_dialog(
            region_names, region_codes, heading_base % ls(32659))
        if region != None:
            region_name = [i['name']
    ...
    def cast(self):
        ...
        if len(result) > 1:
            for item in result:
                name = item['name']
                known_for_list = [i.get('title', 'NA')
                                  for i in item['known_for']]
                ...
                append({'line1': name, 'line2': known_for,
                       'icon': icon, 'name': name, 'id': item['id']})
            heading = heading_base % ls(32664)
            kwargs = {'items': json.dumps(actor_list), 'heading': heading,
                      'enumerate': 'false', 'multi_choice': 'false', 'multi_line': 'true'}
    ...
    def network(self):
        ...
        for item in networks:
            name = item['name']
            append(
                {'line1': name, 'icon': item['logo'], 'name': name, 'id': item['id']})
        heading = heading_base % ls(32480)
        kwargs = {'items': json.dumps(network_list), 'heading': heading,
                  'enumerate': 'false', 'multi_choice': 'false', 'multi_line': 'false'}
    ...
    def companies(self):
        ...
        if not isinstance(current_company_ids, list):
            current_company_ids = current_company_ids.replace(
                '&with_companies=', '').split('|')
        ...
        if company:
            ...
            try:
                ...
                if results['total_results'] == 1:
                    company_choice = results['results']
                if not company_choice:
                    results = results['results']
                    company_choice = self._multiselect_dialog(heading_base % ls(
                        32660), [i['name'].upper() for i in results], results)
                if company_choice != None:
                    for i in company_choice:
                        company_ids_append(str(i['id']))
                        company_append(i['name'].upper())
                values = ('&with_companies=%s' % '|'.join(
                    [i for i in current_company_ids]), ', '.join([i for i in current_companies]))
                self._process(key, values)
            except:
                pass
    ...
    def sort_by(self):
        ...
        sort_by_value = self._selection_dialog([i[0] for i in sort_by_list], [
                                               i[1] for i in sort_by_list], heading_base % ls(32067))
        if sort_by_value != None:
            sort_by_name = [i[0]
                            for i in sort_by_list if i[1] == sort_by_value][0]
    ...
    def export(self):
        try:
            media_type, query, name = self.discover_params[
                'media_type'], self.discover_params['final_string'], self.discover_params['name']
            ...
            menu_item_json = json.dumps(
                {'name': name, 'mode': mode, 'action': action, 'query': query, 'iconImage': 'discover'})
            if self.params['export_type'] == 'menu':
                mode = 'menu_editor.add_external'
            else:
                mode = 'menu_editor.shortcut_folder_add_item'
            run_plugin({'mode': mode, 'iconImage': 'discover',
                       'name': name, 'menu_item': menu_item_json})
        except:
            notification(32574)
        ...
        def _builder():
            for count, item in enumerate(data):
                try:
                    ...
                    remove_all_params = {
                        'mode': 'discover.remove_all_history', 'media_type': media_type, 'silent': 'true'}
                    listitem = make_listitem()
                    listitem.setLabel(display)
                    listitem.setArt({'icon': default_icon, 'poster': default_icon,
                    ...
                    cm_append((add_folder_str, 'RunPlugin(%s)' % build_url(
                        {'mode': 'menu_editor.shortcut_folder_add_item', 'name': name, 'iconImage': 'discover'})))
                    listitem.addContextMenuItems(cm)
                    listitem.setProperty('fenda.context_main_menu_params', build_url(
                        {'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': default_icon}))
    ...
    def help(self):
        text_file = translate_path(
            'special://home/addons/plugin.video.fenda/resources/text/tips/135. Discover.txt')
    ...
    def _set_default_params(self):
        if not 'media_type' in self.discover_params:
            ...
            self.discover_params['value_names'] = {
                'media_type': param_media_type, 'adult': ls(32860)}
            self._set_property()
        self.value_names = self.discover_params['value_names']

    def _add_defaults(self):
        if not 'final_string' in self.discover_params:
            return
        if self.discover_params['media_type'] == 'movie':
            mode, action = 'build_movie_list', 'tmdb_movies_discover'
        else:
        ...
        self.add({'mode': 'discover.export', 'media_type': self.media_type,
                 'export_type': 'folder', 'list_name': folder_export_str}, icon=export_icon)
        self.add({'mode': mode, 'action': action, 'query': query, 'name': name,
                 'list_name': ls(32666) % name}, isFolder=True, icon=get_icon('search'))

    def _action(self, key):
        dict_item = self.discover_params
        add_to_list = ('keyword', 'companies')
        action = ls(32602) if any(
            word in key for word in add_to_list) else ls(32668)
        if key in dict_item['value_names']:
            action = self._selection_dialog([action.capitalize(), ls(
                32671)], (action, 'clear'), heading_base % ls(32670))
        if action == None:
            return
        if action == 'clear':
            index = self._position(key)
            for k in ('value_strings', 'value_names'):
                dict_item[k].pop(key, None)
    ...
    def add(self, params, isFolder=False, icon=None):
        ...
        listitem.setArt({'icon': icon, 'poster': icon,
                        'thumb': icon, 'fanart': fanart, 'banner': icon})
        info_tag = listitem.getVideoInfoTag()
        info_tag.setMediaType('video')
        info_tag.setPlot(' ')
    ...
    def _selection_dialog(self, dialog_list, function_list, string):
        ...
        kwargs = {'items': json.dumps(
            list_items), 'heading': string, 'narrow_window': 'true'}
    ...
    def _multiselect_dialog(self, string, dialog_list, function_list=None, preselect=[]):
        if not function_list:
            function_list = dialog_list
        list_items = [{'line1': item} for item in dialog_list]
        kwargs = {'items': json.dumps(list_items), 'heading': string, 'enumerate': 'false',
                  'multi_choice': 'true', 'multi_line': 'false', 'preselect': preselect}
        return select_dialog(function_list, **kwargs)

    def _build_string(self):
        string_params = self.discover_params['value_strings']
        if 'recommended' in string_params:
            self.discover_params['final_string'] = string_params['base_recommended'] % (
                string_params['recommended'], '%s')
        else:
            self.discover_params['final_string'] = string_params['base'] + \
                ''.join([v for k, v in string_params.items() if k in url_params])
    ...
    def _movies_sort(self):
        return [
            (ls(33166), '&sort_by=popularity.asc'),            (ls(
                33167), '&sort_by=popularity.desc'),
            (ls(33168), '&sort_by=primary_release_date.asc'),  (ls(
                33169), '&sort_by=primary_release_date.desc'),
            (ls(33170), '&sort_by=revenue.asc'),               (ls(
                33171), '&sort_by=revenue.desc'),
            (ls(33172), '&sort_by=original_title.asc'),        (ls(
                33173), '&sort_by=original_title.desc'),
            (ls(33174), '&sort_by=vote_average.asc'),          (ls(
                33175), '&sort_by=vote_average.desc')
        ]
    ...
    def _tvshows_sort(self):
        return [
            (ls(33166), '&sort_by=popularity.asc'),       (ls(
                33167), '&sort_by=popularity.desc'),
            (ls(33186), '&sort_by=first_air_date.asc'),  (ls(
                33187), '&sort_by=first_air_date.desc'),
            (ls(33174), '&sort_by=vote_average.asc'),     (ls(
                33175), '&sort_by=vote_average.desc')
        ]
    ...
    def remove_all_history(self):
        ...
        for item in all_history:
            self.remove_from_history({'data_id': item, 'silent': 'true'})
        notification(32576)
