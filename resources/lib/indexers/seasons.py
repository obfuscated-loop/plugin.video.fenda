...
def build_season_list(params):
    def _process():
        ...
        for item in season_data:
            try:
                ...
                name, overview, poster_path, air_date = item_get('name'), item_get(
                    'overview'), item_get('poster_path'), item_get('air_date')
                season_number, episode_count = item_get(
                    'season_number'), item_get('episode_count')
                first_airdate, premiered = adjust_premiered_date_function(
                    air_date, adjust_hours)
                tmdb_poster = '%s%s%s' % (
                ...
                if fanart_enabled:
                    ...
                    poster, banner, thumb = tmdb_poster, show_banner, show_fanart
                if season_number == 0:
                    unaired = False
                    ...
                    if not first_airdate or current_date < first_airdate:
                        unaired = True
                    else:
                        unaired = False
                if unaired:
                ...
                title = name if use_season_title and name else season_name_str % (
                    season_str, season_number)
                if unaired:
                    title = unaired_label % title
                playcount, overlay, watched, unwatched = get_watched_status(
                    watched_info, str_tmdb_id, season_number, episode_count)
                try:
                ...
                extras_params = build_url(
                    {'mode': 'extras_menu_choice', 'tmdb_id': tmdb_id, 'media_type': 'tvshow', 'is_external': is_external})
                ...
                if progress:
                    ...
                    set_properties({'watchedepisodes': string(
                        watched), 'unwatchedepisodes': string(unwatched)})
                set_properties({'totalepisodes': string(episode_count), 'watchedprogress': string(
                    progress), 'fenda.extras_params': extras_params, 'fenda.options_params': options_params})
                if is_external:
                ...
                info_tag.setUniqueIDs(
                    {'imdb': imdb_id, 'tmdb': str_tmdb_id, 'tvdb': str_tvdb_id})
    ...
    handle, is_external, category_name = int(
        sys.argv[1]), external(), season_str
    ...
    image_resolution, hide_watched = meta_user_info['image_resolution'][
        'poster'], is_external and meta_user_info['widget_hide_watched']
    fanart_enabled = meta_user_info['extra_fanart_enabled']
    meta = tvshow_meta(
        'tmdb_id', params['tmdb_id'], meta_user_info, current_date)
    meta_get = meta.get
    tmdb_id, tvdb_id, imdb_id, show_title, show_year = meta_get('tmdb_id'), meta_get(
        'tvdb_id'), meta_get('imdb_id'), meta_get('title'), meta_get('year') or '2050'
    orig_title, status, show_plot, total_aired_eps = meta_get(
        'original_title', ''), meta_get('status'), meta_get('plot'), meta_get('total_aired_eps')
    str_tmdb_id, str_tvdb_id, rating, genre = string(tmdb_id), string(
        tvdb_id), meta_get('rating'), meta_get('genre')
    cast, mpaa, votes, trailer, studio, country = meta_get('cast', []), meta_get('mpaa'), meta_get(
        'votes'), string(meta_get('trailer')), meta_get('studio'), meta_get('country')
    episode_run_time, season_data, total_seasons = meta_get(
        'duration'), meta_get('season_data'), meta_get('total_seasons')
    ...
    if fanart_enabled:
        ...
        show_banner, show_clearart, show_landscape, season_art = '', '', '', {}
