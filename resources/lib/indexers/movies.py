...
trakt_main = ('trakt_movies_trending', 'trakt_movies_trending_recent',
              'trakt_movies_most_watched', 'trakt_movies_top10_boxoffice', 'trakt_recommendations')
trakt_personal = ('trakt_collection', 'trakt_watchlist',
                  'trakt_collection_lists')
imdb_all = ('imdb_watchlist', 'imdb_user_list_contents', 'imdb_keywords_list_contents',
            'imdb_featured', 'imdb_most_voted', 'imdb_movies_oscar_winners')
...
class Movies:
    def __init__(self, params):
        ...
        self.category_name = self.params_get(
            'category_name', None) or self.params_get('name', None) or 32028
        self.id_type, self.list, self.action = self.params_get(
            'id_type', 'tmdb_id'), self.params_get('list', []), self.params_get('action', None)
        self.items, self.new_page, self.total_pages, self.is_external, self.is_home, self.max_threads = [
        ], {}, None, external(), home(), max_threads()
    ...
    def fetch_list(self):
        ...
        try:
            ...
            if page_no == 1 and not self.is_external:
                set_property('fenda.exit_params', folder_path())
            ...
            if self.action in tmdb_main:
                ...
                if self.action == 'tmdb_movies_search_sets':
                    builder = self.movie_sets_worker
                ...
                if data['total_pages'] > page_no:
                    self.new_page = {'new_page': string(
                        data['page'] + 1), key: function_var}
            elif self.action in personal:
                data = function('movie', page_no)
                if self.action == 'recent_watched_movies':
                    all_pages, total_pages = '', 1
                else:
                    data, all_pages, total_pages = self.paginate_list(
                        data, page_no)
                ...
                if total_pages > page_no:
                    self.new_page = {'new_page': string(
                        page_no + 1), 'paginate_start': self.paginate_start}
                ...
                if self.action == 'trakt_collection_lists':
                    all_pages, total_pages = '', 1
                else:
                    data, all_pages, total_pages = self.paginate_list(
                        data, page_no)
                ...
                self.list = [i['id'] for i in data['results']]
                if data['total_pages'] > page_no:
                    self.new_page = {'query': query, 'name': name,
                                     'new_page': string(data['page'] + 1)}
            elif self.action == 'tmdb_movies_sets':
                data = sorted(movieset_meta(self.params_get('tmdb_id'), metadata_user_info())[
                              'parts'], key=lambda k: k['release_date'] or '2050')
            ...
            if self.total_pages and not self.is_external:
                ...
                if jump_to != 3:
                    url_params = json.dumps(
                        {**self.new_page, **{'mode': mode, 'action': self.action, 'category_name': self.category_name}})
            ...
            if self.new_page and not self.widget_hide_next_page:
                self.new_page.update(
                    {'mode': mode, 'action': self.action, 'category_name': self.category_name})
                add_dir(self.new_page, nextpage_str %
                        self.new_page['new_page'], handle, 'item_next')
        except:
            pass
        ...
        if not self.is_external:
            if self.params_get('refreshed') == 'true':
                sleep(1000)
    ...
    def build_movie_content(self, _position, _id):
        try:
            meta = meta_function(
                self.id_type, _id, self.meta_user_info, self.current_date, self.current_time)
            if not meta or 'blank_entry' in meta:
                return
            meta_get = meta.get
            playcount, overlay = get_watched_function(
                self.watched_info, string(meta_get('tmdb_id')))
            cm = []
            cm_append = cm.append
            listitem = make_listitem()
            set_properties = listitem.setProperties
            clearprog_params, watched_status_params = '', ''
            rootname, title, year = meta_get('rootname'), meta_get(
                'title'), meta_get('year') or '2050'
            tmdb_id, imdb_id = meta_get('tmdb_id'), meta_get('imdb_id')
            poster = meta_get('custom_poster') or meta_get(
                self.poster_main) or meta_get(self.poster_backup) or poster_empty
            fanart = meta_get('custom_fanart') or meta_get(
                self.fanart_main) or meta_get(self.fanart_backup) or fanart_empty
            clearlogo = meta_get('custom_clearlogo') or meta_get(
                self.clearlogo_main) or meta_get(self.clearlogo_backup) or ''
            if self.fanart_enabled:
                banner, clearart = meta_get('custom_banner') or meta_get(
                    'banner') or '', meta_get('custom_clearart') or meta_get('clearart') or ''
                landscape, discart = meta_get('custom_landscape') or meta_get(
                    'landscape') or '', meta_get('custom_discart') or meta_get('discart') or ''
            ...
            watched_status_params = build_url(
                {'mode': 'watched_status.mark_movie', 'action': watched_action, 'tmdb_id': tmdb_id, 'title': title, 'year': year})
            play_params = build_url(
                {'mode': 'playback.media', 'media_type': 'movie', 'tmdb_id': tmdb_id})
            extras_params = build_url({'mode': 'extras_menu_choice', 'media_type': 'movie',
                                      'tmdb_id': tmdb_id, 'is_external': self.is_external})
            play_options_params = build_url(
                {'mode': 'playback_choice', 'media_type': 'movie', 'poster': poster, 'meta': tmdb_id})
            ...
            if progress:
                clearprog_params = build_url(
                    {'mode': 'watched_status.erase_bookmark', 'media_type': 'movie', 'tmdb_id': tmdb_id, 'refresh': 'true'})
                cm_append((clearprog_str, run_plugin % clearprog_params))
            cm_append((watchedstr % self.watched_title,
                      run_plugin % watched_status_params))
            if self.is_external:
                cm_append((refr_widg_str, run_plugin %
                          build_url({'mode': 'kodi_refresh'})))
                set_properties({'fenda.external': 'true'})
            else:
                cm_append((exit_str, run_plugin % build_url(
                    {'mode': 'navigator.exit_media_menu'})))
            ...
            info_tag.setCast([xbmc_actor(name=item['name'], role=item['role'],
                             thumbnail=item['thumbnail']) for item in meta_get('cast', [])])
            ...
            set_properties({'fenda.extras_params': extras_params, 'fenda.clearprog_params': clearprog_params, 'fenda.options_params': options_params,
                            'fenda.unwatched_params': watched_status_params, 'fenda.watched_params': watched_status_params})
    ...
    def worker(self):
        ...
        self.meta_user_info, self.watched_indicators, self.include_year = metadata_user_info(
        ), watched_indicators(), include_year_in_title('movie')
        self.watched_info, self.bookmarks = get_watched_info_function(
            self.watched_indicators), get_bookmarks(self.watched_indicators, 'movie')
        self.open_extras, self.watched_title = extras_open_action(
            'movie'), trakt_str if self.watched_indicators == 1 else fenda_str
        self.fanart_enabled, self.home_hide_watched = self.meta_user_info[
            'extra_fanart_enabled'], self.is_home and self.meta_user_info['widget_hide_watched']
        ...
        if self.custom_order:
            threads = list(make_thread_list_multi_arg(
                self.build_movie_content, self.list, self.max_threads))
            [i.join() for i in threads]
        else:
            threads = list(make_thread_list_enumerate(
                self.build_movie_content, self.list, self.max_threads))
            [i.join() for i in threads]
    ...
    def build_movie_sets_content(self, _position, _id):
        try:
            ...
            if len(parts) <= 1:
                return
            if len([i for i in parts if i['release_date'] and adjust_premiered_date(i['release_date'], self.adjust_hours)[0] <= self.current_date]) <= 1:
                return
            title, plot, tmdb_id = meta_get(
                'title'), meta_get('plot'), meta_get('tmdb_id')
            poster = meta_get(self.poster_main) or meta_get(
                self.poster_backup) or poster_empty
            fanart = meta_get(self.fanart_main) or meta_get(
                self.fanart_backup) or fanart_empty
            ...
            if self.fanart_enabled:
                ...
                banner, clearart, landscape, discart, keyart = '', '', '', '', ''
            url = build_url({'mode': 'build_movie_list',
                            'action': 'tmdb_movies_sets', 'tmdb_id': tmdb_id})
            listitem = make_listitem()
            set_property = listitem.setProperty
            cm_append((addmenu_str, run_plugin % build_url(
                {'mode': 'menu_editor.add_external', 'name': title, 'iconImage': poster})))
            cm_append((addshortcut_str, run_plugin % build_url(
                {'mode': 'menu_editor.shortcut_folder_add_item', 'name': title, 'iconImage': poster})))
            cm_append((add_coll_str, run_plugin % build_url(
                {'mode': 'movie_sets_to_collection_choice', 'collection_id': tmdb_id})))
            if self.is_external:
                cm_append((refr_widg_str, run_plugin %
                          build_url({'mode': 'kodi_refresh'})))
                set_property('fenda.external', 'true')
            info_tag = listitem.getVideoInfoTag()
            info_tag.setMediaType('movie')
            ...
            set_property('fenda.context_main_menu_params', build_url(
                {'mode': 'menu_editor.edit_menu_external', 'name': title, 'iconImage': poster}))
            self.append(((url, listitem, True), _position))
        except:
            pass

    def movie_sets_worker(self):
        self.meta_user_info, self.current_date, self.current_time, self.adjust_hours = metadata_user_info(
        ), get_datetime_function(), get_current_timestamp(), date_offset()
        ...
        self.poster_main, self.poster_backup, self.fanart_main, self.fanart_backup = get_art_provider()[
            0:4]
        threads = list(make_thread_list_enumerate(
            self.build_movie_sets_content, self.list, self.max_threads))
        [i.join() for i in threads]
        self.items.sort(key=lambda k: k[1])
        self.items = [i[0] for i in self.items]
        return self.items
    ...
    def paginate_list(self, data, page_no):
        if paginate(self.is_home):
            ...
            data, all_pages, total_pages = paginate_list(
                data, page_no, limit, self.paginate_start)
            if self.is_home:
                self.paginate_start = limit
        else:
            all_pages, total_pages = '', 1
