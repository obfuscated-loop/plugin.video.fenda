...
fenda_str, trakt_str, watched_str, unwatched_str, extras_str, options_str = ls(32036), ls(32037), ls(32642), ls(32643), ls(32645), ls(32646)
...
class Movies:
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external: set_property('fenda.exit_params', folder_path())
	...
	def worker(self):
		...
		self.open_extras, self.watched_title = extras_open_action('movie'), trakt_str if self.watched_indicators == 1 else fenda_str
	...
	def build_movie_sets_content(self, _position, _id):
		try:
			...
			set_property('fenda.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': title, 'iconImage': poster}))
