...
def search_history(params):
    def _builder():
        for i in main_cache.get(setting_id):
            try:
                ...
                if '|' in query:
                    display = history_str % '%s (%s)' % tuple(query.split('|'))
                else:
                    display = history_str % query
                url = build_url(url_params)
                cm.append((remove_str, 'RunPlugin(%s)' % build_url(
                    {'mode': 'history.remove', 'setting_id': setting_id, 'query': query})))
                cm.append((remove_all_str, 'RunPlugin(%s)' % build_url(
                    {'mode': 'history.clear_all', 'setting_id': setting_id, 'refresh': 'true'})))
                ...
                listitem.setProperty('fenda.context_main_menu_params', build_url(
                    {'mode': 'menu_editor.edit_menu_external'}))
    ...
    add_dir(action_dict, new_search_str, handle,
            iconImage='search_new', isFolder=False)
    try:
