...
class Images():
    def run(self, params):
        ...
        elif self.mode == 'popular_people_image_results':
            self.popular_people_image_results()
        elif self.mode == 'browser_image':
            self.browser_image(params['folder_path'])
        elif self.mode == 'imageviewer':
    ...
    def open_window_xml(self):
        hide_busy_dialog()
        open_window(('windows.imageviewer', 'ThumbImageViewer'), 'thumbviewer.xml',
                    list_items=self.list_items, next_page_params=self.next_page_params, ImagesInstance=self)
    ...
    def popular_people_image_results(self):
        def builder():
            for item in image_info['results']:
                if item['profile_path']:
                    actor_poster, actor_image = tmdb_image_base % (
                        'w185', item['profile_path']), tmdb_image_base % ('h632', item['profile_path'])
                else:
                    actor_poster, actor_image = get_icon(
                        'genre_family'), get_icon('genre_family')
        ...
        if image_info['total_pages'] > page_no:
            page_no += 1
        else:
            page_no = 'final_page'
        self.next_page_params = {
            'mode': 'popular_people_image_results', 'page_no': page_no}
    ...
    def imdb_image_results(self):
        def builder(rolling_count):
            for item in image_info:
                try:
                    ...
                    pass
        imdb_id, page_no, rolling_count_list = self.params[
            'imdb_id'], self.params['page_no'], self.params['rolling_count_list']
        ...
        if len(image_info) == 48:
            if len(rolling_count_list) == page_no:
                rolling_count_list.insert(page_no, rolling_count)
            page_no = next_page
        else:
            page_no = 'final_page'
        self.next_page_params = {'mode': 'imdb_image_results', 'imdb_id': imdb_id,
                                 'page_no': page_no, 'rolling_count_list': rolling_count_list, 'media_title': media_title}

    def people_image_results(self):
        def get_tmdb():
            try:
                tmdb_append(tmdb_people_full_info(actor_id).get('images', []))
            except:
                pass

        def get_imdb():
            try:
                imdb_append(imdb_people_images(actor_imdb_id, page_no)[0])
            except:
                pass
        ...
        actor_imdb_id = self.params['actor_imdb_id'] or imdb_people_id(
            actor_name)
        ...
        rolling_count = rolling_count_list[page_no-1]
        if page_no == 1:
            threads.append(Thread(target=get_tmdb))
        ...
        imdb_images = [(i['image'], '%s_%s_%03d' % (actor_name, i['title'], count), i['thumb'])
                       for count, i in enumerate(imdb_image_info, rolling_count+1)]
    ...
    def people_tagged_image_results(self):
        ...
        actor_name, actor_id = self.params['actor_name'], self.params['actor_id']
        try:
            results = tmdb_people_full_info(actor_id)['tagged_images']
        except:
            return
        image_info = results['results']
        image_info.sort(key=lambda x: x['file_path'])
        all_images = [(tmdb_image_base % ('original', i['file_path']), (i['media']['title'])
                       if 'title' in i['media'] else i['media']['name']) for i in image_info]
        image_action = json.dumps(
            {'mode': 'imageviewer', 'all_images': all_images})
        self.list_items = list(builder())
        self.next_page_params = {
    ...
    def browser_image(self, folder_path, return_items=False):
        ...
        all_images = [(os.path.join(folder_path, i), i)
                      for i in image_info if i in thumbs_info]
        image_action = json.dumps(
            {'mode': 'imageviewer', 'all_images': all_images, 'page_no': 'final_page'})
        self.list_items = list(builder())
        self.next_page_params = {}
        if return_items:
    ...
    def delete_image(self):
        ...
        hide_busy_dialog()
        set_property('fenda.delete_image_finished', 'true')
