...
def pm_torrent_cloud(folder_id=None, folder_name=None):
        ...
        for count, item in enumerate(cloud_files, 1):
            try:
                ...
                if file_type == 'folder':
                    ...
                    url_params = {'mode': 'premiumize.pm_torrent_cloud', 'id': item['id'], 'folder_name': normalize(
                ...
                listitem.setArt({'icon': default_pm_icon, 'poster': default_pm_icon,
                                'thumb': default_pm_icon, 'fanart': fanart, 'banner': default_pm_icon})
    ...
    try:
        ...
        cloud_files = [i for i in cloud_files if ('link' in i and i['link'].lower(
        ).endswith(tuple(extensions))) or i['type'] == 'folder']
...
def pm_transfers():
    def _builder():
        for count, item in enumerate(transfer_files, 1):
            try:
                cm = []
                file_id, status, progress, name = item['file_id'], item['status'], item['progress'], clean_file_name(
                    item['name']).upper()
                ...
                if file_type == 'folder':
                    ...
                    url_params = {'mode': 'premiumize.pm_torrent_cloud',
                                  'id': item['folder_id'], 'folder_name': normalize(item['name'])}
                else:
                    is_folder = False
...
def pm_account_info():
    try:
        ...
        points_used = int(math.floor(
            float(account_info['space_used']) / 1073741824.0))
        space_used = float(int(account_info['space_used']))/1073741824
        percentage_used = str(
            round(float(account_info['limit_used']) * 100.0, 1))
        body = []
        append = body.append
