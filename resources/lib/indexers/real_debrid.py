...
default_rd_icon, fanart = kodi_utils.get_icon(
            ...
            try:
                ...
                if linked_folder:
                    ...
                    display = '%02d | [B]%s[/B] | [I]%s [/I]' % (
                        count, folder_str, clean_folder_name)
                url_params = {
                    'mode': 'real_debrid.browse_rd_cloud', 'id': folder_id}
                link_folders_add = {'mode': 'link_folders_choice',
                                    'service': 'RD', 'folder_id': folder_id, 'action': 'add'}
                ...
                listitem.setArt({'icon': default_rd_icon, 'poster': default_rd_icon,
                                'thumb': default_rd_icon, 'fanart': fanart, 'banner': default_rd_icon})
                info_tag = listitem.getVideoInfoTag()
                info_tag.setMediaType('video')
                info_tag.setPlot(' ')
                listitem.setProperty('fenda.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': clean_folder_name, 'iconImage': default_rd_icon,
                                                                                  'service': 'RD'}))
                yield (url, listitem, True)
            except:
                pass
    try:
        cloud_files = [i for i in RealDebrid.user_cloud()
                       if i['status'] == 'downloaded']
    except:
        cloud_files = []
    if cloud_files:
        assigned_content = gather_assigned_content("'Fenda_RD_%'")
    ...
    def _builder():
        for count, item in enumerate(downloads, 1):
            try:
                cm = []
                cm_append = cm.append
                datetime_object = jsondate_to_datetime(
                    item['generated'], '%Y-%m-%dT%H:%M:%S.%fZ', remove_time=True)
                filename, size = item['filename'], float(
                    int(item['filesize']))/1073741824
                ...
                info_tag.setMediaType('video')
                info_tag.setPlot(' ')
                listitem.setProperty('fenda.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': default_rd_icon,
                                                                                  'action': 'cloud.realdebrid_direct'}))
                yield (url, listitem, True)
            except:
                pass
    try:
        downloads = [i for i in RealDebrid.downloads(
...
def browse_rd_cloud(folder_id):
    def _builder():
        for count, item in enumerate(pack_info, 1):
            try:
                cm = []
                name, url_link, size = item['path'], item['url_link'], float(
                    int(item['bytes']))/1073741824
                if name.startswith('/'):
                    name = name.split('/')[-1]
                ...
                url_params = {'mode': 'real_debrid.resolve_rd',
                              'url': url_link, 'play': 'true'}
                url = build_url(url_params)
                down_file_params = {'mode': 'downloader', 'name': name, 'url': url_link,
    ...
    files = [i for i in torrent_files['files'] if i['selected']
             == 1 and i['path'].lower().endswith(tuple(extensions))]
    file_info = [dict(i, **{'url_link': torrent_files['links'][idx]})
...
def resolve_rd(params):
    ...
    FendaPlayer().run(resolved_link, 'video')
...
def rd_account_info():
    try:
        ...
        expires = datetime_workaround(
            account_info['expiration'], '%Y-%m-%dT%H:%M:%S.%fZ')
