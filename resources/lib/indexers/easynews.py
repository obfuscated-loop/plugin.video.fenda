...
default_easynews_icon, fanart, sleep = kodi_utils.get_icon(
    ...
    def _builder():
        for count, item in enumerate(files, 1):
            try:
                ...
                size = str(
                    round(float(int(item_get('rawSize')))/1048576000, 1))
                length = item_get('runtime', '0')
                display = '%02d | [B]%s[/B] | [B]%sGB | %sMINS | [/B][I]%s [/I]' % (
                    count, display_res, size, length, name)
                ...
                listitem.setArt({'icon': thumbnail, 'poster': thumbnail, 'thumb': thumbnail,
                                'fanart': fanart, 'banner': default_easynews_icon})
                info_tag = listitem.getVideoInfoTag()
                info_tag.setMediaType('video')
                info_tag.setPlot(' ')
                listitem.setProperty('fenda.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': default_easynews_icon,
